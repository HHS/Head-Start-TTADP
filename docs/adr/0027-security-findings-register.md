...
Until the grouped Jira tickets exist, the register may use explicit temporary ticket placeholders such as `TEMP-SAST-HARDCODED-HMAC` or `TEMP-SCA-CURRENT-ADVISORIES`. These placeholders are only a short-lived bridge and must be replaced with real Jira keys before the final approval record is considered complete.
