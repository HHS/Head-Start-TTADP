...
To run Biome lint checks, use `yarn lint`. To apply autofixes where supported, use `yarn lint:fix`.
...
- `ActivityReport` fixtures used in tests that trigger a submission or approval transition (`submissionStatus → SUBMITTED` or `calculatedStatus → APPROVED`) must include all Joi-required fields — currently `creatorRole`, `language`, `activityReason`, `deliveryMethod`, `duration`, `startDate`, `endDate`, `activityRecipientType`, `targetPopulations`, `participants`, `ttaType`, and `numberOfParticipants` (non-hybrid). Missing any of these will cause the `beforeUpdate` hook to throw a Joi `ValidationError` and the test will fail. See `src/models/schemas/activityReport.js` for the full schema. When adding a new required field to the schema, update all fixture files that define `ActivityReport` objects: `src/scopes/activityReport/testHelpers.js`, `src/models/hooks/testHelpers.js`, and any inline fixtures in test files under `src/models/tests/` and `src/services/`.
...
test("get /endpoint", async ({ request }) => {
  ...
  const schema = Joi.array().items(
    Joi.object({
      key: Joi.string().valid("keyA", "keyB", "keyC").required(),
...
yarn test:e2e:api    # Playwright API tests
yarn test:bdd        # Cucumber BDD tests
```
