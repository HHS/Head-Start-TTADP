...
- **Soft deletes**: `DeliveredReviews`, `Citations`, and `Categories` use paranoid soft deletes (`deletedAt`).
...
| `categoryId` | INTEGER | FK to `Categories.id` — the normalized category row for this `guidance_category` value |
...
Mostly to keep a list of active category values that can be referenced by TTA Hub, Categories is maintained as a dimensional table of unique `guidance_category` values observed across all Citations. It contains one row per distinct `MonitoringStandards.guidance` value seen in reports delivered since the monitoring start date. Populated and maintained by the same update script that manages Citations; a row is soft-deleted when no non-deleted Citation references that category name.
...
- **Models**: `src/models/deliveredReview.js`, `src/models/citation.js`, `src/models/category.js`, `src/models/deliveredReviewCitation.js`, `src/models/grantDeliveredReview.js`, `src/models/grantCitation.js`
