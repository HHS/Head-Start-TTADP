...
describe('ssdi', () => {
  ...
  describe('safeResolvePath', () => {
    ...
    it('should throw an error if the resolved path is outside BASE_DIRECTORY', () => {
      expect(() => safeResolvePath('../../outside/path.sql')).toThrow('Attempted to access a file outside the allowed directory.')
    })
  })
  ...
  describe('listQueryFiles', () => {
    ...
    it('should return a list of SQL files when the user has permission', async () => {
      ...
      fs.promises.stat.mockImplementation((filePath) =>
        !filePath.endsWith('sql') ? { isDirectory: () => true, isFile: () => false } : { isFile: () => true }
      )
     ...
     SELECT * FROM test;`)
      ...
      const checkFolderPermissionsMock = jest.spyOn(UserPolicy.prototype, 'checkPermissions').mockResolvedValue(true)
    ...
    it('should only return SQL files when user has permission and ignore non-SQL files', async () => {
     ...
     SELECT * FROM test;`)
      ...
      expect(result).toEqual([expect.objectContaining({ filePath: expect.stringContaining('file1.sql') })])
    ...
    it('should skip files if the user does not have permission', async () => {
     ...
     SELECT * FROM test;`)
      ...
      const checkFolderPermissionsMock = jest
        ...
        .mockImplementation((targetString, _matchStrings, _featureFlag) => targetString.includes('file1.sql'))
  ...
  describe('executeQuery', () => {
    ...
    it('should throw an error if the file path is not a string', async () => {
      await expect(executeQuery(123)).rejects.toThrow('The "paths[1]" argument must be of type string. Received type number (123)')
    })
    ...
    it('should throw an error if the JSON header cannot be read from the file', async () => {
      ...
      await expect(executeQuery('invalid.sql')).rejects.toThrow(`Unable to read and parse the JSON header from file: ${mockResolvedPath}`)
    })
