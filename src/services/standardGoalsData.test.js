...
import { standardGoalsForRecipient, createObjectivesForGoal } from './standardGoals'
  ...
  describe('standardGoalsForRecipient Only Approved Topics', () => {
    ...
    afterAll(async () => {
      ...
      await ActivityReportObjectiveTopic.destroy({
        where: {
          topicId: [topicOnApprovedReport.id, topicOnNonApprovedReport.id],
        },
      })

      await Topic.destroy({
        where: {
          id: [topicOnApprovedReport.id, topicOnNonApprovedReport.id],
        },
        force: true,
      })
      ...
      await ActivityReportObjective.destroy({
        where: {
          id: [activityReportObjectiveForApprovedReport.id, activityReportObjectiveForNonApprovedReport.id],
        },
      ...
      await Objective.destroy({
        where: {
          id: [approvedObjective.id, nonApprovedObjective.id],
        },
        force: true,
      })
    ...
    it('returns goals with topics and citations only from activity reports with calculatedStatus=APPROVED', async () => {
      const result = await standardGoalsForRecipient(recipientForTopics.id, grant.regionId, {}, true)
