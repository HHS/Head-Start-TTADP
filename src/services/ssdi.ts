...
const checkFolderPermissions = async (user, scriptPath: string) =>
  new User(user).checkPermissions(scriptPath, ['dataRequests/ohs', 'dataRequests/internal'], 'ssdi_restricted')
...
const isValidJsonHeader = (json: unknown): boolean =>
  json && typeof (json as HeaderStructure).name === 'string' && Array.isArray((json as HeaderStructure).filters)
...
const generateArtificialFilters = (cachedFile: CachedFile, currentUserId: number): Filters => {
  ...
  if (cachedFile.jsonHeader.output.multipleDataSets) {
    ...
    artificialFilters.dataSetSelection = {
      ...
      options: cachedFile.jsonHeader.output.multipleDataSets.map((dataSet) => dataSet.name).filter((name) => name !== 'process_log'),
      defaultValues: cachedFile.jsonHeader.output.multipleDataSets.map((dataSet) => dataSet.name).filter((name) => name !== 'process_log'),
    }
  }
...
const readFiltersFromFile = async (filePath: string, currentUserId: number, returnOptions = false): Promise<Filters> => {
...
const preprocessAndValidateFilters = (filters: Filters, input: Record<string, any>) => {
  ...
  Object.keys(input).forEach((key) => {
    ...
    if (suffix) {
      ...
      const isDateArrayFilter = (suf, filterType) => (suf === '.win' || suf === '.in') && filterType === FilterType.DateArray

      const isArrayWithSeparator = (arr, separator) => Array.isArray(arr) && arr.some((item) => typeof item === 'string' && item.includes(separator))

      const splitValue = (value, separator) =>
        Array.isArray(value) ? value.flatMap((item) => (typeof item === 'string' ? item.split(separator) : item)) : value.split(separator)
