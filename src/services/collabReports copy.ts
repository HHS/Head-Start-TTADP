...
type ReportGoalInput = number | string | {
  goalTemplateId?: number | string;
  goalTemplate?: { id: number | string };
  value?: number | string;
};
...
async function saveReportGoals(collabReportId: number, reportGoals: ReportGoalInput[]) {
  ...
  const goalTemplateIdFromInput = (goal: ReportGoalInput) => {
    if (goal === null || goal === undefined) return null;
    if (typeof goal === 'number') return goal;
    if (typeof goal === 'string') {
      const parsed = parseInt(goal, DECIMAL_BASE);
      return Number.isNaN(parsed) ? null : parsed;
    }

    const candidate = goal.goalTemplateId ?? goal.goalTemplate?.id ?? goal.value;
    if (typeof candidate === 'number') return candidate;
    if (typeof candidate === 'string') {
      const parsed = parseInt(candidate, DECIMAL_BASE);
      return Number.isNaN(parsed) ? null : parsed;
    }
  ...
  if (reportGoals && reportGoals.length > 0) {
    const goalTemplateIds = reportGoals
      .map(goalTemplateIdFromInput)
    ...
    const newGoals = [...new Set(goalTemplateIds)].map((goalTemplateId) => ({
      collabReportId,
      goalTemplateId,
    }));
