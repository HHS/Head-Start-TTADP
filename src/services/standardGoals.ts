...
export async function removeObjectivesFromReport(objectivesToRemove, reportId) {
  ...
  const objectivesToDefinitelyDestroy = objectivesToPossiblyDestroy.filter((o) => !o.activityReports.length)
...
export async function removeActivityReportObjectivesFromReport(reportId, objectiveIdsToRemove) {
  const activityReportObjectivesToDestroy =
    Array.isArray(objectiveIdsToRemove) && objectiveIdsToRemove.length > 0
...
export async function removeUnusedGoalsObjectivesFromReport(reportId, currentObjectives) {
  ...
  const activityReportObjectivesToRemove = previousActivityReportObjectives.filter((aro) => !currentObjectiveIds.includes(aro.objectiveId))
...
export async function createObjectivesForGoal(goal, objectives, reportId) {
  ...
  */
  ...
  return Promise.all(
    objectives
      .filter((o) => o.title || o.ttaProvided || o.topics?.length || o.resources?.length || o.courses?.length || o.files?.length)
      .map(async (objective, index) => {
        const {
          id,
          ids,
          isNew,
...
export async function saveStandardGoalsForReport(goals, userId, report) {
  const goalsWithGrants = (goals || []).filter((goal) => goal && Array.isArray(goal.grantIds) && goal.grantIds.length > 0)
  ...
  let updatedGoals = goalsWithGrants.length
    ? await Promise.all(
        goalsWithGrants.map(async (goal) => {
          // Loops recipients update / create goals.
          // eslint-disable-next-line implicit-arrow-linebreak
          const goalTemplate = goal.goalTemplateId ? await GoalTemplate.findByPk(goal.goalTemplateId) : null
          ...
          return Promise.all(
            goal.grantIds.map(async (grantId) => {
              ...
              const isActivelyBeingEditing = goal.isActivelyBeingEditing ? goal.isActivelyBeingEditing : false
              ...
              const newGoalObjectives = await createObjectivesForGoal(newOrUpdatedGoal, goal.objectives, reportId)
  ...
  await Promise.all(
    uniqueObjectives.map(async (savedObjective) => {
      const {
        status,
        index,
        topics,
  ...
  const goalsToRemove = previousActivityReportGoals.filter((arg) => !currentGoalIds.includes(arg.goalId)).map((r) => r.goalId)
 ...
 *
 */
export async function goalForRtr(grantId: number, goalTemplateId: number, status: string[] = [GOAL_STATUS.NOT_STARTED, GOAL_STATUS.IN_PROGRESS]) {
  return Goal.findOne({
    where: {
      grantId,
      goalTemplateId,
    ...
    attributes: ['id', 'name', 'status', 'goalTemplateId', 'grantId'],
    include: [
      {
        model: Grant,
        as: 'grant',
        attributes: ['numberWithProgramTypes', 'id', 'number'],
        include: [
          {
            model: Program,
            as: 'programs',
            attributes: ['grantId', 'programType'],
          },
        ],
      },
      {
        attributes: ['id', 'title', 'onAR', 'status', 'objectiveTemplateId', 'createdAt', 'updatedAt', 'createdVia', 'onApprovedAR'],
        model: Objective,
        as: 'objectives',
        separate: true,
        where: {
      ...
      },
      {
        ...
        as: 'responses',
        attributes: ['id', 'goalId', 'response'],
      },
    ],
  })
}

export async function getStardard(standardGoalId: number, grantId: number, rootCauses?: Array<string>) {
    ...
    include: [
      {
        ...
        where: {
          grantId,
          status: [GOAL_STATUS.NOT_STARTED, GOAL_STATUS.IN_PROGRESS],
        },
        required: false,
      },
      {
...
export async function newStandardGoal(
  ...
  return goalForRtr(grantId, standardGoalId)
}
...
export async function updateExistingStandardGoal(
  ...
  if (objectives.length) {
    updatedObjectives = await Promise.all(
      objectives.map(async (objective) => {
        if (objective.objectiveTemplateId) {
          const orOptions = [{ title: objective.title }] as {
            id?: number
            title?: string
          }[]
        ...
        const existingObjective = await (objective.id
        ...
        if (existingObjective) {
          ...
          const objectiveStatus =
            existingObjective.status === OBJECTIVE_STATUS.COMPLETE || existingObjective.status === OBJECTIVE_STATUS.SUSPENDED
...
export async function standardGoalsForRecipient(
  ...
  const goalRows = await Goal.findAll({
    ...
    order: [...orderGoalsBy(sortBy, sortDir), [{ model: GoalStatusChange, as: 'statusChanges' }, 'createdAt', 'ASC']],
  })
  ...
  const processedRows = goalRows.map((current) => {
    ...
    const processedObjectives = reduceObjectivesForRecipientRecord(current, goalToAdd, [current.grant.number])
  ...
  const statuses = processedRows.reduce(
    (
      accumulator: {
        key: number
      },
