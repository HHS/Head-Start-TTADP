...
describe('standardGoalsForRecipient sorting tests', () => {
  ...
  beforeAll(async () => {
    ...
    const goalIds = [notStartedGoal?.id, inProgressGoal?.id, suspendedGoal?.id, closedGoal?.id].filter(Boolean)
  ...
  it('sorts goals by status with ASC direction showing Not Started first', async () => {
    ...
    const expectedOrder = [GOAL_STATUS.NOT_STARTED, GOAL_STATUS.IN_PROGRESS, GOAL_STATUS.SUSPENDED, GOAL_STATUS.CLOSED]
