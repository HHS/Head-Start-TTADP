...
describe('Recipient DB service', () => {
  ...
  describe('recipientsByName', () => {
    ...
    it('finds based on recipient name', async () => {
      const foundRecipients = await recipientsByName('apple', await regionToScope(1), 'name', 'asc', 0, [1]);
    ...
    it('finds based on recipient id', async () => {
      const foundRecipients = await recipientsByName('5555', await regionToScope(1), 'name', 'asc', 0, [1]);
    ...
    it('finds based on region', async () => {
      const foundRecipients = await recipientsByName('banana', await regionToScope(2), 'name', 'asc', 0, [1]);
      expect(foundRecipients.rows.length).toBe(1);
    ...
    it('sorts based on program specialist', async () => {
      const foundRecipients = await recipientsByName('apple', await regionToScope(1), 'programSpecialist', 'asc', 0, [1]);
      expect(foundRecipients.rows.length).toBe(3);
      expect(foundRecipients.rows.map((g) => g.id).sort()).toStrictEqual([66, 63, 68].sort());
    });

    it('sorts based on grant specialist', async () => {
      const foundRecipients = await recipientsByName('apple', await regionToScope(1), 'grantSpecialist', 'asc', 0, [1]);
      expect(foundRecipients.rows.length).toBe(3);
      expect(foundRecipients.rows.map((g) => g.id).sort()).toStrictEqual([68, 63, 66].sort());
    });

    it('respects sort order', async () => {
      const foundRecipients = await recipientsByName('apple', await regionToScope(1), 'name', 'desc', 0, [1]);
      expect(foundRecipients.rows.length).toBe(3);
      expect(foundRecipients.rows.map((g) => g.id).sort()).toStrictEqual([66, 63, 68].sort());
    });

    it('respects the offset passed in', async () => {
      const foundRecipients = await recipientsByName('apple', await regionToScope(1), 'name', 'asc', 1, [1]);
    ...
    it('finds inactive grants that fall in the accepted range', async () => {
      const foundRecipients = await recipientsByName('Pumpkin', await regionToScope(1), 'name', 'asc', 0, [1]);
      expect(foundRecipients.rows.length).toBe(2);
