...
import db, { SessionReportPilotFile, SessionReportPilotSupportingAttachment, Grant, Recipient, SessionReportPilot } from '../models'
...
describe('session reports service', () => {
  ...
  describe('createSession', () => {
    ...
    it('throws an error when the event is not found', async () => {
      await expect(createSession({ eventId: 999999, data: { card: 'ace' } })).rejects.toThrow('Event with id 999999 not found')
    })
  })
  ...
  describe('getPossibleSessionParticipants', () => {
    ...
    it('retrieves participants from alternate states', async () => {
      ...
      expect(participants.map((p) => p.id)).toEqual(expect.arrayContaining([recipient.id, alternateRecipient.id]))
    })
  })
  ...
  describe('getSessionReports', () => {
    ...
    it('should sort by sessionName ASC', async () => {
      ...
      expect(result.rows.length).toBeGreaterThanOrEqual(0)
      const pairs = result.rows.slice(0, -1).map((row, idx) => [row.sessionName || '', result.rows[idx + 1].sessionName || ''])
      pairs.forEach(([current, next]) => {
    ...
    it('should sort by startDate ASC with date casting', async () => {
      ...
      const pairs = result.rows
        .slice(0, -1)
        .map((row, idx) => [new Date(row.startDate || 0).getTime(), new Date(result.rows[idx + 1].startDate || 0).getTime()])
      pairs.forEach(([current, next]) => {
        expect(current).toBeLessThanOrEqual(next)
      })
    })
    ...
    it('should sort by endDate ASC', async () => {
      ...
      const pairs = result.rows
        .slice(0, -1)
        .map((row, idx) => [new Date(row.endDate || 0).getTime(), new Date(result.rows[idx + 1].endDate || 0).getTime()])
      pairs.forEach(([current, next]) => {
        expect(current).toBeLessThanOrEqual(next)
      })
    })
    ...
    it('should sort by eventId ASC', async () => {
      ...
      expect(result.rows.length).toBeGreaterThanOrEqual(0)
      const pairs = result.rows.slice(0, -1).map((row, idx) => [row.eventId || '', result.rows[idx + 1].eventId || ''])
      pairs.forEach(([current, next]) => {
        expect(current.localeCompare(next)).toBeLessThanOrEqual(0)
      })
    })
    ...
    it('should sort by eventName DESC', async () => {
      ...
      expect(result.rows.length).toBeGreaterThanOrEqual(0)
      const pairs = result.rows.slice(0, -1).map((row, idx) => [row.eventName || '', result.rows[idx + 1].eventName || ''])
      pairs.forEach(([current, next]) => {
        expect(current.localeCompare(next)).toBeGreaterThanOrEqual(0)
      })
    })
