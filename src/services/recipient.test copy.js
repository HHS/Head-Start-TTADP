...
describe('Recipient DB service', () => {
  ...
  describe('missingStandardGoals', () => {
    ...
    it('does not count templates as missing when each grant uses a template that shares a name', async () => {
      ...
      const grantOneId = faker.datatype.number({ min: 1000 });
      const grantTwoId = faker.datatype.number({ min: 1000 });

      const recipient = {
        id: faker.datatype.number({ min: 1000 }),
        grants: [
          { id: grantOneId, status: 'Active' },
          { id: grantTwoId, status: 'Active' },
        ],
      };
      ...
      const goalTemplates = [
        {
          ...
          goals: [{ grantId: grantOneId }],
        },
        {
          id: faker.datatype.number({ min: 1000 }),
          templateName: duplicateTemplateName,
          goals: [{ grantId: grantTwoId }],
        },
      ];

      const findAllSpy = jest.spyOn(GoalTemplate, 'findAll').mockResolvedValue(goalTemplates);
