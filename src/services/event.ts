...
import { Op, cast, type WhereOptions as SequelizeWhereOptions } from 'sequelize'
...
import type { EventShape, CreateEventRequest, UpdateEventRequest, SessionShape, TRAlertShape } from './types/event'
import EventReport from '../policies/event'
import { trEventComplete } from '../lib/mailer'
import { FILE_STATUSES } from '../constants'

const { EventReportPilot, SessionReportPilot, GoalTemplate, User, EventReportPilotNationalCenterUser, File, sequelize } = db
...
export async function findEventHelper(where, plural = false): Promise<EventShape | EventShape[] | null> {
  const query = {
    attributes: ['id', 'ownerId', 'pocIds', 'collaboratorIds', 'regionId', 'data', 'updatedAt', 'version'],
    where,
    include: [
      {
...
const checkSessionForCompletion = (session: SessionShape, event: EventShape, checker: TChecker, missingSessionInfo: TRAlertShape[]) => {
...
async function enrichAlertsWithUserData(alerts: TRAlertShape[]): Promise<TRAlertShape[]> {
  ...
  return alerts.map((alert) => {
    const enrichedAlert: TRAlertShape = {
      ...
      collaboratorNames: alert.collaboratorIds.map((id) => userMap.get(id)).filter((name): name is string => Boolean(name)),
    }
...
export async function getTrainingReportAlertsForUser(userId: number, regions: number[]): Promise<TRAlertShape[]> {
  const where = {
    [Op.or]: [
      {
        ownerId: userId,
 ...
 */
export function filterEventSessions(event: EventShape, userId: number, isAdmin = false): EventShape {
...
export const mapLineToData = (line: Record<string, string>) => {
  ...
  Object.keys(line).forEach((key) => {
    ...
    if (Object.keys(mappings).includes(key)) {
      ...
      data[mappedKey] = toSplit.includes(mappedKey) ? splitPipe(line[key]).map(applyReplacements) : line[key]
    }
  })
...
const checkEventExists = async (eventId: string) => {
  const event = await db.EventReportPilot.findOne({
    ...
    where: {
      id: {
        [Op.in]: sequelize.literal(`(SELECT id FROM "EventReportPilots" WHERE data->>'eventId' = '${eventId}')`),
      },
    },
  })
...
const VALID_STATE_NAMES = [...new Set(ALL_STATES_FLATTENED.map(({ label }) => label.split('(')[0].trim()))]
...
export async function csvImport(buffer: Buffer) {
  ...
  const results = parsed.map(async (line: Record<string, string>) => {
    try {
      const cleanLine = Object.fromEntries(Object.entries(line).map(([key, value]) => [key.trim(), value.trim()]))
      ...
      data.targetPopulations = [...new Set(data.targetPopulations as string[])].filter((target) =>
        [...TARGET_POPULATIONS, ...EVENT_TARGET_POPULATIONS].includes(target)
      )
