...
export async function synchonizeUserDataOnLegacyReports() {
  const reports = await ActivityReport.findAll({
    attributes: ['id', 'legacyId', 'userId', 'imported'],
    where: {
      legacyId: {
        [Op.ne]: null,
      },
  ...
  return Promise.all(reports.map(async (report) => {
    ...
    const { createdBy, modifiedBy, manager } = imported;
    ...
    if (modifiedBy) {
      ...
      if (collaborator) {
        const arCollaborator = await ActivityReportCollaborator.findOne({
          where: {
        ...
        if (!arCollaborator) {
          promises.push(ActivityReportCollaborator.create({
            activityReportId: report.id,
            userId: collaborator.id,
          }));
        }
    ...
    if (manager) {
      ...
      if (approver) {
        const arApprover = await ActivityReportApprover.findOne({
          where: {
            activityReportId: report.id,
            userId: approver.id,
          },
        ...
        if (!arApprover) {
          promises.push(ActivityReportApprover.create({
            activityReportId: report.id,
            userId: approver.id,
          }));
        }
