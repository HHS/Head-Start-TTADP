...
describe('event service', () => {
  ...
  describe('finders', () => {
    ...
    it('findEventsByStatus sort order', async () => {
      ...
      const found = await findEventsByStatus(TRS.NOT_STARTED, [], 11_111, null, true, [{ id: [e1.id, e2.id, e3.id] }])
      ...
      const found2 = await findEventsByStatus(TRS.NOT_STARTED, [], 11_112, null, true, [{ id: [e4.id, e5.id, e6.id] }])
      ...
      const found3 = await findEventsByStatus(TRS.NOT_STARTED, [], 11_113, null, true, [{ id: [e7.id, e8.id, e9.id] }])
...
${email},01-TR-4256,${eventTitle},${typeOfEvent},${ncTwo.name},${trainingType},${reasons},${vision},${targetPopulation},${audience},${poc.name}`
      ...
      expect(result.skipped).toEqual(['Invalid "Event ID" format expected R##-TR-#### received 01-TR-4256'])
    })
...
${reportId},${eventTitle},${typeOfEvent},${ncTwo.name},${trainingType},${reasons},${vision},${targetPopulation},Recipients,${poc.name}`
  ...
  describe('findAllEvents', () => {
    it('should return all events', async () => {
      ...
      expect(events).toEqual(expect.arrayContaining([expect.objectContaining({ id: event1.id }), expect.objectContaining({ id: event2.id })]))
