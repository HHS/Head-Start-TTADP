...
import { findAll, create, updateById, deleteById } from './nationalCenters'
...
describe('nationalCenters service', () => {
  ...
  describe('findAll', () => {
    ...
    let centers
    beforeAll(async () => {
      centers = await db.NationalCenter.bulkCreate([{ name: centerOneName }, { name: centerTwoName }])
    })
  ...
  describe('updateById', () => {
    ...
    it("doesn't update if name hasn't changed", async () => {
      ...
      expect(center.name).toBe(originalCenterName)
      expect(auditLogger.info).toHaveBeenCalledWith(`Name ${center.name} has not changed`)
      expect(auditLogger.info).toHaveBeenCalledWith(`Name ${center.name} has not changed the national center user`)
    })
  ...
  describe('updateById with the firing of hooks', () => {
    ...
    it('updates a nationalCenter and the proper session report', async () => {
      ...
      expect(updatedSessionReport.data.objectiveTrainers.sort()).toEqual([newCenterName, center2.name].sort())

      const reportNotUpdated = await db.SessionReportPilot.findByPk(sessionReport2.id)
      expect(reportNotUpdated.data.objectiveTrainers.sort()).toEqual([centerOneName, center2.name, center3.name].sort())
    })
  })
  describe('deleteById', () => {
    let center
