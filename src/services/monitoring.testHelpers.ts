...
async function createAdditionalMonitoringData(findingId: string, reviewId: string, granteeId: string) {
  const timestamps = {
...
async function destroyMonitoringData(grantNumber: string, reviewId = 'C48EAA67-90B9-4125-9DB5-0011D6D7C808', statusId = 6006) {
  ...
  await MonitoringReviewGrantee.destroy({ where: { reviewId }, force: true, individualHooks: true })
