...
import { createGrant, createRecipient, createGoalTemplate, createReport } from '../testUtils'
...
describe('save standard goals for report', () => {
  ...
  const cleanUpGoalAndAllAssociations = async (
    ...
    const activityReportObjectiveIds = activityReportObjectives.map((activityReportObjective) => activityReportObjective.id)
    ...
    await Promise.all(resources.map((resource) => Resource.destroy({ where: { url: resource.url } })))
  ...
  describe('save standard ar goals', () => {
    describe('creates standard goal for ar', () => {
      ...
      beforeAll(async () => {
        ...
        await Promise.all(mockFiles.map(async (mockFile) => File.findOrCreate({ where: { ...mockFile } })))
      ...
      afterAll(async () => {
        await cleanUpGoalAndAllAssociations(goalTemplate.id, report.id, [grant.id, grantWithSuspendedGoal.id], topics, courses, files, resources)
      })
    ...
    */
    it('creates activity report goals for a grant with a monitoring goal and not for ones without', async () => {
      const goals = [
        {
          goalIds: [],
          grantIds: [grantWithMonitoringGoal.id, grantWithoutMonitoringGoal.id, grantWithClosedMonitoringGoal.id],
          goalTemplateId: monitoringTemplate.id,
          name: monitoringTemplate.templateName,
          status: GOAL_STATUS.NOT_STARTED,
          timeframe: null,
      ...
      const savedGoals = await Goal.findAll({
        where: {
          ...
          grantId: [grantWithMonitoringGoal.id, grantWithoutMonitoringGoal.id, grantWithClosedMonitoringGoal.id],
        },
      })
