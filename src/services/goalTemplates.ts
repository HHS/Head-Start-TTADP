...
*/
export async function getCuratedTemplates(grantIds: number[] | null, includeClosedAndSuspendedGoals = false): Promise<GoalTemplate[]> {
...
@returns An array of FieldPrompts objects containing prompt information and associated
  responses (if any).
*/
export async function getFieldPromptsForCuratedTemplate(goalTemplateId: number, goalIds: number[] | null): Promise<FieldPrompts[]> {
  ...
  const [prompts, responses] = await Promise.all([
    GoalTemplateFieldPromptModel.findAll({
      attributes: [['id', 'promptId'], 'ordinal', 'title', 'prompt', 'hint', 'caution', 'fieldType', 'options', 'validations', 'goalTemplateId'],
    ...
    GoalFieldResponseModel.findAll({
      attributes: [
        ...
        'response',
        [sequelize.fn('ARRAY_AGG', sequelize.fn('DISTINCT', sequelize.col('"GoalFieldResponse"."goalId"'))), 'goalIds'],
      ],
...
*/
export async function getFieldPromptsForActivityReports(goalTemplateId: number, goalIds: number[] | null): Promise<FieldPrompts[]> {
  // Collect the data from the DB for the passed goalTemplate and goalIds
  ...
  const [prompts, responses] = await Promise.all([
    ...
    GoalFieldResponseModel.findAll({
      ...
      group: ['"GoalFieldResponse"."goalTemplateFieldPromptId"', '"GoalFieldResponse"."response"', '"goal"."grantId"'],
      raw: true,
    }),
  ])
  ...
  const restructuredPrompts = responses.reduce((promptsWithResponses: FieldPrompts[], response: PromptResponse) => {
...
*/
export async function setFieldPromptForCuratedTemplate(goalIds: number[], promptId: number, response: string[] | null) {
  ...
  const [currentResponses, promptRequirements] = await Promise.all([
    GoalModel.findAll({
      ...
      where: {
        id: goalIds,
        [Op.or]: [{ '$prompts.id$': { [Op.eq]: sequelize.col('responses.goalTemplateFieldPromptId') } }, { '$"responses"."id"$': null }],
      },
      include: [
        {
          attributes: [],
...
@returns {Promise} - A promise that resolves with an array of voids once all field prompts have
  been set.
*/
export async function setFieldPromptsForCuratedTemplate(goalIds: number[], promptResponses: PromptResponse[]) {
  return Promise.all(promptResponses.map(({ promptId, response }) => setFieldPromptForCuratedTemplate(goalIds, promptId, response)))
}
...
*/
export async function getOptionsByGoalTemplateFieldPromptName(name: string): Promise<FieldPrompts[]> {
