...
import { goalForRtr, newStandardGoal, standardGoalsForRecipient, updateExistingStandardGoal, createObjectivesForGoal } from './standardGoals'
import { createGrant, createRecipient, createGoalTemplate, createReport, destroyReport } from '../testUtils'
...
describe('standardGoal service', () => {
  ...
  describe('rtr form scenarios', () => {
    ...
    describe('newStandardGoal', () => {
      ...
      it('sets the goal status to the provided status', async () => {
        ...
        const g2 = await newStandardGoal(grant.id, goalTemplateNoPrompt.id, [], [], GOAL_STATUS.IN_PROGRESS)
        expect(g2).toBeDefined()
    ...
    describe('updateExistingStandardGoal', () => {
      ...
      it('updates existing root causes', async () => {
        const g = await updateExistingStandardGoal(grant.id, goalTemplate.id, [], ['Option 1', 'Option 2'])
  ...
  describe('standardGoalsForRecipient', () => {
    ...
    afterAll(async () => {
      ...
      await GoalCollaborator.destroy({
        where: {
          goalId: [firstGoalForFirstTemplate.id, secondGoalForFirstTemplate.id, firstGoalForSecondTemplate.id, secondGoalForSecondTemplate.id],
        },
        force: true,
      })
      ...
      await ActivityReportObjective.destroy({
        where: {
          activityReportId: [activityReportOne.id, activityReportTwo.id, activityReportThree.id, activityReportFour.id],
        },
        individualHooks: true,
        force: true,
      })
      ...
      await Goal.destroy({
        where: {
          id: [firstGoalForFirstTemplate.id, secondGoalForFirstTemplate.id, firstGoalForSecondTemplate.id, secondGoalForSecondTemplate.id],
        },
        individualHooks: true,
        force: true,
      })
    ...
    it('retrieves standard goals for a recipient with correct structure and data', async () => {
      ...
      const secondTemplateGoal = result.goalRows.find((g) => g.goalTemplateId === secondGoalTemplate.id)
  ...
  describe('standardGoalsForRecipient Only Approved Objectives Param', () => {
    ...
    afterAll(async () => {
      ...
      await Objective.destroy({
        where: {
          id: [createdViaRtrObjective.id, createdViaArButNotApprovedObjective.id, createdViaArAndApprovedObjective.id],
        },
        force: true,
      })
    ...
    it('returns goals for recipient with default params', async () => {
      ...
      const createdViaRtrObjectiveToAssert = result.goalRows[0].objectives.find((o) => o.title === 'Created via RTR Objective')
      ...
      const createdViaArAndApprovedObjectiveToAssert = result.goalRows[0].objectives.find((o) => o.title === 'Created via AR and approved Objective')
      expect(createdViaArAndApprovedObjectiveToAssert).toBeDefined()
    })

    it('returns only approved objectives and rtr', async () => {
      const result = await standardGoalsForRecipient(recipientForParam.id, grant.regionId, {}, true)
      expect(result.count).toBe(1)
  ...
  describe('standardGoalsForRecipient includes Curated and Automatic templates', () => {
    ...
    it('returns goals for both Curated and Automatic creation methods', async () => {
      ...
      expect(templateIds).toEqual(expect.arrayContaining([curatedTemplate.id, automaticTemplate.id]))
    })
  })
