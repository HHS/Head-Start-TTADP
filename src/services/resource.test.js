...
import { SOURCE_FIELD, NEXTSTEP_NOTETYPE, OBJECTIVE_STATUS, GOAL_STATUS } from '../constants'
...
describe('resource', () => {
  ...
  describe('Resource Table', () => {
    describe('findOrCreateResource', () => {
      ...
      afterAll(async () => {
        ...
        await Resource.destroy({
          where: {
            id: [createdECLKCResource ? createdECLKCResource.id : 0, createdECLKCResource2 ? createdECLKCResource2.id : 0],
          },
        ...
        await Resource.destroy({
          where: {
            id: [existingHeadStartResource ? existingHeadStartResource.id : 0, createdHeadStartResource ? createdHeadStartResource.id : 0],
          },
          individualHooks: false,
          force: true,
        })
    ...
    describe('findOrCreateResources', () => {
      ...
      it('expected usage, concurrent existing', async () => {
        const [resources1, resources2] = await Promise.all([findOrCreateResources(urls), findOrCreateResources(urls)])
        expect(resources2.sort(sorter)).toMatchObject(resources1.sort(sorter))
      })
  ...
  describe('Helper functions', () => {
    ...
    describe('transformRecordByURLToResource', () => {
      ...
      let resources
      beforeEach(() => {
        records = [{ url: 'http://google.com' }, { url: 'http://github.com' }, { url: 'http://youtube.com' }]
        resources = [
      ...
      it('expected usage', () => {
        expect(transformRecordByURLToResource(records, resources)).toMatchObject([{ resourceId: 1 }, { resourceId: 2 }, { resourceId: 3 }])
      })
    ...
    describe('filterResourcesForSync', () => {
      ...
      it('expected usage, empty incomingResources and currentResources', () => {
        incomingResources = []
        currentResources = []
        expect(filterResourcesForSync(incomingResources, currentResources, calculateIsAutoDetectedFunc)).toMatchObject({
          create: [],
          update: [],
          destroy: [],
        })
  ...
  describe('ActivityReports Resource Processing', () => {
    ...
    describe('syncResourcesForActivityReport', () => {
      ...
      beforeAll(async () => {})
      beforeEach(async () => {
        const urls = ['http://google.com', 'http://github.com', 'http://cloud.gov', 'https://adhocteam.us/']
      ...
      it('expected usage, update', async () => {
        ...
        expect(arResources.find((r) => r.resourceId === resources[0].id).sourceFields.length).toEqual(2)
        expect(arResources.find((r) => r.resourceId === resources[0].id).isAutoDetected).toEqual(false)
      })
      ...
      it('expected usage, insert/update/delete', async () => {
        ...
        expect(arResources.find((r) => r.dataValues.resourceId === resources[0].id).dataValues.sourceFields.length).toEqual(2)
        expect(arResources.find((r) => r.dataValues.resourceId === resources[0].id).isAutoDetected).toEqual(false)
      })
    })
    describe('processActivityReportForResourcesById', () => {
      ...
      it('expected usage, add and remove urls', async () => {
        ...
        expect(arResources.find((r) => r.dataValues.resource.dataValues.url === urls[0]).dataValues.sourceFields.sort()).toEqual([
        ...
        expect(arResources[0].dataValues.sourceFields.sort()).toEqual([SOURCE_FIELD.REPORT.RESOURCE])
        ...
        expect(arResources.find((r) => r.dataValues.resource.dataValues.url === urls[1]).dataValues.resourceId).toEqual(
          resources.find((r) => r.url === urls[1]).id
        )
        expect(arResources.find((r) => r.dataValues.resource.dataValues.url === urls[1]).dataValues.sourceFields.sort()).toEqual([
          SOURCE_FIELD.REPORT.RESOURCE,
        ])
  ...
  describe('NextSteps Resource Processing', () => {
    ...
    describe('syncResourcesForNextStep', () => {
      ...
      it('expected usage, insert/update/delete', async () => {
        ...
        expect(nsResources.find((r) => r.resourceId === resources[0].id).sourceFields.length).toEqual(1)
        expect(nsResources[0].isAutoDetected).toEqual(false)
      })
    })
    describe('processNextStepForResourcesById', () => {
      ...
      it('expected usage, with urls', async () => {
        ...
        nsResources.forEach((resource) => {
          expect(resource.dataValues.sourceFields.sort()).toEqual([SOURCE_FIELD.NEXTSTEPS.RESOURCE])
        })
      })
      it('expected usage, add and remove urls', async () => {
  ...
  describe('Goal Resource processing', () => {
    ...
    describe('syncResourcesForGoal', () => {
      ...
      it('expected usage, insert/update/delete', async () => {
        ...
        expect(gResources.find((r) => r.resourceId === resources[0].id).sourceFields.length).toEqual(1)
        expect(gResources.find((r) => r.resourceId === resources[0].id).isAutoDetected).toEqual(false)
      })
    })
  ...
  describe('ActivityReportObjective Resource Processing', () => {
    describe('calculateIsAutoDetectedForActivityReportObjective', () => {
      ...
      it('returns false when title and tta provided change', () => {
        sourceFields = [SOURCE_FIELD.REPORTOBJECTIVE.TITLE, SOURCE_FIELD.REPORTOBJECTIVE.TTAPROVIDED]
        expect(calculateIsAutoDetectedForActivityReportObjective(sourceFields)).toEqual(false)
      })
    ...
    describe('syncResourcesForActivityReportObjective', () => {
      ...
      it('expected usage, insert/update/delete', async () => {
        ...
        expect(oResources.find((r) => r.resourceId === resources[0].id).sourceFields.length).toEqual(1)
        expect(oResources.find((r) => r.resourceId === resources[0].id).isAutoDetected).toEqual(false)
      })
    })
    describe('processActivityReportObjectiveForResourcesById', () => {
      ...
      it('expected usage, with urls', async () => {
        const oResources = await processActivityReportObjectiveForResourcesById(reportObjective.id, urls)
        expect(oResources.length).toEqual(4)
      ...
      it('expected usage, add and remove urls', async () => {
        let oResources = await processActivityReportObjectiveForResourcesById(reportObjective.id, [])
        expect(oResources.length).toEqual(0)

        oResources = await processActivityReportObjectiveForResourcesById(reportObjective.id, [urls[0]])
        expect(oResources.length).toEqual(1)
        expect(oResources[0].dataValues.sourceFields).toEqual([SOURCE_FIELD.REPORTOBJECTIVE.RESOURCE])

        oResources = await processActivityReportObjectiveForResourcesById(reportObjective.id, [urls[1]])
        expect(oResources.length).toEqual(1)
