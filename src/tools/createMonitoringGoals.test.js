...
describe('createMonitoringGoals', () => {
  ...
  const assertMonitoringGoals = async () => {
    ...
    const grant1Goals = await Goal.findAll({
      where: { grantId: grantThatNeedsMonitoringGoal1.id },
    });
    ...
    const grant2Goals = await Goal.findAll({
      where: { grantId: grantThatAlreadyHasMonitoringGoal2.id },
    });
    ...
    const grant3Goals = await Goal.findAll({
      where: { grantId: grantThatFallsBeforeStartDate3.id },
    });
    expect(grant3Goals.length).toBe(0);

    // CASE 4: Does not create a monitoring goal for a grant that falls after the cut off date.
    const grant4Goals = await Goal.findAll({
      where: { grantId: grantThatFallsAfterCutOffDate4.id },
    });
    ...
    const grant6Goals = await Goal.findAll({
      where: { grantId: grantThatsMonitoringReviewStatusIsNotComplete6.id },
    });
    ...
    const grant7Goals = await Goal.findAll({
      where: { grantId: grantThatsMonitoringFindingStatusIsNotActive7.id },
    });
    ...
    const grant8Goals = await Goal.findAll({
      where: { grantId: grantThatsMonitoringReviewReviewTypeIsNotAllowed8.id },
    });
    expect(grant8Goals.length).toBe(0);

    // CASE 9: Creates a monitoring goal for a grant cited in findings.
    const grant9Goals = await Goal.findAll({
      where: { grantId: inactiveGrantThatHasBeenReplacedByActiveGrant9.id },
    });
    ...
    const replacementPairGoals = await Goal.findAll({
      where: {
        grantId: {
          [Op.in]: [grantThatIsInactive5.id, inactiveGrantThatHasBeenReplacedByActiveGrant9.id],
        },
        goalTemplateId: goalTemplate.id,
      },
    });
    ...
    const inactiveGrant9BGoals = await Goal.findAll({
      where: { grantId: inactiveGrantCitedOnly9B.id },
    });
    ...
    const grant10AGoals = await Goal.findAll({
      where: { grantId: grantBeingMonitoredSplit10A.id },
    });
    expect(grant10AGoals.length).toBe(1);
    expect(grant10AGoals[0].goalTemplateId).toBe(goalTemplate.id);

    const grant10CGoals = await Goal.findAll({
      where: { grantId: grantBeingMonitoredSplit10B.id },
    });
    ...
    const grant10BGoals = await Goal.findAll({
      where: { grantId: grantBeingMonitoredSplit10C.id },
    });
    ...
    const grant12Goals = await Goal.findAll({
      where: { grantId: grantWithGoalClosedAfterDelivery12.id },
    });
    ...
    */
    ...
    const grant14Goals = await Goal.findAll({
      where: { grantId: grantToNotCloseMonitoringGoal14.id },
    });
    ...
    const grant15Goals = await Goal.findAll({
      where: { grantId: grantWithApprovedReportsButOpenCitations15.id },
    });
    ...
    const grant16Goals = await Goal.findAll({
      where: { grantId: grantWithNonMonitoringGoalToOpen16.id },
    });
    ...
    const grant17Goals = await Goal.findAll({
      where: { grantId: grantWithNonMonitoringGoalToClose17.id },
    });
  ...
  it('creates one goal for multiple citations on the same grant and skips null reportDeliveryDate reviews', async () => {
    const completeReviewStatus = await MonitoringReviewStatus.findOne({
      where: { name: 'Complete' },
    });
    const activeFindingStatus = await MonitoringFindingStatus.findOne({
      where: { name: 'Active' },
    });
