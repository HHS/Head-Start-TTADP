...
import db, { EventReportPilot, EventReportPilotNationalCenterUser, User, NationalCenter, NationalCenterUser } from '../models'
...
describe('updateCompletedEventReportPilots', () => {
  ...
  it('creates EventReportPilotNationalCenterUser records for completed event report pilots', async () => {
    ...
    expect(erp1Users.map((u) => u.userId)).toEqual(expect.arrayContaining([user1.id, user2.id, user3.id]))
    expect(erp2Users.map((u) => u.userId)).toEqual(expect.arrayContaining([user1.id, user2.id, user3.id]))
