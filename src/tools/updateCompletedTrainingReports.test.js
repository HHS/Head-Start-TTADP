...
describe('updateCompletedTrainingReports', () => {
  ...
  beforeAll(async () => {
    ...
    await TrainingReportNationalCenterUser.destroy({
      where: {
  ...
  afterAll(async () => {
    ...
    await TrainingReport.destroy({ where: { id: [erp1.id, erp2.id, erp3.id] } });
  ...
  it('creates TrainingReportNationalCenterUser records for completed event report pilots', async () => {
    await updateCompletedTrainingReports();

    let erp1Users = await TrainingReportNationalCenterUser.findAll({
      where: { trainingReportId: erp1.id },
    });
    let erp2Users = await TrainingReportNationalCenterUser.findAll({
      where: { trainingReportId: erp2.id },
    });
    let erp3Users = await TrainingReportNationalCenterUser.findAll({
    ...
    erp1Users = await TrainingReportNationalCenterUser.findAll({
      where: { trainingReportId: erp1.id },
    });
    erp2Users = await TrainingReportNationalCenterUser.findAll({
      where: { trainingReportId: erp2.id },
    }); erp3Users = await TrainingReportNationalCenterUser.findAll({
      where: { trainingReportId: erp3.id },
    });
  ...
  it('does not create TrainingReportNationalCenterUser records for report owners without national centers', async () => {
    ...
    const erpWithoutNationalCenter = await TrainingReport.create({
      regionId: 1,
    ...
    const erpNationalCenterUsers = await TrainingReportNationalCenterUser.findAll({
    ...
    await TrainingReport.destroy({ where: { id: erpWithoutNationalCenter.id } });
