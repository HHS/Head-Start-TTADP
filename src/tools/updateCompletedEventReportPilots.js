...
export default async function updateCompletedEventReportPilots() {
  return sequelize.transaction(async () => {
    const completedTrainingReports = await db.EventReportPilot.findAll({
      attributes: ['id', 'ownerId', 'collaboratorIds', 'data'],
      where: {
        data: {
    ...
    const bulkCreate = []
    completedTrainingReports.forEach((report) => {
      ...
      const collaborators = usersWithNationalCenters.filter((user) => report.collaboratorIds.includes(user.id))
    ...
    return Promise.all(bulkCreate.map((item) => db.EventReportPilotNationalCenterUser.findOrCreate({ where: item })))
  })
}
