...
const STATUSES_TO_INCLUDE = [GOAL_STATUS.NOT_STARTED, GOAL_STATUS.IN_PROGRESS, GOAL_STATUS.CLOSED, GOAL_STATUS.SUSPENDED]
...
export default async function goalStatusByGoalName(scopes) {
  const goalsFromDb = await Goal.findAll({
    ...
    attributes: [
      [sequelize.cast(sequelize.fn('COUNT', sequelize.fn('DISTINCT', sequelize.fn('TRIM', sequelize.col('"Goal".name')))), 'int'), 'count'],
      'status',
    ],
    group: ['"Goal".status'],
