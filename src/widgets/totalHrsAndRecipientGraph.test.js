...
import db, { ActivityReport, ActivityRecipient, User, Recipient, Grant, NextStep, Region } from '../models'
...
const reportObject = {
  ...
  activityRecipients: [{ activityRecipientId: GRANT_ID_ONE }, { activityRecipientId: GRANT_ID_TWO }],
...
describe('Total Hrs and Recipient Graph widget', () => {
  ...
  afterAll(async () => {
    const reports = await ActivityReport.findAll({ where: { userId: [mockUser.id, mockUserTwo.id, mockUserThree.id] } })
