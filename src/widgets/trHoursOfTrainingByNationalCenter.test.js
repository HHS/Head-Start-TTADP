...
import db, { EventReportPilot, SessionReportPilot, Recipient, NationalCenter, Grant, User } from '../models'
  ...
  beforeAll(async () => {
    ...
    await createSessionReport({
      ...
      data: {
        ...
        objectiveTrainers: [nationalCenter1.name, `${nationalCenter2.name} ${userCreator.fullName}`],
      },
    })
  ...
  it('ignores trainers that do not match any national center', async () => {
    ...
    await createSessionReport({
      ...
      data: {
        ...
        status: TRAINING_REPORT_STATUSES.IN_PROGRESS,
        objectiveTrainers: ['Unknown Trainer Name That Does Not Match', 'Another Unrecognized Center'],
      },
    })
