...
const formatFunc = ({ level, message, label, timestamp, meta = {} }) => `${timestamp} ${label || '-'} ${level}: ${message} ${JSON.stringify(meta)}`

const stringFormatter = format.combine(format.timestamp(), format.colorize(), format.align(), format.printf(formatFunc))
...
export { logger, auditLogger, requestLogger, errorLogger }
