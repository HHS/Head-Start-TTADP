...
function getRenameCommand(file, date) {
  const dateTimePrefix = new Date().toISOString().replace(/[-:.]/g, '').replace('T', '').slice(0, 14)
...
async function checkFileDatePrefix() {
  ...
  const mainFiles = (await Promise.all(paths.map(async (p) => listTree('origin/main', p)))).flat().map(({ name }) => name)
  const prFiles = (await Promise.all(paths.map(async (p) => listTree(await getCurrentBranch(), p)))).flat().map(({ name }) => name)
  ...
  if (tooManyDigits && tooManyDigits.length) {
    ...
    const cleanedMainFileDates = mainFileDates.map((mdf) =>
      mdf > 99999999999999 ? Math.floor(mdf / (10 ** Math.floor(Math.log10(mdf)) - 99999999999999 + 1)) : mdf
    )
