...
import db from '../..'
import { createReport, createTrainingReport, createRegion, createUser, destroyReport } from '../../../testUtils'
...
describe('purifyFields', () => {
  ...
  describe('live updates', () => {
    ...
    test('properly escaped fields', async () => {
      ...
      await ActivityReport.update({ context: xss }, { where: { id: report.id }, individualHooks: true })
      await ActivityReportApprover.update({ note: xss }, { where: { id: approver.id }, individualHooks: true })
      await EventReportPilot.update({ data: { ...event.data, eventName: xss } }, { where: { id: event.id }, individualHooks: true })
