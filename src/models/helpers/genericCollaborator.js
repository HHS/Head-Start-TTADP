 ...
 */
const createCollaborator = async (genericCollaboratorType, sequelize, transaction, entityId, userId, typeName, linkBack = null) => {
  const collaboratorType = await getIdForCollaboratorType(genericCollaboratorType, sequelize, transaction, typeName)
 ...
 */
const findOrCreateCollaborator = async (genericCollaboratorType, sequelize, transaction, entityId, userId, typeName, linkBack = null) => {
  ...
  let collaborator = await getCollaboratorRecord(genericCollaboratorType, sequelize, transaction, entityId, userId, typeName)

  // If no collaborator record found, create a new one
  if (!collaborator) {
    collaborator = await createCollaborator(genericCollaboratorType, sequelize, transaction, entityId, userId, typeName, linkBack)
  } else {
    collaborator = await sequelize.models[collaboratorDetails[genericCollaboratorType].collaborators].update(
      {
 ...
 */
const currentUserPopulateCollaboratorForType = async (genericCollaboratorType, sequelize, transaction, entityId, typeName, linkBack = null) => {
  ...
  // ID and typeName
  return findOrCreateCollaborator(genericCollaboratorType, sequelize, transaction, entityId, userId, typeName, linkBack)
}
 ...
 * @param {Object} linkBack - The link back object.
 */
const removeCollaboratorsForType = async (genericCollaboratorType, sequelize, transaction, entityId, typeName, linkBack = null) => {
  ...
  const currentCollaboratorsForType = await sequelize.models[collaboratorDetails[genericCollaboratorType].collaborators].findAll({
    where: {
      [collaboratorDetails[genericCollaboratorType].idName]: entityId,
      ...
      (acc, current) => {
        if (current.dataValues.linkBack[linkBackKey].length > 1) {
          ...
          const newLinkBack = {
            ...
            [linkBackKey]: current.dataValues.linkBack[linkBackKey].filter((value) => !linkBackValues.includes(value)),
          }
          acc.updates.push({
