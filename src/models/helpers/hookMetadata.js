...
const hasHookMetadata = (options) => options.hookMetadata !== undefined && options.hookMetadata !== null

// check if the metadata has the key passed.
const hasHookMetadataKey = (options, key) => options.hookMetadata[key] !== undefined && options.hookMetadata[key] !== null
...
const getSingularOrPluralData = (options, singular, plural) => {
  ...
  if (hasHookMetadata(options)) {
    if (hasHookMetadataKey(options, singular) && typeof options.hookMetadata[singular] === 'number') {
      data = [options.hookMetadata[singular]]
    } else if (
      hasHookMetadataKey(options, plural) &&
