...
const autoPopulateHash = (sequelize, instance, options) => {
  ...
  if (Array.isArray(changed) && changed.includes('templateTitle') && instance.templateTitle !== null && instance.templateTitle !== undefined) {
...
const autoPopulateCreationMethod = (sequelize, instance, options) => {
  const changed = instance.changed()
  if (Array.isArray(changed) && (!changed.includes('creationMethod') || instance.creationMethod === null || instance.creationMethod === undefined)) {
...
const propagateTemplateTitle = async (sequelize, instance, options) => {
  const changed = instance.changed()
  if (Array.isArray(changed) && changed.includes('templateTitle') && instance.creationMethod === AUTOMATIC_CREATION) {
    await sequelize.models.Objective.update(
      { title: instance.templateTitle },
      {
        where: {
          [Op.and]: [{ goalTemplateId: instance.id }, { onApprovedAR: false }, { name: { [Op.not]: instance.templateTitle } }],
        },
...
export { autoPopulateTemplateTitleModifiedAt, autoPopulateCreationMethod, propagateTemplateTitle, beforeValidate, beforeUpdate, afterUpdate }
