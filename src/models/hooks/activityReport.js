...
const { OBJECTIVE_STATUS, GOAL_COLLABORATORS, OBJECTIVE_COLLABORATORS } = require('../../constants')
const { auditLogger } = require('../../logger')
const { GOAL_STATUS } = require('../../constants')
const { findOrCreateObjectiveTemplate } = require('./objective')
const { findOrCreateCollaborator, removeCollaboratorsForType } = require('../helpers/genericCollaborator')
...
const copyStatus = (instance) => {
  ...
  if (submissionStatus === REPORT_STATUSES.DRAFT || submissionStatus === REPORT_STATUSES.DELETED) {
...
const moveDraftGoalsToNotStartedOnSubmission = async (sequelize, instance, options) => {
  ...
  if (Array.isArray(changed) && changed.includes('submissionStatus') && instance.submissionStatus === REPORT_STATUSES.SUBMITTED) {
    try {
...
const clearAdditionalNotes = (_sequelize, instance, options) => {
  try {
    if (instance.previous('calculatedStatus') !== REPORT_STATUSES.APPROVED && instance.calculatedStatus === REPORT_STATUSES.APPROVED) {
...
const checkForNewGoalCycleOnApproval = async (_sequelize, instance, _options) => {
  try {
    ...
    if (
      (instance.previous('calculatedStatus') !== REPORT_STATUSES.APPROVED && instance.calculatedStatus === REPORT_STATUSES.APPROVED) ||
      (instance.previous('calculatedStatus') === REPORT_STATUSES.APPROVED && instance.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION) ||
      (instance.previous('calculatedStatus') !== REPORT_STATUSES.SUBMITTED && instance.calculatedStatus === REPORT_STATUSES.SUBMITTED)
    ) {
...
const propagateSubmissionStatus = async (sequelize, instance, options) => {
  ...
  if (Array.isArray(changed) && changed.includes('submissionStatus') && instance.submissionStatus === REPORT_STATUSES.SUBMITTED) {
    ...
    try {
      ...
      const distinctlyTitledObjectives = [...new Map(objectives.map((objective) => [objective.title, objective])).values()]
      ...
      const distinctTemplates = await Promise.all(
        distinctlyTitledObjectives.map(async (objective) =>
          findOrCreateObjectiveTemplate(sequelize, options.transaction, instance.regionId, objective.title, objective.createdAt, objective.updatedAt)
        )
      )
      // Add the corresponding template id to each of the objectives.
      objectives = objectives.map((objective) => {
        const objectiveTemplateId = distinctTemplates.find((dt) => dt.title === objective.title).id
...
const determineObjectiveStatus = async (activityReportId, sequelize, isUnlocked) => {
    ...
    include: [
      {
        ...
        attributes: ['id', 'objectiveId', 'status', 'closeSuspendReason', 'closeSuspendContext'],
        where: {
  ...
  const approvedReports = allReports.filter((report) => report.calculatedStatus === REPORT_STATUSES.APPROVED)
  ...
  if (isUnlocked && !approvedReports.length) {
    ...
    const objectivesToReset = report && report.activityReportObjectives ? report.activityReportObjectives.map((a) => a.objectiveId) : []
    ...
    const objectiveIdsForNotClosedGoals = goalsThatAreNotClosed.flatMap((g) => g.objectives.map((o) => o.id))
  ...
  return Promise.all(
    objectiveIds.map((objectiveId) => {
      ...
      const relevantARs = approvedReports.filter((a) => a.activityReportObjectives.find((aro) => aro.objectiveId === objectiveId))

      // 8. Get all the ARO objectives that are used by these approved activity reports.
      const objectivesToUpdate = relevantARs.flatMap((a) =>
        a.activityReportObjectives.filter((aro) => aro.objectiveId === objectiveId).map((aro) => aro.objective)
      )
      ...
      const latestAR = relevantARs.reduce(
        (r, a) => {
          if (r && r.endDate) {
      ...
      return Promise.all(
        objectivesToUpdate.map(async (objectiveToUpdate) => {
          let newStatus = aro.status || OBJECTIVE_STATUS.NOT_STARTED
          // See TTAHUB-4138 for standard goals objective status logic.
          if (objectiveToUpdate.status === OBJECTIVE_STATUS.IN_PROGRESS && newStatus === OBJECTIVE_STATUS.NOT_STARTED) {
            newStatus = OBJECTIVE_STATUS.IN_PROGRESS
          } else if (objectiveToUpdate.status === OBJECTIVE_STATUS.COMPLETE && newStatus === OBJECTIVE_STATUS.NOT_STARTED) {
            newStatus = OBJECTIVE_STATUS.IN_PROGRESS
          } else if (objectiveToUpdate.status === OBJECTIVE_STATUS.SUSPENDED && newStatus === OBJECTIVE_STATUS.NOT_STARTED) {
          ...
          if (objectiveToUpdate.status === OBJECTIVE_STATUS.NOT_STARTED && newStatus === OBJECTIVE_STATUS.IN_PROGRESS) {
            if (!objectiveToUpdate.firstInProgressAt) {
...
const propagateApprovedStatus = async (sequelize, instance, options) => {
  ...
  if (Array.isArray(changed) && changed.includes('calculatedStatus')) {
    if (instance.previous('calculatedStatus') === REPORT_STATUSES.APPROVED && instance.calculatedStatus !== REPORT_STATUSES.APPROVED) {
      ...
      try {
        objectives = await sequelize.models.Objective.findAll({
          attributes: ['id', [sequelize.literal('count(DISTINCT "activityReports"."calculatedStatus")::int'), 'cntApproved']],
          include: [
            {
              attributes: [],
              model: sequelize.models.ActivityReportObjective,
        ...
        await Promise.all([
          ...
          sequelize.models.Objective.update(
            { onApprovedAR: false },
            {
              where: {
                id: {
                  [Op.in]: objectives.filter((o) => o.dataValues.cntApproved === 0).map((o) => o.id),
                },
                onApprovedAR: true,
              },
              transaction: options.transaction,
      ...
      try {
        goals = await sequelize.models.Goal.findAll({
          attributes: ['id', [sequelize.literal('count(DISTINCT "activityReports"."calculatedStatus")'), 'cntApproved']],
          include: [
            {
              attributes: [],
              model: sequelize.models.Objective,
    ...
    } else if (instance.previous('calculatedStatus') !== REPORT_STATUSES.APPROVED && instance.calculatedStatus === REPORT_STATUSES.APPROVED) {
...
const forceStatusEventOnReopenedGoal = async (instance) => {
  ...
  if (
    ...
    const inProgressGoals = reportGoals.filter((goal) => goal.status === GOAL_STATUS.IN_PROGRESS && goal.isReopened && goal.firstUsage)

    await Promise.all(
      inProgressGoals.map((s) =>
        changeGoalStatus({
          goalId: s.id,
          userId: instance.userId,
          newStatus: GOAL_STATUS.IN_PROGRESS,
...
const automaticGoalObjectiveStatusCachingOnApproval = async (sequelize, instance, options) => {
  ...
  if (
    ...
    await Promise.all(
      goals.map(async (goal) =>
        sequelize.models.ActivityReportGoal.update(
          { status: goal.status },
          {
            where: {
              activityReportId: instance.id,
...
const autoPopulateUtilizer = async (sequelize, instance, options) => {
  ...
  if (
    ...
    const [collaborators, goals, objectives] = await Promise.all([
      ...
      sequelize.models.ActivityReportCollaborator.findAll({
        ...
        where: {
          // Filter the models based on the following conditions
      ...
      sequelize.models.ActivityReportObjective.findAll({
        // Find all ActivityReportObjective models
    ...
    await Promise.all(
      findOrCreateCollaboratorOptions.map((option) =>
...
const autoCleanupUtilizer = async (sequelize, instance, options) => {
  ...
  if (
    ...
    // Find all ActivityReportGoals models
    const [arGoals, arObjectives] = await Promise.all([
      sequelize.models.ActivityReportGoal.findAll({
        attributes: ['goalId'], // Select the 'goalId' attribute for each model
        where: {
          // Filter the models based on the 'activityReportId' matching the 'id' of the 'instance'
    ...
    await Promise.all([
      ...arGoals.map(async (arGoal) =>
        removeCollaboratorsForType('goal', sequelize, options.transaction, arGoal.goalId, GOAL_COLLABORATORS.UTILIZER, {
          activityReportIds: [instance.id],
        })
      ),
      ...arObjectives.map(async (arObjective) =>
        removeCollaboratorsForType('objective', sequelize, options.transaction, arObjective.objectiveId, OBJECTIVE_COLLABORATORS.UTILIZER, {
          activityReportIds: [instance.id],
        })
      ),
    ])
    ...
    if (previousRevision !== null && previousRevision !== currentRevision) {
      ...
      if (process.env.NODE_ENV !== 'test') {
        ...
        if (mesh) {
          ...
          await mesh.broadcastRoom(roomName, 'revision-updated', {
