...
import { currentUserPopulateCollaboratorForType, removeCollaboratorsForType } from '../helpers/genericCollaborator'
...
const autoPopulateLinker = async (sequelize, instance, options) => {
  ...
  return currentUserPopulateCollaboratorForType('objective', sequelize, options.transaction, objectiveId, OBJECTIVE_COLLABORATORS.LINKER, {
...
const autoCleanupLinker = async (sequelize, instance, options) => {
  const { objectiveId, activityReportId } = instance
  return removeCollaboratorsForType('objective', sequelize, options.transaction, objectiveId, OBJECTIVE_COLLABORATORS.LINKER, {
    activityReportIds: [activityReportId],
  })
}
...
export { propagateDestroyToMetadata, recalculateOnAR, afterCreate, beforeValidate, beforeDestroy, afterDestroy }
