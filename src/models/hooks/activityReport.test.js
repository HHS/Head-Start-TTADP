...
import { moveDraftGoalsToNotStartedOnSubmission, propagateSubmissionStatus, revisionBump } from './activityReport'
  ...
  describe('automatic goal status changes', () => {
    ...
    describe('sets the correct objective status', () => {
      ...
      afterAll(async () => {
        ...
        await Objective.destroy({
          where: {
            [db.Sequelize.Op.or]: [{ id: objStatusObjective.id }, { createdViaActivityReportId: [existingReport.id, newReport.id] }],
          },
          force: true, // force to ensure deletion
        })
        await ActivityReportGoal.destroy({
