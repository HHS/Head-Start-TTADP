...
describe('activityReportApprover hooks', () => {
  ...
  describe('afterRestore', () => {
    ...
    it('archives ACTIVITY_REPORT_SUBMITTED_COLLABORATOR notifications when report transitions to APPROVED on restore', async () => {
      const ar = await ActivityReport.create({
        ...draftObject,
        submissionStatus: REPORT_STATUSES.SUBMITTED,
      });
  ...
  describe('afterUpdate', () => {
    ...
    it('archives ACTIVITY_REPORT_SUBMITTED_COLLABORATOR notifications when report transitions to APPROVED', async () => {
      const ar = await ActivityReport.create({
        ...draftObject,
        submissionStatus: REPORT_STATUSES.SUBMITTED,
      });
    ...
    it('does not archive ACTIVITY_REPORT_SUBMITTED_COLLABORATOR notifications when report is not fully approved', async () => {
      const ar = await ActivityReport.create({
        ...draftObject,
        submissionStatus: REPORT_STATUSES.SUBMITTED,
      });
      ...
      // Only one approver approved — report stays SUBMITTED
      const approvals = [
        { activityReportId: ar.id, userId: mockUserIds[0], status: APPROVER_STATUSES.APPROVED },
        { activityReportId: ar.id, userId: mockUserIds[1], status: null },
        { activityReportId: ar.id, userId: mockUserIds[2], status: null },
    ...
    it('archives ACTIVITY_REPORT_SUBMITTED_CREATOR notifications when report transitions to APPROVED', async () => {
      const ar = await ActivityReport.create({
        ...draftObject,
        submissionStatus: REPORT_STATUSES.SUBMITTED,
      });
      ...
      const notification = await Notification.create({
        entityId: ar.id,
        type: NOTIFICATION_TYPES.ACTIVITY_REPORT_SUBMITTED_CREATOR,
      });
