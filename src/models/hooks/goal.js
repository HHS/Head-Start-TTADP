...
const autoPopulateCreator = async (sequelize, instance, options) => {
  ...
  return currentUserPopulateCollaboratorForType('goal', sequelize, options.transaction, goalId, GOAL_COLLABORATORS.CREATOR)
}

const autoPopulateEditor = async (sequelize, instance, options) => {
  const { id: goalId } = instance
  const changed = instance.changed()
  if (Array.isArray(changed) && changed.includes('name') && instance.previous('name') !== instance.name) {
    return currentUserPopulateCollaboratorForType('goal', sequelize, options.transaction, goalId, GOAL_COLLABORATORS.EDITOR)
  }
  ...
  if (Array.isArray(changed) && changed.includes('name')) {
    ...
    await Promise.all(
      events.map(async (event) => {
        ...
        await sequelize.models.EventReportPilot.update({ data }, { where: { id: ev.id } })
      })
    )
  }
}
...
const createInitialStatusChange = async (sequelize, instance, options) => {
  ...
  const goalCollaborator = creatorType
    ? await sequelize.models.GoalCollaborator.findOne({
        where: {
          goalId: instance.id,
