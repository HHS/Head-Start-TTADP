...
import { GOAL_STATUS, CLOSE_SUSPEND_REASONS, REPORT_STATUSES } from '@ttahub/common'
import {
  sequelize,
  GoalStatusChange,
  Grant,
  ...
  describe('preventCloseIfObjectivesOpen with Data', () => {
    ...
    it('correctly handles a mix of objective statuses', async () => {
      ...
      const expectedError = new RegExp(
        `Cannot close a goal ${goal.id} with open objectives\\. ` + `(${mixedStatusObjective.id}|${objectiveRTR.id}) is open\\.`
      )
      await expect(preventCloseIfObjectivesOpen(sequelize, instance)).rejects.toThrow(expectedError)
