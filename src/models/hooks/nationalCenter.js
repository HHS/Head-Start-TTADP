...
const updateExistingSessionReports = async (sequelize, instance, options, isDestroy = false) => {
  ...
  if ((Array.isArray(changed) && changed.includes('name')) || isDestroy) {
    ...
    await Promise.all(
      (sessionReports || []).map(async (sessionReport) => {
        ...
        const newObjectiveTrainersWithoutOld = objectiveTrainers.filter((trainer) => trainer !== oldNationalCenterName)
