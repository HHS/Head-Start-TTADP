...
describe('Activity Reports model', () => {
  ...
  it('propagateApprovedStatus', async () => {
    ...
    await ActivityReport.update({ calculatedStatus: REPORT_STATUSES.NEEDS_ACTION }, { where: { id: report.id }, individualHooks: true })
  ...
  it('submittedDate', async () => {
    ...
    // Submitted.
    await preReport.update({ calculatedStatus: REPORT_STATUSES.SUBMITTED, submissionStatus: REPORT_STATUSES.SUBMITTED }, { individualHooks: true })
    ...
    // Approved.
    await ActivityReport.update({ calculatedStatus: REPORT_STATUSES.APPROVED }, { where: { id: reportToSubmit.id }, individualHooks: true })
    reportToSubmit = await ActivityReport.findOne({ where: { id: reportToSubmit.id } })
    expect(reportToSubmit.submittedDate).not.toBeNull()
    expect(reportToSubmit.submittedDate).toBe(moment(submittedDate).format('MM/DD/YYYY'))
    ...
    // Reset to Draft.
    await ActivityReport.update({ calculatedStatus: REPORT_STATUSES.DRAFT }, { where: { id: reportToSubmit.id }, individualHooks: true })
    reportToSubmit = await ActivityReport.findOne({ where: { id: reportToSubmit.id } })
