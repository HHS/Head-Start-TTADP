...
describe('GrantRelationshipToActive', () => {
  ...
  describe('error cases', () => {
    ...
    it('throws an error on create', async () => {
      await expect(GrantRelationshipToActive.create({ grantId: grant.id })).rejects.toThrow('Insertion not allowed on materialized view')
    })

    it('throws an error on update', async () => {
      instance = await GrantRelationshipToActive.findOne({ where: { grantId: grant.id } })
      await expect(instance.update({ activeGrantId: 12345 })).rejects.toThrow('Update not allowed on materialized view')
    })
