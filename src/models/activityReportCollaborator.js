...
export default (sequelize, DataTypes) => {
  class ActivityReportCollaborator extends Model {
    static associate(models) {
      ...
      ActivityReportCollaborator.hasMany(models.CollaboratorRole, {
      ...
      fullName: {
        ...
        get() {
          if (!this.user) {
            ...
            auditLogger.error(
              `Access attempt to undefined user for userID: ${this.userId}, stack: ${stack}`
            );
            return '';
          }
          const roles = this.roles && this.roles.length ? this.roles : this.user.roles;
