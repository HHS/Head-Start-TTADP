			...
			applied_filtered_grants AS (
				...
				AND (
				  ...
				  OR (
					  EXISTS (
						...
						WHERE p."programType" = value::text
				...
				AND (
				  ...
				  OR (
					  EXISTS (
						SELECT 1
						FROM json_array_elements_text(COALESCE(state_code_filter, '[]')::json) AS value
						WHERE gr."stateCode" = value::text
					  ) != state_code_not_filter
					)
				)
				...
				AND (
				  ...
				  OR (
					  EXISTS (
						SELECT 1
						FROM json_array_elements_text(COALESCE(region_ids_filter, '[]')::json) AS value
						WHERE gr."regionId" = value::integer
					  )
					)
				)
				ORDER BY 1
			...
			applied_filtered_grants AS (
				...
				AND (
				  ...
				  OR (
					EXISTS (
					  SELECT 1
					  FROM json_array_elements_text(COALESCE(group_filter, '[]')::json) AS value
				...
				AND (
				  ...
				  OR (
					EXISTS (
					  SELECT 1
					  FROM json_array_elements_text(COALESCE(current_user_id_filter, '[]')::json) AS value
					  WHERE gc."userId" = value::integer
					)
				  )
				)
				WHERE 1 = 1
				...
				AND (
				  ...
				  OR (
					EXISTS (
					  ...
					  WHERE r1."fullName" = value::text
					)
					OR EXISTS (
					  SELECT 1
					  FROM json_array_elements_text(COALESCE(roles_filter, '[]')::json) AS value
					  WHERE r2."fullName" = value::text
					)
				  ) != roles_not_filter
				)
				GROUP BY 1
