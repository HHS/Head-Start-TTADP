...
export default class ActivityReport {
  ...
  canUpdate() {
    const canUpdateAsAuthorAndCollaborator = (this.isAuthor() || this.isCollaborator()) && this.canWriteInRegion() && this.reportHasEditableStatus()

    const canUpdateAsApprover =
      this.canReview() &&
      (this.activityReport.calculatedStatus === REPORT_STATUSES.SUBMITTED || this.activityReport.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION)

    return canUpdateAsAuthorAndCollaborator || canUpdateAsApprover
  }

  canReset() {
    return (this.isAuthor() || this.isCollaborator()) && this.activityReport.calculatedStatus === REPORT_STATUSES.SUBMITTED
  }

  canDelete() {
    return (this.isAdmin() || this.isAuthor()) && this.activityReport.calculatedStatus !== REPORT_STATUSES.APPROVED
  }

  canUnlock() {
    return this.isUnlockAdmin() && this.activityReport.calculatedStatus === REPORT_STATUSES.APPROVED
  }
  ...
  canApproveInRegion() {
    const regionId = this.activityReport.regionId || (this.activityReport.dataValues && this.activityReport.dataValues.regionId)

    const permissions = _.find(
      this.user.permissions,
      (permission) => permission.scopeId === SCOPES.APPROVE_REPORTS && permission.regionId === regionId
    )
  ...
  canWriteInRegion() {
    ...
    const permissions = _.find(
      this.user.permissions,
      (permission) => permission.scopeId === SCOPES.READ_WRITE_REPORTS && permission.regionId === regionId
    )
    return !_.isUndefined(permissions)
  }
  ...
  isAdmin() {
    const adminScope = this.user.permissions.find((permission) => permission.scopeId === SCOPES.ADMIN)
    return !_.isUndefined(adminScope)
  }

  isUnlockAdmin() {
    const adminScope = this.user.permissions.find((permission) => permission.scopeId === SCOPES.UNLOCK_APPROVED_REPORTS)
    return !_.isUndefined(adminScope)
  }
  ...
  isCollaborator() {
    if (!this.activityReport.activityReportCollaborators || this.activityReport.activityReportCollaborators.length === 0) {
      return false
    }

    return this.activityReport.activityReportCollaborators.some((collab) => collab.user.id === this.user.id)
  }
  ...
  reportHasEditableStatus() {
    ...
    if (this.activityReport.submissionStatus === REPORT_STATUSES.DRAFT || this.activityReport.calculatedStatus === REPORT_STATUSES.DRAFT) {
      return true
    }
