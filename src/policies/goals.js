...
export default class Goal {
  ...
  isAdmin() {
    return !isUndefined(this.user.permissions.find((permission) => permission.scopeId === SCOPES.ADMIN))
  }
    ...
    return (
      ...
      this.goal.objectives.some(
        (objective) =>
          ...
          objective.activityReports.some((report) => report.calculatedStatus === REPORT_STATUSES.APPROVED)
      )
    )
  }
  ...
  isOnActivityReports() {
    return (
      this.goal.objectives &&
      this.goal.objectives.length &&
      this.goal.objectives.some((objective) => objective.activityReports && objective.activityReports.length)
    )
  }
