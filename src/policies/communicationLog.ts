...
export default class CommunicationLog {
  ...
  canCreateLog() {
    const hasWritePermission = this.user.permissions.some(
      (permission) => permission.regionId === this.regionId && permission.scopeId === SCOPES.READ_WRITE_REPORTS
    )
  ...
  canReadLog() {
    return (
      this.user.permissions.some(
        (permission) => permission.regionId === this.regionId && [SCOPES.READ_WRITE_REPORTS, SCOPES.READ_REPORTS].includes(permission.scopeId)
      ) || this.isAdmin()
    )
  }
