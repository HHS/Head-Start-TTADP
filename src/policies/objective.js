...
export default class Objective {
  ...
  canWriteInRegion(region) {
    const permissions = find(
      ...
      (permission) => permission.scopeId === SCOPES.READ_WRITE_REPORTS && permission.regionId === region
    )
  ...
  canUpload() {
    if (
      this.objective.status !== OBJECTIVE_STATUS.COMPLETE &&
  ...
  canUpdate() {
    if (
      !this.objective.onApprovedAR &&
      (this.objective.otherEntityId ||
        (this.objective.goal && this.objective.goal.grant && this.canWriteInRegion(this.objective.goal.grant.regionId)))
    ) {
      return true
    }
    return false
