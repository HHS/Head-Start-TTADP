...
describe('Activity Report policies', () => {
  ...
  describe('canUpdate', () => {
    describe('if the user has write permissions in the region', () => {
      ...
      it('is true if the user is the author and report calculatedStatus is NEEDS_ACTION', () => {
        const report = activityReport(author.id, null, null, REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.NEEDS_ACTION)
        const policy = new ActivityReport(author, report)
        expect(policy.canUpdate()).toBeTruthy()
      })
      ...
      it('is true if the user is an approver', () => {
        const report = activityReport(author.id, null, [approver.id], REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.SUBMITTED)
        const policy = new ActivityReport(approver, report)
        expect(policy.canUpdate()).toBeTruthy()
      })

      it('is true if the user is an approver and report needs action', () => {
        const report = activityReport(author.id, null, [approver.id], REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.NEEDS_ACTION)
        const policy = new ActivityReport(approver, report)
        expect(policy.canUpdate()).toBeTruthy()
      })

      it('...unless the report is draft', () => {
        const report = activityReport(author.id, null, [approver.id], REPORT_STATUSES.DRAFT, REPORT_STATUSES.DRAFT)
        const policy = new ActivityReport(approver, report)
        expect(policy.canUpdate()).toBe(false)
      })

      it('...unless the report is approved', () => {
        const report = activityReport(author.id, null, [approver.id], REPORT_STATUSES.APPROVED, REPORT_STATUSES.APPROVED)
        const policy = new ActivityReport(approver, report)
        expect(policy.canUpdate()).toBe(false)
      })

      it('is true even if another approver has already approved the report', () => {
        const report = activityReport(author.id, null, [approver.id, canApproveRegion.id], REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.SUBMITTED)
    ...
    it('is false if the report has been submitted', () => {
      const report = activityReport(author.id, null, null, REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.SUBMITTED)
      const policy = new ActivityReport(author, report)
      expect(policy.canUpdate()).toBeFalsy()
    })

    it('is false if the report has been approved', () => {
      const report = activityReport(author.id, null, null, REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.APPROVED)
      const policy = new ActivityReport(author, report)
      expect(policy.canUpdate()).toBeFalsy()
    })
  })
  ...
  describe('canReset', () => {
    ...
    it('is true for collaborators', () => {
      const report = activityReport(author.id, activityReportCollaborator, null, REPORT_STATUSES.SUBMITTED, REPORT_STATUSES.SUBMITTED)
      const policy = new ActivityReport(activityReportCollaborator.user, report)
      expect(policy.canReset()).toBeTruthy()
    })
