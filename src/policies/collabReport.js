...
export default class CollabReport {
  ...
  canUpdate() {
    const canUpdateAsAuthorOrCollaborator = (this.isAuthor() || this.isCollaborator()) && this.canWriteInRegion() && this.reportHasEditableStatus()

    const canUpdateAsApprover =
      this.canReview() &&
      (this.collabReport.calculatedStatus === REPORT_STATUSES.SUBMITTED || this.collabReport.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION)

    return canUpdateAsAuthorOrCollaborator || canUpdateAsApprover
  }

  canReset() {
    return (this.isAuthor() || this.isCollaborator()) && this.collabReport.calculatedStatus === REPORT_STATUSES.SUBMITTED
  }

  canDelete() {
    return (this.isAdmin() || this.isAuthor()) && this.collabReport.calculatedStatus !== REPORT_STATUSES.APPROVED
  }
  ...
  canApproveInRegion() {
    const regionId = this.collabReport.regionId || (this.collabReport.dataValues && this.collabReport.dataValues.regionId)
  ...
  isCollaborator() {
    if (!this.collabReport.collabReportSpecialists || this.collabReport.collabReportSpecialists.length === 0) {
      return false
    }

    return this.collabReport.collabReportSpecialists.some((collab) => collab.specialist.id === this.user.id)
  }
  ...
  reportHasEditableStatus() {
    ...
    if (this.collabReport.submissionStatus === REPORT_STATUSES.DRAFT || this.collabReport.calculatedStatus === REPORT_STATUSES.DRAFT) {
      return true
    }
