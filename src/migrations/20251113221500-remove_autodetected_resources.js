    ...
    queryInterface.sequelize.transaction(async (transaction) => {
      ...
      const deletions = [
        {
          ...
          keepFields: [SOURCE_FIELD.REPORT.RESOURCE, SOURCE_FIELD.REPORT.ECLKC, SOURCE_FIELD.REPORT.NONECLKC],
        },
        {
          table: 'GoalResources',
        },
      ...
      await Promise.all(
        deletions.map(({ table, keepFields = [] }) => {
          const whereClause = keepFields.length ? keepFields.map((field) => `NOT ('${field}' = ANY("sourceFields"))`).join(' AND ') : ''
