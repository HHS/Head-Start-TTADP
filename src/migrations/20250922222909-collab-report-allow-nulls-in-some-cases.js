...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.changeColumn('CollabReports', 'name', {
        type: Sequelize.STRING,
        allowNull: true,
      }, { transaction });

      // Allow nulls for startDate field
      await queryInterface.changeColumn('CollabReports', 'startDate', {
        type: Sequelize.DATEONLY,
        allowNull: true,
      }, { transaction });

      // Allow nulls for endDate field
      await queryInterface.changeColumn('CollabReports', 'endDate', {
        type: Sequelize.DATEONLY,
        allowNull: true,
      }, { transaction });

      // Allow nulls for duration field
      await queryInterface.changeColumn('CollabReports', 'duration', {
      ...
      await queryInterface.changeColumn('CollabReports', 'conductMethod', {
        type: '"enum_CollabReports_conductMethod"[]',
        allowNull: true,
      }, { transaction });

      // Allow nulls for description field
      await queryInterface.changeColumn('CollabReports', 'description', {
        type: Sequelize.TEXT,
        allowNull: true,
      }, { transaction });

      // Make userId required (not null)
      await queryInterface.changeColumn('CollabReports', 'userId', {
