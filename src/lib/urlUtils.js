...
export const VALID_URL_REGEX = /(?<url>(?<scheme>http(?:s)?|ftp(?:s)?|sftp):\/\/(?:(?<user>[a-zA-Z0-9._]+)(?:[:](?<password>[a-zA-Z0-9%._\+~#=]+))?[@])?(?:(?:www\.)?(?<host>[a-zA-Z0-9%._\+~#=]{1,256}\.[a-z]{2,6}\b)|(?<ip>(?:[0-9]{1,3}\.){3}[0-9]{1,3}))(?:[:](?<port>[0-9]+))?(?:[\/](?<path>[-a-zA-Z0-9@:%_\+.~#&\/=()]*))?(?:[?](?<query>[-a-zA-Z0-9@:%_\+.~#&\/=()]*))?)/ig;
