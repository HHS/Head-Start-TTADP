...
const vacuumTables = async (offset = 0, limit = numOfModels, triggeredById = null) => {
  ...
  return maintenanceCommand(
    async (logMessages, logBenchmarks, triggered) => ({
      isSuccessful: (
        await Promise.all(models.map(async (model) => vacuumTable(model, triggered)))
      ).every((p) => p === true),
    }),
    MAINTENANCE_CATEGORY.DB,
...
const reindexTables = async (offset = 0, limit = numOfModels, triggeredById = null) => {
  ...
  return maintenanceCommand(
    async (logMessages, logBenchmarks, triggered) => ({
      isSuccessful: (
        await Promise.all(models.map(async (model) => reindexTable(model, triggered)))
      ).every((p) => p === true),
    }),
    MAINTENANCE_CATEGORY.DB,
    ...
    async (logMessages, logBenchmarks, triggeredById) => {
      try {
        ...
        const results = await Promise.all([
          vacuumTablesPromise,
          reindexTablesPromise,
          correctArFlagsPromise,
        ]);
...
registerCronEnrollmentFunction(async (instanceId, contextId, env) => {
  if (env !== 'production') {
    auditLogger.log(
      'info',
      `Skipping DB cron job enrollment in non-production environment (${env})`
    );
    return;
  }
  ...
  if (instanceId !== '0') {
    auditLogger.log(
      'info',
      `Skipping DB cron job enrollment on instance ${instanceId} in environment ${env}`
    );
    return;
  }
  ...
  if (contextId !== 1) {
    auditLogger.log(
      'info',
      `Skipping DB cron job enrollment on context ${contextId} in environment ${env} instance ${instanceId}`
    );
    return;
  }
  ...
  auditLogger.log(
    'info',
    `Registering DB maintenance cron jobs for context ${contextId} in environment ${env} instance ${instanceId}`
  );
