    ...
    async (_logMessages, _logBenchmarks, _triggeredById) => {
      ...
      try {
        ...
        await lockManager.executeWithLock(async () => {
          ...
          auditLogger.log(
            'info',
            `import: importProcess->maintenanceCommand: ${JSON.stringify({ processResults })}`
          );
          ...
          auditLogger.log(
            'info',
            `import: importProcess->maintenanceCommand: ${JSON.stringify({ more })}`
          );
          if (more) {
...
registerCronEnrollmentFunction(async (instanceId, contextId, env) => {
  if (env !== 'production') {
    auditLogger.log(
      'info',
      `Skipping import cron job enrollment in non-production environment (${env})`
    );
    return;
  }
  ...
  if (instanceId !== '0') {
    auditLogger.log(
      'info',
      `Skipping import cron job enrollment on instance ${instanceId} in environment ${env}`
    );
    return;
  }
  ...
  if (contextId !== 1) {
    auditLogger.log(
      'info',
      `Skipping import cron job enrollment on context ${contextId} in environment ${env} for instance ${instanceId}`
    );
    return;
  }
  ...
  auditLogger.log(
    'info',
    `Registering import maintenance cron jobs for context ${contextId} in environment ${env} for instance ${instanceId}`
  );
