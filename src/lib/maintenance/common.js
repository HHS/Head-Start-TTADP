...
const processMaintenanceQueue = () => {
  ...
  Object.entries(maintenanceQueueProcessors).map(([category, { processor, runInTransaction }]) =>
    maintenanceQueue.process(category, runInTransaction ? transactionQueueWrapper(processor, category) : processor)
  )
}
...
const hasCronJobBeenStarted = (category, type, name) => !!maintenanceCronJobs?.[category]?.[type]?.[name]?.started
...
const createCategory = (category, typeJobs, timezone) => {
  ...
  const jobs = Object.entries(typeJobs).reduce((acc, [type, names]) => {
    ...
    const namedJobs = Object.entries(names).reduce((typeAcc, [name, { jobCommand, schedule, started }]) => {
...
const maintenanceCommand = async (callback, category, type, data = {}, triggeredById = null) => {
  ...
  try {
    ...
    isSuccessful =
      logMessages.some((message) => message.toLowerCase().includes('successfully') || message.toLowerCase().includes('executed')) ||
      !!result?.isSuccessful
