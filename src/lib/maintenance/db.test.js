...
describe('maintenance', () => {
  ...
  describe('vacuumTables', () => {
    it('should call vacuumTable for each model in the database', async () => {
      ...
      expect(logs.every((log) => log.type === MAINTENANCE_TYPE.VACUUM_ANALYZE || log.type === MAINTENANCE_TYPE.VACUUM_TABLES)).toBe(true)
      expect(
        logs.every(
          (log) => (log.data?.messages?.length > 0 && log.data.messages[0].includes('VACUUM ANALYZE')) || log.type === MAINTENANCE_TYPE.VACUUM_TABLES
        )
      ).toBe(true)
      expect(
        logs.every(
          (log) => (log.data?.benchmarks?.length > 0 && typeof log.data.benchmarks[0] === 'number') || log.type === MAINTENANCE_TYPE.VACUUM_TABLES
        )
      ).toBe(true)
    })
  ...
  describe('reindexTables', () => {
    it('should call reindexTable for each model in the database', async () => {
      ...
      expect(logs.length).toBe(numOfModels + 1)
      expect(logs.every((log) => log.isSuccessful)).toBe(true)
      expect(logs.every((log) => log.type === MAINTENANCE_TYPE.REINDEX || log.type === MAINTENANCE_TYPE.REINDEX_TABLES)).toBe(true)
      expect(
        logs.every(
          (log) => (log.data?.messages?.length > 0 && log.data.messages[0].includes('REINDEX TABLE')) || log.type === MAINTENANCE_TYPE.REINDEX_TABLES
        )
      ).toBe(true)
      expect(
        logs.every(
          (log) => (log.data?.benchmarks?.length > 0 && typeof log.data.benchmarks[0] === 'number') || log.type === MAINTENANCE_TYPE.REINDEX_TABLES
        )
      ).toBe(true)
    })
  ...
  describe('dailyMaintenance', () => {
    it('should call vacuumTables and reindexTables', async () => {
      ...
      expect(
        logs.every(
          (log) =>
            log.type === MAINTENANCE_TYPE.REINDEX ||
            ...
            log.type === MAINTENANCE_TYPE.CORRECT_AR_FLAGS
        )
      ).toBe(true)
      expect(
        logs.every(
          (log) =>
            (log.data?.messages?.length > 0 && (log.data.messages[0].includes('REINDEX TABLE') || log.data.messages[0].includes('VACUUM ANALYZE'))) ||
            log.type === MAINTENANCE_TYPE.REINDEX_TABLES ||
            log.type === MAINTENANCE_TYPE.VACUUM_TABLES ||
            log.type === MAINTENANCE_TYPE.DAILY_DB_MAINTENANCE ||
            log.type === MAINTENANCE_TYPE.CORRECT_AR_FLAGS
      ...
      expect(
        logs.every(
          (log) =>
            (log.data?.benchmarks?.length > 0 && typeof log.data.benchmarks[0] === 'number') ||
            log.type === MAINTENANCE_TYPE.REINDEX_TABLES ||
            log.type === MAINTENANCE_TYPE.VACUUM_TABLES ||
            log.type === MAINTENANCE_TYPE.DAILY_DB_MAINTENANCE ||
            log.type === MAINTENANCE_TYPE.CORRECT_AR_FLAGS
