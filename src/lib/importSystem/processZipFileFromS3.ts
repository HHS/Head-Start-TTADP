...
import { getNextFileToProcess, recordAvailableDataFiles, setImportFileStatus, setImportDataFileStatusByPath } from './record'
...
const processZipFileFromS3 = async (importId: number) => {
  ...
  const { importFileId, processAttempts = 0, fileKey: key, importDefinitions: processDefinitions } = importFile
  ...
  const filteredFileDetails = fileDetails
    ...
    .filter(({ name, path }) => neededFiles.some((neededFile) => neededFile.name === name && neededFile.path === path)) as ZipFileInfo[]

  await Promise.all(
    fileDetails
      .filter(({ name, path }) => !neededFiles.some((neededFile) => neededFile.name === name && neededFile.path === path))
      .map(async (fileDetail) => setImportDataFileStatusByPath(importFileId, fileDetail, IMPORT_DATA_STATUSES.WILL_NOT_PROCESS))
  )
  ...
  try {
    ...
    results = await processFilesFromZip(importFileId, zipClient, filteredFileDetails, processDefinitions)
  } catch (err) {
    // If an error occurs, set the import file status to PROCESSING_FAILED
    await setImportFileStatus(importFileId, IMPORT_STATUSES.PROCESSING_FAILED)
