...
const handlePostProcessing = async (id) => {
  try {
    ...
    await Promise.all(
      importRecord.postProcessingActions.map(async (action) => {
        switch (action.function) {
          case 'createMonitoringGoals':
            // Create monitoring goals for the import.
            auditLogger.info(
              `Starting Post Processing: Creating monitoring goals for import: ${id} - ${importRecord.name} task: ${action.name}`
            );
            await createMonitoringGoals();
            auditLogger.info(
              `Finished Post Processing: Creating monitoring goals for import: ${id} - ${importRecord.name} task: ${action.name}`
            );
            break;
          ...
          default:
            ...
            auditLogger.error(
              `Unknown import post processing action: ${action.function} for import: ${id} - ${importRecord.name} skipping`
            );
            break;
        }
      })
