import SftpClient, { type SFTPSettings, type FileListing } from '../stream/sftp'
import Hasher from '../stream/hasher'
import S3Client from '../stream/s3'
import db from '../../models'
import { getPriorFile, recordAvailableFiles, logFileToBeCollected, setImportFileHash, setImportFileStatus } from './record'
...
const collectNextFile = async (
  ...
  try {
    ...
    await Promise.all([
      ...
      setImportFileHash(importFileData.importFileId, await hashStream.getHash(), IMPORT_STATUSES.COLLECTED),
    ])
    ...
    used.push(new Date().getTime() - currentStart.getTime())
    return collectNextFile(importId, [availableFile, ...availableFiles], { start, limit, used }, importedFiles)
  }
 ...
 */
const collectServerSettings = (importId: number, ftpSettings: { host: string; port: string; username: string; password }) => {
  const {
  ...
  const { [hostEnv]: host, [portEnv]: port, [userEnv]: username, [passwordEnv]: password } = process.env
...
const collectFilesFromSource = async (
  ...
  const priorFile = await getPriorFile(importId, [IMPORT_STATUSES.COLLECTED, IMPORT_STATUSES.PROCESSED]) // Get the prior file for the import
...
const downloadFilesFromSource = async (importId: number, timeBox = 5 * 60 * 1000) => {
  ...
  return collectFilesFromSource(importFileData.importId, timeBox, importFileData.ftpSettings, importFileData.path, importFileData.fileMask)
}

export { collectNextFile, collectFilesFromSource, collectServerSettings, downloadFilesFromSource, sortFilesByFullPath }
