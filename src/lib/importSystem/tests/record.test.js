...
import { ImportFile, ImportDataFile, File, Import, ZALImportFile } from '../../../models' // Adjust the import path as necessary
import {
  getPriorFile,
...
describe('record', () => {
  ...
  describe('recordAvailableFiles', () => {
    ...
    it('should update matched files in the database', async () => {
      ImportFile.findAll.mockResolvedValueOnce([{ id: 2, importId, fileInfo: availableFiles[0].fileInfo }])
      await recordAvailableFiles(importId, availableFiles)
  ...
  describe('recordAvailableDataFiles', () => {
    ...
    const currentImportDataFiles = [
      ...
      {
        fileInfo: { path: '/data/current/', name: 'file1.csv' },
        importFileId: 1,
        status: 'PROCESSED',
      },
      {
        fileInfo: { path: '/data/current/', name: 'file2.csv' },
        importFileId: 1,
        status: 'PROCESSED',
      },
    ]
    ...
    it('should create new files in the database when there are new files', async () => {
      ...
      const newFiles = availableFiles.filter(
        (af) => !currentImportDataFiles.some((cif) => cif.fileInfo.path === af.path && cif.fileInfo.name === af.name)
      )

      // Mock the database response for create
      const mockNewFilesResponse = newFiles.map((file) => ({
        ...file,
      ...
      expect(ImportDataFile.create).toHaveBeenCalledWith(expectedBulkCreateArgument[0])
    })
  ...
  describe('updateAvailableDataFileMetadata', () => {
    ...
    it('should return the result of the ImportDataFile.update call', async () => {
      ...
      // Act
      const result = await updateAvailableDataFileMetadata(importFileId, fileInfo, status, metadata)
  ...
  describe('setImportFileHash', () => {
    ...
    it('should call ImportFile.update with the correct parameters when only hash is provided', async () => {
      ...
      expect(ImportFile.update).toHaveBeenCalledWith({ hash: newHash }, { where: { id: importFileId }, individualHooks: true, lock: true })
    })
    ...
    it('should call ImportFile.update with the correct parameters when hash and status are provided', async () => {
      ...
      expect(ImportFile.update).toHaveBeenCalledWith({ hash: newHash, status }, { where: { id: importFileId }, individualHooks: true, lock: true })
    })
    ...
    it('should handle null hash values', async () => {
      ...
      expect(ImportFile.update).toHaveBeenCalledWith({ hash: null }, { where: { id: importFileId }, individualHooks: true, lock: true })
    })
  ...
  describe('setImportDataFileStatus', () => {
    ...
    const setImportDataFileStatus = async (importDataFileId, status) =>
    ...
    it('should call update with the correct parameters', async () => {
      ...
      expect(ImportDataFile.update).toHaveBeenCalledWith({ status: newStatus }, { where: { id: importFileId }, individualHooks: true, lock: true })
    })
    ...
    it('should resolve when the update is complete', async () => {
      ...
      await expect(setImportDataFileStatus(importFileId, newStatus)).resolves.toEqual([1, [mockUpdatedImportDataFile]])
    })
  })
  ...
  describe('setImportDataFileStatusByPath', () => {
    ...
    it('should update the status of the import data file if found', async () => {
      ...
      expect(ImportDataFile.update).toHaveBeenCalledWith({ status: mockStatus }, { where: { id: mockImportDataFile.id }, individualHooks: true })
    })
    ...
    it('should handle errors thrown by the database operation during file search', async () => {
      ...
      // Act & Assert
      await expect(setImportDataFileStatusByPath(mockImportFileId, mockFileInfo, mockStatus)).rejects.toThrow(errorMessage)
    })
