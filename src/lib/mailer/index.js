...
const { SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, SMTP_SECURE, SMTP_IGNORE_TLS, NODE_ENV, SEND_NON_PRODUCTION_NOTIFICATIONS } = process.env
 ...
 */
export const programSpecialistRecipientReportApprovedNotification = (report, programSpecialists, recipients) => {
...
export const trEventComplete = async (event) => {
  if (process.env.CI) return
  try {
    const userIds = uniq([...event.collaboratorIds, ...event.pocIds]).filter((id) => id && id !== event.ownerId)
...
export const changesRequestedNotification = (report, approver, authorWithSetting, collabsWithSettings) => {
  // Send group notification to author and collaborators
  try {
    const data = {
      report,
  ...
  try {
    ...
    specialists = await Promise.all(
      specialists.map(async (ps) => {
        const setting = await userSettingOverridesById(ps.id, USER_SETTINGS.EMAIL.KEYS.RECIPIENT_APPROVAL)
...
export const processNotificationQueue = () => {
  ...
  notificationQueue.process(EMAIL_ACTIONS.NEEDS_ACTION, transactionQueueWrapper(notifyChangesRequested, EMAIL_ACTIONS.NEEDS_ACTION))

  notificationQueue.process(EMAIL_ACTIONS.SUBMITTED, transactionQueueWrapper(notifyApproverAssigned, EMAIL_ACTIONS.SUBMITTED))

  notificationQueue.process(EMAIL_ACTIONS.APPROVED, transactionQueueWrapper(notifyReportApproved, EMAIL_ACTIONS.APPROVED))

  notificationQueue.process(EMAIL_ACTIONS.COLLABORATOR_ADDED, transactionQueueWrapper(notifyCollaboratorAssigned, EMAIL_ACTIONS.COLLABORATOR_ADDED))
  ...
  notificationQueue.process(EMAIL_ACTIONS.NEEDS_ACTION_DIGEST, transactionQueueWrapper(notifyDigest, EMAIL_ACTIONS.NEEDS_ACTION_DIGEST))
  notificationQueue.process(EMAIL_ACTIONS.SUBMITTED_DIGEST, transactionQueueWrapper(notifyDigest, EMAIL_ACTIONS.SUBMITTED_DIGEST))
  notificationQueue.process(EMAIL_ACTIONS.APPROVED_DIGEST, transactionQueueWrapper(notifyDigest, EMAIL_ACTIONS.APPROVED_DIGEST))
  notificationQueue.process(EMAIL_ACTIONS.COLLABORATOR_DIGEST, transactionQueueWrapper(notifyDigest, EMAIL_ACTIONS.COLLABORATOR_DIGEST))
  notificationQueue.process(
  ...
  notificationQueue.process(
    ...
    transactionQueueWrapper(sendTrainingReportNotification, EMAIL_ACTIONS.TRAINING_REPORT_COLLABORATOR_ADDED)
  )

  notificationQueue.process(
    EMAIL_ACTIONS.TRAINING_REPORT_SESSION_CREATED,
    transactionQueueWrapper(sendTrainingReportNotification, EMAIL_ACTIONS.TRAINING_REPORT_SESSION_CREATED)
  )

  notificationQueue.process(
    EMAIL_ACTIONS.TRAINING_REPORT_EVENT_COMPLETED,
    transactionQueueWrapper(sendTrainingReportNotification, EMAIL_ACTIONS.TRAINING_REPORT_EVENT_COMPLETED)
  )

  notificationQueue.process(
    EMAIL_ACTIONS.TRAINING_REPORT_EVENT_IMPORTED,
    transactionQueueWrapper(sendTrainingReportNotification, EMAIL_ACTIONS.TRAINING_REPORT_EVENT_IMPORTED)
  )
 ...
 * @returns Promise<any>
 */
export const sendEmailVerificationRequestWithToken = (user, token, transport = defaultTransport) => {
  const toEmails = filterAndDeduplicateEmails([user.email])
