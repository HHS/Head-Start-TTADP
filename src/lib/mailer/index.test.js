...
import { EMAIL_ACTIONS, EMAIL_DIGEST_FREQ, DIGEST_SUBJECT_FREQ } from '../../constants'
import { auditLogger, logger } from '../../logger'
import { userById } from '../../services/users'
import db, { ActivityReport, ActivityReportCollaborator, User, ActivityReportApprover } from '../../models'
...
describe('mailer tests', () => {
  ...
  describe('Changes requested by manager', () => {
    it('Tests that an email is sent', async () => {
      ...
      expect(email.envelope.to).toStrictEqual([mockAuthor.email, mockCollaborator1.user.email, mockCollaborator2.user.email])
  ...
  describe('Manager Approval Requested', () => {
    it('Tests that an email is sent', async () => {
      ...
      expect(message.text).toContain(`Activity Report ${mockReport.displayId} was submitted for your review.`)
      expect(message.text).toContain(reportPath)
    })
    it('Tests that emails are not sent without SEND_NOTIFICATIONS', async () => {
      process.env.SEND_NOTIFICATIONS = 'false'
  ...
  describe('Add Collaborators', () => {
    it('Tests that an email is sent', async () => {
      ...
      expect(message.text).toContain(`You've been added as a collaborator on Activity Report ${mockReport.displayId}.`)
      expect(message.text).toContain(reportPath)
    })
    it('Tests that emails are not sent without SEND_NOTIFICATIONS', async () => {
      process.env.SEND_NOTIFICATIONS = 'false'
  ...
  describe('sendTrainingReportNotification', () => {
    it('Tests that an email is sent', async () => {
      ...
      expect(message.text).toContain('Select the link below to review and submit the session details in the TTA Hub:')
  ...
  describe('Collaborators digest', () => {
    ...
    it('tests that an email is sent if there are no new collaborator notifications', async () => {
      ...
      expect(message.text).toContain(`Hello ${mockNewCollaborator.name}`)
      expect(message.text).toContain("You haven't been added as a collaborator on any activity reports today.")
  ...
  describe('Program Specialist: Report approved digest', () => {
    it('tests that an email is sent for a daily setting', async () => {
      ...
      expect(message.text).toContain('Below are your report notifications for today.')
      expect(message.text).toContain('Activity Reports associated with your recipients were approved:')
      expect(message.text).toContain(`* ${mockReport.displayId}`)
      expect(message.text).toContain(reportPath)
    })
    it('tests that an email is sent for a weekly setting', async () => {
  ...
  describe('training report notifications', () => {
    ...
    it('trSessionCreated success', async () => {
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(EMAIL_ACTIONS.TRAINING_REPORT_SESSION_CREATED, expect.any(Object))
    })
    ...
    it('trOwnerAdded correctly gets added to the notificationQueue', async () => {
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(EMAIL_ACTIONS.TRAINING_REPORT_EVENT_IMPORTED, expect.any(Object))
    })
    ...
    it('trEventComplete correctly gets added to the notificationQueue', async () => {
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(EMAIL_ACTIONS.TRAINING_REPORT_EVENT_COMPLETED, expect.any(Object))
    })
    ...
    it('trCollaboratorAdded success', async () => {
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(EMAIL_ACTIONS.TRAINING_REPORT_COLLABORATOR_ADDED, expect.any(Object))
    })
    ...
    it('logs info if emailTo is an empty array', async () => {
      ...
      expect(logger.info).toHaveBeenCalledTimes(1)
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send tr collaborator added notification for 1234 preferences are not set or marked as "no-send"'
      )
    })
