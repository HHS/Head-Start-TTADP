...
describe('mailer tests', () => {
  ...
  describe('onCompletedNotification', () => {
    ...
    it('logs if result is null, single report', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        `Did not send ${EMAIL_ACTIONS.APPROVED} notification for ${mockReport.displayId} preferences are not set or marked as "no-send"`
      );
    });
    ...
    it('logs if result is null, single report, no display id', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        `Did not send ${EMAIL_ACTIONS.APPROVED} notification for [object Object] preferences are not set or marked as "no-send"`
      );
    });
    it('logs if result is good, single report', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Successfully sent reportApproved notification for mockReport-1'
      );
    });
    ...
    it('logs if result is good, single report, no display id', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Successfully sent reportApproved notification for [object Object]'
      );
    });
    ...
    it('log if result is null, many reports', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send reportApproved notification for mockReport-1 preferences are not set or marked as "no-send"'
      );
    });
  });
  ...
  describe('Program Specialists: Recipient Report Approved', () => {
    it('Tests that an email is sent', async () => {
      ...
      expect(message.text).toContain(
        'An Activity Report associated with one of your recipients has been approved.'
      );
      expect(message.text).toContain(reportPath);
    });
  ...
  describe('Collaborator: Report submitted for approval', () => {
    ...
    it('uses Revised wording for resubmissions', async () => {
      ...
      const email = await notifyCollaboratorReportSubmittedForReview(
        {
          data: {
            report: mockReport,
            collaborator: mockNewCollaborator,
            isResubmission: true,
          },
        },
        jsonTransport
      ...
      expect(message.text).toContain(
        `Revised Activity Report ${mockReport.displayId}, on which you are a collaborator, has been submitted for approval.`
      );
    });
    ...
    it('enqueues one job per collaborator', () => {
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          report: mockReport,
          collaborator: collaborators[0].user,
      ...
      expect(notificationQueueMock.add).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          report: mockReport,
          collaborator: collaborators[1].user,
          isResubmission: false,
        })
      );
    });

    it('enqueues collaborator jobs with the resubmission flag', () => {
      jest.spyOn(notificationQueueMock, 'add').mockClear();
      const collaborators = [{ user: { email: 'collab1@test.gov' }, userId: 10 }];
      collaboratorReportSubmittedForReviewNotification(mockReport, collaborators, true);
      expect(notificationQueueMock.add).toHaveBeenCalledWith(
        EMAIL_ACTIONS.COLLABORATOR_REPORT_SUBMITTED_FOR_REVIEW,
        expect.objectContaining({
          report: mockReport,
  ...
  describe('sendTrainingReportNotification', () => {
    it('Tests that an email is sent', async () => {
      ...
      const message = JSON.parse(email.message);
      expect(message.subject).toBe(
        `Action needed: Submit session details for Training Report ${data.displayId}`
      );
      expect(message.text).toContain(
    ...
    it('logs the info the job.data.report.id when we have no emails and job.data.report.displayId is null', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send undefined notification for 123 preferences are not set or marked as "no-send"'
      );
    });
  });
  ...
  describe('Collaborators digest', () => {
    ...
    it('tests that an email is sent for a daily setting', async () => {
      ...
      expect(message.text).toContain('You were added as a collaborator on these activity reports:');
      expect(message.text).toContain(`* ${mockReport.displayId}`);
      expect(message.text).toContain(reportPath);
    });
  ...
  describe('Changes requested digest', () => {
    it('tests that an email is sent for a daily setting', async () => {
      ...
      expect(message.text).toContain(`Hello ${mockNewCollaborator.name}`);
      expect(message.text).toContain('Below are your report notifications for today.');
      expect(message.text).toContain('Changes were requested to these activity reports:');
      expect(message.text).toContain(`* ${mockReport.displayId}`);
      expect(message.text).toContain(reportPath);
    });
    it('tests that an email is sent for a weekly setting', async () => {
    ...
    it('tests that an email is sent if there are no changes requested notifications', async () => {
      ...
      expect(message.text).toContain(`Hello ${mockNewCollaborator.name}`);
      expect(message.text).toContain('No changes were requested to any reports this week.');
      expect(message.text).not.toContain(reportPath);
    });
  });
  ...
  describe('Approved digest', () => {
    it('tests that an email is sent for a daily setting', async () => {
      ...
      expect(message.text).toContain(`Hello ${mockNewCollaborator.name}`);
      expect(message.text).toContain('Below are your report notifications for today.');
      expect(message.text).toContain('These activity reports were approved:');
      expect(message.text).toContain(`* ${mockReport.displayId}`);
      expect(message.text).toContain(reportPath);
    });
    it('tests that an email is sent for a weekly setting', async () => {
  ...
  describe('Program Specialist: Report approved digest', () => {
    it('tests that an email is sent for a daily setting', async () => {
      ...
      const message = JSON.parse(email.message);
      expect(message.subject).toBe(
        "TTA Hub daily digest: your recipients' approved Activity Reports"
      );
    ...
    it('tests that an email is sent for a weekly setting', async () => {
      ...
      const message = JSON.parse(email.message);
      expect(message.subject).toBe(
        "TTA Hub weekly digest: your recipients' approved Activity Reports"
      );
      expect(message.text).toContain(`Hello ${mockProgramSpecialist.name}`);
      expect(message.text).toContain('Below are your report notifications for this week.');
      expect(message.text).toContain(
    ...
    it('tests that an email is sent for a monthly setting', async () => {
      ...
      const message = JSON.parse(email.message);
      expect(message.subject).toBe(
        "TTA Hub monthly digest: your recipients' approved Activity Reports"
      );
      expect(message.text).toContain(`Hello ${mockProgramSpecialist.name}`);
      expect(message.text).toContain('Below are your report notifications for this month.');
      expect(message.text).toContain(
  ...
  describe('enqueue', () => {
    beforeEach(async () => {
      ...
      jest
        .spyOn(notificationDigestQueueMock, 'add')
    ...
    it('"collaborator added" digest which logs on error', async () => {
      usersWithSetting.mockReturnValueOnce(
        Promise.reject(new Error('Unfortunately, it does not work'))
      );
  ...
  describe('training report notifications', () => {
    ...
    it('logs a info if emailTo is an empty array', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send tr session created notification for tr-1234 preferences are not set or marked as "no-send"'
      );
    });
    ...
    it('logs the appropriate message when there are no emails to send to', async () => {
      ...
      expect(logger.info).toHaveBeenCalledTimes(1);
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send tr event complete notification for tr-1234 preferences are not set or marked as "no-send"'
      );
    });
    ...
    it('logs info if emailTo is an empty array', async () => {
      ...
      expect(logger.info).toHaveBeenCalledTimes(1);
      expect(logger.info).toHaveBeenCalledWith(
        'Did not send tr collaborator added notification for tr-1234 preferences are not set or marked as "no-send"'
      );
    });
