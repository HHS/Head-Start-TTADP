...
describe('Programmatic Transaction', () => {
  ...
  it('should throw error for unknown dml_type', async () => {
    ...
    const spy = jest.spyOn(transactionModule, 'fetchAndAggregateChanges').mockResolvedValue([fakeChange])

    await expect(transactionModule.revertChange([fakeChange])).rejects.toThrow('Unknown dml_type(INVALID_TYPE) for table: Topic')
  ...
  describe('hasModifiedData', () => {
    ...
    it('throws error if snapshot entry is not found for a ZAL table', async () => {
      await expect(transactionModule.hasModifiedData([], 'cb26d433-173d-4cea-8ab0-a7af8ed37c81')).rejects.toThrow(
        'Snapshot entry not found for table: ZALActivityRecipients'
      )
    })
    it('returns false when a ZAL table is not present', async () => {
      ...
      await expect(
        transactionModule.hasModifiedData([{ table_name: 'ZALActivityRecipients', max_id: '100' }], 'cb26d433-173d-4cea-8ab0-a7af8ed37c81')
