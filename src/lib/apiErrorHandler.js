...
export async function logRequestError(req, operation, error, logContext) {
  ...
  if (operation !== 'SequelizeError' && process.env.SUPPRESS_ERROR_LOGGING && process.env.SUPPRESS_ERROR_LOGGING.toLowerCase() === 'true') {
    return 0
  }
  if (!error) {
    return 0
...
export const handleError = async (req, res, error, logContext) => {
  ...
  const errorMessage = typeof error === 'object' && error !== null ? error.stack || JSON.stringify(error) : error
