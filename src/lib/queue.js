...
const { host, port, redisOpts } = generateRedisConfig(true)
...
export async function increaseListeners(queue, num = 1) {
  ...
  if (redisClient) {
    ...
    const currentCounts = queue.eventNames().reduce(
      (counts, eventName) => ({
        ...counts,
...
export function removeQueueEventHandlers(queue, errorListener, shutdownListener, exceptionListener, rejectionListener) {
  ...
  const errorListener = (err) => {
    ...
    queue
      ...
      .then(() => {
        ...
        removeQueueEventHandlers(queue, errorListener, shutdownListener, exceptionListener, rejectionListener)
      })
      .catch((closeErr) => {
