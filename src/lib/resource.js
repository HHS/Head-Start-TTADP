...
const getMetadataValuesFrommJson = async (url) => {
  let result;
  try {
    // Attempt to get the resource metadata (if valid HeadStart resource).
...
const getResourceMetaDataJob = async (job) => {
  ...
  try {
    // Determine if this is an HeadStart.gov resource.
    const isHeadStartResource = resourceUrl.includes('headstart.gov');
    ...
    // If it is an HeadStart resource, get the metadata values.
    if (isHeadStartResource) {
