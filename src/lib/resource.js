...
const getMetadataValuesFrommJson = async (url) => {
  ...
  try {
    ...
    result = {
      ...
      metadata:
        typeof res.data === 'object' && Object.keys(res.data)?.length !== 0 && res?.headers['content-type'] === 'application/json' ? res.data : null,
...
const getMetadataValuesFromHtml = async (url) => {
  ...
  try {
    ...
    const metadata = metadataPatterns
      .flatMap((pattern) => (res && res.data && typeof res.data === 'string' ? [...(res?.data?.matchAll(pattern) || null)] : []))
      .reduce((acc, meta) => {
    ...
    result = {
      metadata: typeof metadata === 'object' && Object.keys(metadata).length !== 0 ? metadata : null,
      statusCode: overrideStatusCodeOnAuthRequired(
        res?.status,
        commonAuthNames,
...
const getMetadataValues = async (url) => {
  ...
  try {
    ...
    if (fromJson.statusCode === httpCodes.OK) {
      ...
      const { metadata: metadataFromHtml, statusCode: statuscodeFromHtml, mimeType: mimeTypeFromHtml } = fromHtml.value
      // filter out unsupported characters.
    ...
    auditLogger.error(`Resource Queue: Unable to retrieving metadata for Resource (URL: ${url}). Please make sure this is a valid address:`, error) // Log an error message if there is an exception while retrieving metadata.
  }
  await Resource.update(
    {
...
const getPageScrapeValues = async (url) => {
  ...
  try {
    ...
    auditLogger.error(`Resource Queue: Unable to page scrape for Resource (URL: ${url}). Please make sure this is a valid address:`, error)
