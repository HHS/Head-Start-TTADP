...
describe('apiErrorHandler plus worker', () => {
  describe('apiErrorHandler', () => {
    ...
    it('handles specific Sequelize connection acquire timeout error', async () => {
      const mockConnAcquireTimeoutError = new Sequelize.ConnectionAcquireTimeoutError('Connection acquire timeout error')

      await expect(handleErrors(mockRequest, mockResponse, mockConnAcquireTimeoutError, mockLogContext)).rejects.toThrow(
        'Unhandled ConnectionAcquireTimeoutError: Restarting server due to database connection acquisition timeout.'
      )

      await expect(handleWorkerErrors(mockJob, mockConnAcquireTimeoutError, mockLogContext)).rejects.toThrow(
        'Unhandled ConnectionAcquireTimeoutError: Restarting server due to database connection acquisition timeout.'
      )
    ...
    it('should include params in the logged request body if present', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Error: Params test error'))
    })
    ...
    it('should use the error value directly if it does not have a stack', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining(`UNEXPECTED ERROR - ${mockError}`))
    })
    ...
    it('should handle null or undefined error gracefully', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(`${mockLogContext.namespace} - UNEXPECTED ERROR - ${mockError}`)
    })
    ...
    it('should handle error when it is an object', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining(`UNEXPECTED ERROR - ${JSON.stringify(mockError)}`))
    })
  ...
  describe('handleError Sequelize connection errors', () => {
    ...
    it('throws an unhandled exception for ConnectionAcquireTimeoutError', async () => {
      const mockConnAcquireTimeoutError = new Sequelize.Sequelize.ConnectionAcquireTimeoutError('Connection acquire timeout error')
  ...
  describe('handleWorkerError', () => {
    ...
    it('should log and throw Sequelize.ConnectionAcquireTimeoutError', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(`${mockErrorLogContext.namespace} - Critical error: Restarting server.`)
    })
