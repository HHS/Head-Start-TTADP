...
export default class LockManager {
  ...
  private unregisterEventListeners(): void {
    ...
    process.removeListener('unhandledRejection', this.handleShutdown as unknown as (reason, promise) => void)
  }
  ...
  public async executeWithLock(callback: () => Promise<void>, holdLock = false): Promise<void> {
    try {
      ...
      if (!hasLock) {
        ...
        if (!hasLock) {
          auditLogger.log('info', `(${process.pid}) Lock for key "${this.lockKey}" is already acquired by another instance. Skipping...`)
          await this.close()
          return
        }
      }
