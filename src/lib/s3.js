import { S3Client, GetBucketVersioningCommand, PutBucketVersioningCommand, GetObjectCommand, DeleteObjectCommand } from '@aws-sdk/client-s3'
...
const getSignedDownloadUrl = (key, bucket = s3Bucket, client = s3Client, expires = 360) => {
  ...
  const host = process.env.S3_ENDPOINT ? new URL(process.env.S3_ENDPOINT).host : `${s3Bucket}.s3.${s3Config.region}.amazonaws.com`
...
export { s3Client, downloadFile, getSignedDownloadUrl, uploadFile, generateS3Config, verifyVersioning, deleteFileFromS3, deleteFileFromS3Job }
