 ...
 */
const isObject = (obj): boolean => !!obj && typeof obj === 'object' && !Array.isArray(obj) && !(obj instanceof Date)
...
const remapPrune = (data, prunePath: string, options: { deleteEmptyParents?: boolean } = {}) => {
  ...
  if (deleteEmptyParents) {
    ...
    let parentPath = prunePath.substring(0, prunePath.includes('.') ? prunePath.lastIndexOf('.') : 0)
    while (parentPath && parentPath !== '') {
      ...
      parentPath = parentPath.includes('.') ? parentPath.substring(0, parentPath.lastIndexOf('.')) : ''
    }
  }
  return prune
}
...
const remap = (
  ...
  const { reverse = false, deleteMappedValues = true, deleteEmptyParents = true, keepUnmappedValues = true, targetFunctions = {} } = options
 ...
 */
const collectChangedValues = (incomingValues: Record<string, any>, currentValues: Record<string, any>): Record<string, any> => {
  ...
  Object.entries(incomingValues).forEach(([key, value]) => {
    ...
    if (!isDeepEqual(value, currentValues[key], true) || (key === 'id' && value === currentValues[key])) {
