...
function computeSummaryStatus(report) {
  ...
  if (deliveryMethod === 'hybrid') {
    participantsValid = [numberOfParticipantsInPerson, numberOfParticipantsVirtually].every((value) => isPositiveNumber(value))
  } else {
  ...
  const isComplete = stringsComplete && arraysComplete && numbersComplete && participantsValid && datesValid
...
function computeGoalsStatus(report) {
  ...
  const objectivesWithoutGoals = Array.isArray(report.objectivesWithoutGoals) ? report.objectivesWithoutGoals : []

  const hasGoals = goals.length > 0
  const hasOtherObjectives = objectivesWithoutGoals.length > 0

  const goalsAreComplete = hasGoals ? goals.every((goal) => objectivesComplete(goal.objectives || [])) : false

  const otherObjectivesComplete = hasOtherObjectives ? objectivesComplete(objectivesWithoutGoals) : false

  const hasData = hasGoals || hasOtherObjectives

  const allComplete = hasData && (hasGoals ? goalsAreComplete : true) && (hasOtherObjectives ? otherObjectivesComplete : true)
...
function computeNextStepsStatus(report) {
  const specialistNextSteps = Array.isArray(report.specialistNextSteps) ? report.specialistNextSteps : []
  const recipientNextSteps = Array.isArray(report.recipientNextSteps) ? report.recipientNextSteps : []

  const flattenSteps = (steps) =>
    steps
      .map((step) => (step?.note !== undefined || step?.completeDate !== undefined ? step : step?.dataValues || {}))
