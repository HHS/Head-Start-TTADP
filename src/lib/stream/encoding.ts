...
class EncodingConverter extends Transform {
  ...
  constructor(
    ...
    super()
    if (targetEncoding && EncodingConverter.supportedEncodings.has(targetEncoding.toLowerCase() as BufferEncoding)) {
      this.targetEncoding = targetEncoding.toLowerCase()
    } else {
      throw new Error(`Unsupported encoding detected: ${targetEncoding}`)
    }
    if (sourceEncoding && EncodingConverter.supportedEncodings.has(sourceEncoding.toLowerCase() as BufferEncoding)) {
  ...
  _transform(chunk: Buffer, _encoding: string, callback: (error?: Error | null, data?: Buffer) => void): void {
    if (this.detectEncoding) {
      ...
      if (this.buffer.length >= 1024) {
        ...
        this.sourceEncoding =
          detectedEncoding && EncodingConverter.supportedEncodings.has(detectedEncoding.toLowerCase() as BufferEncoding)
