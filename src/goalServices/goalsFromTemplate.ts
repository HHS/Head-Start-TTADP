...
export default async function goalsFromTemplate(
  ...
  const template = (await GoalTemplate.findOne({
    attributes: ['templateName', 'source'],
    where: {
      id: goalTemplateId,
    },
  })) as {
  ...
  const unsuspends = suspendedGoals.map((goal) =>
    changeGoalStatus({
      goalId: goal.id,
      userId,
  ...
  const newGoals = missingGrants.map((grant) =>
    Goal.create(
      {
        goalTemplateId,
        grantId: grant,
  ...
  return uniq([...goals.map((goal) => goal.id), ...resolved.map((goal) => goal.id)])
}
