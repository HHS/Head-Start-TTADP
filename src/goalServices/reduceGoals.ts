...
export function reduceObjectives(newObjectives: IObjectiveModelInstance[], currentObjectives: IReducedObjective[]) {
  // objectives = accumulator
  // we pass in the existing objectives as the accumulator
  const objectivesToSort = newObjectives.reduce((objectives: IReducedObjective[], objective) => {
    const exists = objectives.find((o) => o.title.trim() === objective.title.trim() && o.status === objective.status) as IReducedObjective | undefined

    const { id, otherEntityId, title, status, topics, resources, files, courses } = objective

    if (exists) {
      exists.ids = [...exists.ids, id]
      // Make sure we pass back a list of recipient ids for subsequent saves.
      exists.recipientIds = otherEntityId ? [...exists.recipientIds, otherEntityId] : [...exists.recipientIds]
      exists.activityReports = [...(exists.activityReports || []), ...(objective.activityReports || [])]
      return objectives
    }
    ...
    const newObjective = {
      ...
      // Make sure we pass back a list of recipient ids for subsequent saves.
      recipientIds: otherEntityId ? [otherEntityId] : [],
      isNew: false,
...
export function reduceObjectivesForActivityReport(newObjectives: IObjectiveModelInstance[], currentObjectives = []) {
  const objectivesToSort = newObjectives.reduce((objectives, objective) => {
    // check the activity report objective status
    const objectiveStatus =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].status
        ? objective.activityReportObjectives[0].status
        : objective.status

    const objectiveSupportType =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].supportType
        ? objective.activityReportObjectives[0].supportType
        : null

    const objectiveCreatedHere =
      objective.activityReportObjectives && objective.activityReportObjectives.some((aro) => aro.objectiveCreatedHere) ? true : null

    // objectives represent the accumulator in the find below
    // objective is the objective as it is returned from the API
    const exists = objectives.find((o) => o.title.trim() === objective.title.trim() && o.status === objectiveStatus)
    ...
    if (exists) {
      ...
      exists.resources = reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveResources', 'resource', exists, 'value')

      exists.topics = reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveTopics', 'topic', exists)

      exists.courses = reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveCourses', 'course', exists)

      // Citations should return null not exists (every subsequent adding of objective).
      exists.citations = uniq(
        objective.activityReportObjectives && objective.activityReportObjectives.length > 0
          ? [
              ...(exists.citations || []),
              ...objective.activityReportObjectives.flatMap((aro) =>
                (aro.activityReportObjectiveCitations || []).map((c) => ({
                  ...c.dataValues,
      ...
      exists.files = uniqBy(
        [
          ...exists.files,
          ...(objective.activityReportObjectives && objective.activityReportObjectives.length > 0
            ? objective.activityReportObjectives[0].activityReportObjectiveFiles.map((f) => ({
                ...f.file.dataValues,
                url: f.file.url,
              }))
            : []),
    ...
    const ttaProvided =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].ttaProvided
        ? objective.activityReportObjectives[0].ttaProvided
        : null
    const arOrder =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].arOrder
        ? objective.activityReportObjectives[0].arOrder
        : null
    const closeSuspendContext =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].closeSuspendContext
        ? objective.activityReportObjectives[0].closeSuspendContext
        : null
    const closeSuspendReason =
      objective.activityReportObjectives && objective.activityReportObjectives[0] && objective.activityReportObjectives[0].closeSuspendReason
    ...
    const newObjective = {
      ...
      useIpdCourses: objective.activityReportObjectives?.[0]?.useIpdCourses ?? objective.useIpdCourses ?? false,
      useFiles: objective.activityReportObjectives?.[0]?.useFiles ?? objective.useFiles ?? false,
      ...
      topics: reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveTopics', 'topic'),
      resources: reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveResources', 'resource', {}, 'value'),
      files:
        objective.activityReportObjectives && objective.activityReportObjectives.length > 0
          ? objective.activityReportObjectives[0].activityReportObjectiveFiles.map((f) => ({
              ...f.file.dataValues,
          ...
          : [],
      courses: reduceRelationThroughActivityReportObjectives(objective, 'activityReportObjectiveCourses', 'course'),
 ...
 */
function reducePrompts(forReport: boolean, newPrompts: IPrompt[] = [], promptsToReduce: IPrompt[] = []) {
  return newPrompts?.reduce((previousPrompts, currentPrompt) => {
    const promptId = currentPrompt.promptId ? currentPrompt.promptId : currentPrompt.dataValues.promptId

    const existingPrompt = previousPrompts.find((pp) => pp.promptId === currentPrompt.promptId)
    if (existingPrompt) {
      if (!forReport) {
        existingPrompt.response = uniq([...existingPrompt.response, ...currentPrompt.responses.flatMap((r) => r.response)])
      }

      if (forReport) {
        existingPrompt.response = uniq([...existingPrompt.response, ...(currentPrompt.response || []), ...(currentPrompt.reportResponse || [])])
        existingPrompt.reportResponse = uniq([...(existingPrompt.reportResponse || []), ...(currentPrompt.reportResponse || [])])
      }
    ...
    if (forReport) {
      newPrompt.response = uniq([...(currentPrompt.response || []), ...(currentPrompt.reportResponse || [])])
    ...
    return [...previousPrompts, newPrompt]
  }, promptsToReduce)
}
 ...
 */
export function reduceGoals(goals: IGoalModelInstance[], forReport = false): IReducedGoal[] {
  const objectivesReducer = forReport ? reduceObjectivesForActivityReport : reduceObjectives

  const where = (g: IReducedGoal, currentValue: IGoalModelInstance) =>
    forReport
      ? (g.name || '').trim() === (currentValue.dataValues.name || '').trim()
      : (g.name || '').trim() === (currentValue.dataValues.name || '').trim() && g.status === currentValue.dataValues.status

  function getGoalCollaboratorDetails(collabType: string, dataValues: IGoalModelInstance) {
  ...
  const r = goals.reduce((previousValues: IReducedGoal[], currentValue: IGoalModelInstance) => {
    try {
      ...
      if (existingGoal) {
        ...
        existingGoal.objectives = objectivesReducer(currentValue.objectives, existingGoal.objectives)

        if (forReport) {
          // Get the regular prompts for the report.
          const promptsToAdd = (currentValue.dataValues.prompts || []).map(
            (p) =>
              ({
                ...p,
          ...
          const promptsToAddForReview = (currentValue.dataValues.prompts || []).map(
            (p) =>
              ({
      ...
      const promptsForReview = forReport
        ? (currentValue.dataValues.prompts || []).map((p) => ({
            key: `${currentValue.grant.id}-${(p.response || []).join('-')}`,
            promptId: p.promptId,
            responses: p.response || [],
            recipients: [
      ...
      const goal = {
        ...
        objectives: objectivesReducer(currentValue.objectives),
      ...
      if (!forReport) {
        ...
        ].filter((c: { goalCreatorName: string }) => c.goalCreatorName !== null)
      }
