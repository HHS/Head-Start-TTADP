...
import db, { Goal, Grant, Recipient, Objective, ActivityReport, ActivityRecipient, ActivityReportGoal, ActivityReportObjective } from '../models'
...
describe('removeRemovedRecipientsGoals', () => {
  ...
  afterAll(async () => {
    ...
    await Promise.all(grants.map(async (g) => Grant.destroy({ where: { id: g.id }, individualHooks: true })))
  ...
  it('removes extra recipients', async () => {
    const [report, , goalsAndObjectives] = await activityReportAndRecipientsById(multiRecipientReport.id)
    ...
    const [, , goalsAndObjectivesAgain] = await activityReportAndRecipientsById(multiRecipientReport.id)
