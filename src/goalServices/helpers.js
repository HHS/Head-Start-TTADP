const responsesForComparison = (g) =>
  (g.responses || [])
    .map((gfr) => gfr.response)
    .sort()
    .join()
...
const findOrFailExistingGoal = (needle, haystack, translate = goalFieldTransate) => {
  const needleCollaborators = (needle.collaborators || []).map((c) => c.goalCreatorName).filter(Boolean)

  return haystack.find((g) => {
    const haystackCollaborators = (g.collaborators || []).map((c) => c.goalCreatorName).filter(Boolean)
    ...
    return (
      ...
      ((needleCollaborators.length === 0 && haystackCollaborators.length === 0) ||
        haystackCollaborators.some((collaborator) => needleCollaborators.includes(collaborator)))
    )
  })
}
