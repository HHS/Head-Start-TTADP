...
import { OBJECTIVE_STATUS, GOAL_STATUS, SOURCE_FIELD, CREATION_METHOD } from '../constants'
...
export async function goalsByIdsAndActivityReport(goalIds, activityReportId) {
  const goals = await Goal.findAll({
    ...
    include: [
      ...
      },
      {
        ...
        where: {
          ...
          [Op.or]: [
            { createdVia: 'rtr' },
            {
              [Op.and]: [{ createdVia: 'activityReport' }, { onApprovedAR: true }],
            },
            {
              [Op.and]: [
        ...
        attributes: ['id', ['title', 'label'], 'title', 'status', 'goalId', 'onApprovedAR', 'onAR'],
        required: false,
        include: [
          {
            model: ActivityReportObjective,
            as: 'activityReportObjectives',
            attributes: ['ttaProvided', 'closeSuspendReason', 'closeSuspendContext'],
            required: false,
            where: {
              activityReportId,
            },
      ...
      },
      {
        ...
        as: 'prompts',
        attributes: ['id', 'ordinal', 'title', 'prompt', 'hint', 'fieldType', 'options', 'validations'],
        required: false,
        include: [
          {
  ...
  const reformattedGoals = goals.map((goal) => ({
    ...
    objectives: goal.objectives.map((objective) => ({
      ...
      citations: extractObjectiveAssociationsFromActivityReportObjectives(objective.activityReportObjectives, 'activityReportObjectiveCitations'),
    })),
  }))
...
export async function createOrUpdateGoals(goals) {
  ...
  const goalIds = await Promise.all(
    goals.map(async (goalData, rtrOrder) => {
      const {
        ids,
        grantId,
        recipientId,
      ...
      const newObjectives = await Promise.all(
        objectives.map(async (o, index) => {
          const { title, status: objectiveStatus, id: objectiveIdsMayContainStrings } = o

          const objectiveIds = [objectiveIdsMayContainStrings].flat().filter((id) => parseInt(id, DECIMAL_BASE))
          ...
          objective.set({
            ...(!objective.onApprovedAR && title.trim() !== objective.dataValues.title.trim() && { title }),
            rtrOrder: index + 1,
          })
...
export async function goalsForGrants(grantIds) {
  ...
  const grants = await Grant.unscoped().findAll({
    attributes: ['id', [sequelize.fn('ARRAY_AGG', sequelize.fn('DISTINCT', sequelize.col('grantRelationships.grantId'))), 'oldGrantIds']],
    where: {
      id: grantIds,
    },
    include: [
   ...
   */
  const ids = Array.from(new Set(grants.reduce((previous, current) => [...previous, current.id, current.oldGrantId], []).filter((g) => g)))
  ...
  return Goal.findAll({
    attributes: [
      [sequelize.fn('ARRAY_AGG', sequelize.fn('DISTINCT', sequelize.col('grant.id'))), 'grantIds'],
      [sequelize.fn('ARRAY_AGG', sequelize.fn('DISTINCT', sequelize.col('"Goal"."id"'))), 'goalIds'],
      [sequelize.fn('ARRAY_AGG', sequelize.fn('DISTINCT', sequelize.col('grant.grantRelationships.grantId'))), 'oldGrantIds'],
      [sequelize.fn('MAX', sequelize.fn('DISTINCT', sequelize.col('"Goal"."createdAt"'))), 'created'],
      [sequelize.fn('MAX', sequelize.fn('DISTINCT', sequelize.col('"Goal"."goalTemplateId"'))), 'goalTemplateId'],
      'name',
      'status',
      'onApprovedAR',
      'source',
    ...
    group: ['"Goal"."name"', '"Goal"."status"', '"Goal"."onApprovedAR"', '"Goal"."source"', '"Goal"."createdVia"', '"Goal".id'],
    where: {
      name: {
    ...
    order: [[sequelize.fn('MAX', sequelize.fn('DISTINCT', sequelize.col('"Goal"."createdAt"'))), 'desc']],
  })
}
...
export async function setActivityReportGoalAsActivelyEdited(goalIdsAsString, reportId, pageState) {
  try {
    ...
    auditLogger.error(` SERVICE:GOALS:setActivityReportGoalsAsActivelyEdited\nunable to update ActivityReportGoals table \n${error}`)
...
export async function removeRemovedRecipientsGoals(removedRecipientIds, report) {
  if (!removedRecipientIds || !(Array.isArray(removedRecipientIds) && removedRecipientIds.length > 0)) {
    return null
  }
  ...
  const objectivesToDelete = objectives.filter((objective) => !objective.get('onOtherAr')).map((objective) => objective.id)
 ...
 */
export async function updateGoalStatusById(goalIds, userId, oldStatus, newStatus, closeSuspendReason, closeSuspendContext, previousStatus) {
  ...
  const allowed = verifyAllowedGoalStatusTransition(oldStatus, newStatus, previousStatus)
...
export async function destroyGoal(goalIds) {
  try {
    ...
    const objectivesDestroyed =
      Array.isArray(objectiveIds) && objectiveIds.length
        ? await Objective.destroy({
            where: {
 ...
 * }
 */
export const getReportCountForGoals = (goals) =>
 ...
 */
// eslint-disable-next-line max-len
export const hasMultipleGoalsOnSameActivityReport = (countObject) =>
  Object.values(countObject).some((grants) => Object.values(grants).some((c) => c > 1))
...
export async function createMultiRecipientGoalsFromAdmin(data) {
  ...
  if (goalsForNameCheck.length) {
    message = `A goal with that name already exists for grants ${goalsForNameCheck.map((g) => (g.grant ? g.grant.number : 'Unknown')).join(', ')}`
  }
  ...
  const grantsToCreateGoalsFor = grantIds.filter((g) => !grantsForWhomGoalAlreadyExists.includes(g))

  const goals = await Goal.bulkCreate(
    grantsToCreateGoalsFor.map((grantId) => ({
      name,
      grantId,
  ...
  const promptResponses = (data.goalPrompts || [])
    .map((goalPrompt) => {
  ...
  if (data.createReport && data.creator) {
    ...
    await Promise.all([
      ActivityReportGoal.bulkCreate(
        [...goalIds, ...goalsForNameCheck.map((g) => g.id)].map((goalId) => ({
...
export async function closeMultiRecipientGoalsFromAdmin(data, userId) {
  ...
  return {
    isError: false,
    goals: await updateGoalStatusById(goalIds, userId, status, GOAL_STATUS.CLOSED, closeSuspendReason, closeSuspendContext, [GOAL_STATUS.CLOSED]),
  }
}
