...
import { ActivityReport as ActivityReportModel, Role, ActivityReportApprover, User as UserModel, ActivityReportGoal, sequelize } from '../../models'
...
async function checkEmailSettings(report, setting) {
  ...
  const authorWithSetting = settingForAuthor && settingForAuthor.value === USER_SETTINGS.EMAIL.VALUES.IMMEDIATELY ? author : null

  const settingsForAllCollabs = activityReportCollaborators
    ? await Promise.all(activityReportCollaborators.map((c) => userSettingOverridesById(c.userId, setting)))
    : []
  ...
  programSpecialistsToNotify = await Promise.all(
    programSpecialistsToNotify.map(async (ps) => {
  ...
  programSpecialistsToNotify = await Promise.all(programSpecialistsToNotify.map(async (ps) => userById(ps.id)))
...
export async function reviewReport(req, res) {
  try {
    ...
    const [reviewedReport, activityRecipients] = await activityReportAndRecipientsById(activityReportId)

    if (reviewedReport.calculatedStatus === REPORT_STATUSES.APPROVED) {
      const [authorWithSetting, collabsWithSettings] = await checkEmailSettings(reviewedReport, USER_SETTINGS.EMAIL.KEYS.APPROVAL)
      ...
      const [, , programSpecialists] = await checkEmailSettings(reviewedReport, USER_SETTINGS.EMAIL.KEYS.RECIPIENT_APPROVAL)

      programSpecialistRecipientReportApprovedNotification(report, programSpecialists, activityRecipients)
    }

    if (reviewedReport.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION) {
      const [authorWithSetting, collabsWithSettings] = await checkEmailSettings(reviewedReport, USER_SETTINGS.EMAIL.KEYS.CHANGE_REQUESTED)
      changesRequestedNotification(reviewedReport, savedApprover, authorWithSetting, collabsWithSettings)
    }
...
export async function resetToDraft(req, res) {
  try {
    ...
    const [savedReport, activityRecipients, goalsAndObjectives, objectivesWithoutGoals] = await setStatus(report, REPORT_STATUSES.DRAFT)
...
export async function submitReport(req, res) {
  try {
    ...
    const settingsForAllCurrentApprovers = await Promise.all(
      currentApprovers.map((a) => userSettingOverridesById(a.userId, USER_SETTINGS.EMAIL.KEYS.SUBMITTED_FOR_REVIEW))
    )
...
export async function getReport(req, res) {
  const { activityReportId } = req.params
  const [report, activityRecipients, goalsAndObjectives, objectivesWithoutGoals] = await activityReportAndRecipientsById(activityReportId)
  if (!report) {
    res.sendStatus(404)
    return
  }
...
export async function getReportsByManyIds(req, res) {
  try {
    ...
    const { reportIds, offset, sortBy, sortDir, limit } = req.body
  ...
  try {
    ...
    if (savedReport.activityReportCollaborators) {
      ...
      const settingsForAllCollabs = await Promise.all(
        newCollaborators.map((c) => userSettingOverridesById(c.userId, USER_SETTINGS.EMAIL.KEYS.COLLABORATOR_ADDED))
      )
...
export async function createReport(req, res) {
  try {
    ...
    if (report.activityReportCollaborators) {
      ...
      const settingsForAllCollabs = await Promise.all(
        collabs.map((c) => userSettingOverridesById(c.userId, USER_SETTINGS.EMAIL.KEYS.COLLABORATOR_ADDED))
      )
...
export async function downloadReports(req, res) {
  try {
    ...
    const reports = await getDownloadableActivityReportsByIds(readRegions, req.query)
...
export async function downloadAllReports(req, res) {
  try {
    ...
    const reports = await getAllDownloadableActivityReports(readRegions['region.in'], { ...readRegions, limit: null }, userId, ids)
