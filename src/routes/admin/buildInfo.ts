...
export default async function buildInfo(req, res) {
  ...
  try {
    ...
    const branch = process.env.BUILD_BRANCH || (process.env.NODE_ENV !== 'production' ? await git.revparse(['--abbrev-ref', 'HEAD']) : '').trim()
    const commit = process.env.BUILD_COMMIT || (process.env.NODE_ENV !== 'production' ? await git.revparse(['HEAD']) : '').trim()
