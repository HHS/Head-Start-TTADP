...
const deleteOnlyFile = async (req, res) => {
  ...
  try {
    ...
    if (file.reports.length + file.reportObjectiveFiles.length + file.objectiveFiles.length + file.objectiveTemplateFiles.length === 0) {
...
const deleteHandler = async (req, res) => {
  ...
  try {
    ...
    if (reportId) {
      ...
      const rf = file.reportFiles.find((r) => r.activityReportId === parseInt(reportId, DECIMAL_BASE))
      if (rf) {
      ...
      const sof = file.sessionFiles.find((r) => r.sessionReportPilotId === parseInt(eventSessionId, DECIMAL_BASE))
      if (sof) {
      ...
      const clf = file.communicationLogFiles.find((r) => r.communicationLogId === parseInt(communicationLogId, DECIMAL_BASE))
      if (clf) {
      ...
      const sof = file.supportingAttachments.find((r) => r.sessionReportPilotId === parseInt(sessionAttachmentId, DECIMAL_BASE))
      if (sof) {
    ...
    const objectiveTemplateFilesLength = file.objectiveTemplateFiles ? file.objectiveTemplateFiles.length : 0
    const sessionLength = file.sessionFiles ? file.sessionFiles.length : 0

    const canDelete = reportLength + reportObjectiveLength + objectiveLength + objectiveTemplateFilesLength + sessionLength === 0
...
const uploadHandlerRequiredFields = (fields) =>
  ['reportId', 'reportObjectiveId', 'objectiveId', 'objectiveTempleteId', 'sessionId', 'communicationLogId', 'sessionAttachmentId'].some(
    (field) => fields[field]
  )
...
const getAuthorizationAndMetadataFn = async (user, fields) => {
  ...
  if (reportId && objectiveIds) {
    ...
    if (!((await hasReportAuthorization(user, parsedReportId)) || (await validateUserAuthForAdmin(userId)))) {
      error = 'Unauthorized'
    ...
    return {
      ...
      metadataFn: async (originalFileName, fileName, size) =>
        createActivityReportObjectiveFileMetaData(originalFileName, fileName, activityReportObjectiveIds, size),
    }
  }
  ...
  if (reportId) {
    ...
    return {
      ...
      status: null,
      // eslint-disable-next-line max-len
      metadataFn: async (originalFileName, fileName, size) => createActivityReportFileMetaData(originalFileName, fileName, reportId, size),
    }
  }
  ...
  if (sessionId) {
    ...
    return {
      ...
      status: null,
      // eslint-disable-next-line max-len
      metadataFn: async (originalFileName, fileName, size) => createSessionObjectiveFileMetaData(originalFileName, fileName, sessionId, size),
    }
  }
  ...
  if (sessionAttachmentId) {
    ...
    return {
      ...
      // eslint-disable-next-line max-len
      metadataFn: async (originalFileName, fileName, size) =>
        createSessionSupportingAttachmentMetaData(originalFileName, fileName, sessionAttachmentId, size),
    }
  }
