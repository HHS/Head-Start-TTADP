...
import db, { File, ActivityReport, ActivityReportFile, User, Grant, Goal, Objective, Recipient } from '../../models'
// import app from '../../app';
...
describe('File Upload', () => {
  ...
  describe('File Upload Handlers error handling', () => {
    it('tests a file upload without a report id', async () => {
      ...
      await request(app).post('/api/files').attach('file', `${__dirname}/testfiles/testfile.pdf`).expect(400, {
        error:
    ...
    it('tests a file upload without a file', async () => {
      ...
      await request(app).post('/api/files').field('reportId', report.dataValues.id).expect(400, { error: 'file required' })
      expect(uploadFile).not.toHaveBeenCalled()
    })
    ...
    it('tests an upload failure', async () => {
      ...
      await request(app)
        ...
        .then(() => {
          expect(uploadFile).toHaveBeenCalled()
          expect(updateStatus).toHaveBeenCalledWith(expect.any(Number), FILE_STATUSES.UPLOAD_FAILED)
        })
    })
  })
})
