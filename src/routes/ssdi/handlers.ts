...
const validateScriptPath = async (scriptPath: string, user, res: Response, skipFileCheck = false) => {
...
const filterAttributes = <T extends object>(obj: T, keysToRemove: (keyof T)[]): Partial<T> =>
  Object.fromEntries(Object.entries(obj).filter(([key]) => !keysToRemove.includes(key as keyof T))) as Partial<T>
...
const runQuery = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (filterValues['region.in'] && Array.isArray(filterValues['region.in'])) {
      filterValues['region.in'] = filterValues['region.in'].map(Number).filter((num) => !Number.isNaN(num))
