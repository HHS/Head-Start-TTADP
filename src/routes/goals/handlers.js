...
export async function changeGoalStatus(req, res) {
  try {
    ...
    const updatedGoal = await updateGoalStatusById(ids, userId, oldStatus, newStatus, closeSuspendReason, closeSuspendContext, previousStatus)
...
export async function getGoalHistory(req, res) {
  try {
    ...
    const hasPermissionInRegion = user.permissions.some((permission) => permission.regionId === grantRecord.regionId)
