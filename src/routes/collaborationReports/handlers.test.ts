...
describe('Collaboration Reports Handlers', () => {
  ...
  describe('getReports', () => {
    ...
    it('should handle currentUserId errors', async () => {
      ...
      expect(handleErrors).toHaveBeenCalledWith(mockRequest, mockResponse, error, { namespace: 'SERVICE:COLLAB_REPORTS' })
      expect(mockJson).not.toHaveBeenCalled()
    })
  ...
  describe('submitReport', () => {
    ...
    it('should not reset approver statuses when an initial approver is added', async () => {
      ...
      expect(CRServices.createOrUpdateReport).toHaveBeenCalledWith({ ...existingReport, ...mockRequest.body, lastUpdatedById: 123 }, existingReport)
  ...
  describe('saveReport', () => {
    ...
    it('should successfully save a report', async () => {
      ...
      const savedReport = {
        ...existingReport,
        ...updatedReport,
        collabReportSpecialists: [{ user: { email: 'existing@example.com' } }, { userId: 456, user: { email: 'new@example.com' } }],
      }
      ...
      expect(CRServices.collabReportById).toHaveBeenCalledWith('1')
      expect(CRServices.createOrUpdateReport).toHaveBeenCalledWith({ ...existingReport, ...updatedReport }, existingReport)
    })
    ...
    it('should remove regionId from updates', async () => {
      ...
      expect(CRServices.createOrUpdateReport).toHaveBeenCalledWith({ ...existingReport, ...expectedUpdate }, existingReport)
    })
  ...
  describe('downloadReports', () => {
    ...
    it('should successfully download reports as CSV', async () => {
      ...
      ;(collabReportToCsvRecord as jest.Mock).mockResolvedValueOnce(mockCsvRecords[0]).mockResolvedValueOnce(mockCsvRecords[1])
  ...
  describe('sendCollabReportCSV', () => {
    ...
    it('should handle reports with special characters in CSV', async () => {
      const mockReports = [{ id: '1', name: 'Report "with quotes"', description: 'Description,with,commas' }]
