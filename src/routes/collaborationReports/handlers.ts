...
import { NOT_FOUND, FORBIDDEN, BAD_REQUEST, NO_CONTENT } from 'http-codes'
import CollabReportPolicy from '../../policies/collabReport'
import { collabReportById, createOrUpdateReport, getReports as getReportsService, deleteReport, getCSVReports } from '../../services/collabReports'
...
export async function submitReport(req: Request, res: Response) {
  try {
    ...
    const newApprovers = (newReport.approvers || []).map((a) => a.user?.id)
    const oldApprovers = (existingReport.approvers || []).map((a) => a.user?.id)
    const oldSet = new Set(oldApprovers.filter((id) => id != null))
