...
export const getByStatus = async (req, res) => {
  try {
    ...
    const events = await findEventsByStatus(status, auth.readableRegions, userId, null, false, scopes, auth.isAdmin())
...
export const updateHandler = async (req, res) => {
  try {
    ...
    if (req.body.data && req.body.data.status) {
      ...
      if (status === TRAINING_REPORT_STATUSES.COMPLETE || status === TRAINING_REPORT_STATUSES.SUSPENDED) {
