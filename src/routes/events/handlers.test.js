...
import { getHandler, createHandler, updateHandler, deleteHandler, getByStatus, getTrainingReportAlertsHandler } from './handlers'
import {
  createEvent,
...
describe('event handlers', () => {
    ...
    it('filters sessions based on user role - owner sees all sessions', async () => {
      const eventWithSessions = {
        ...
        sessionReports: [
          {
            id: 1,
            data: { status: TRAINING_REPORT_STATUSES.IN_PROGRESS, sessionName: 'Session 1' },
          },
    ...
    it('filters sessions for regional user - only complete sessions', async () => {
      const eventWithSessions = {
        ...
        sessionReports: [
          ...
          {
            id: 2,
            data: { status: TRAINING_REPORT_STATUSES.COMPLETE, sessionName: 'Session 2' },
            approverId: null,
            submitted: false,
          },
        ],
    ...
    it('filters sessions for approver - only submitted sessions they are assigned to', async () => {
      const eventWithSessions = {
        ...
        sessionReports: [
          ...
          {
            id: 2,
            data: { status: TRAINING_REPORT_STATUSES.IN_PROGRESS, sessionName: 'Session 2' },
            approverId: 99_999,
            submitted: false,
          },
          {
            id: 3,
            data: { status: TRAINING_REPORT_STATUSES.IN_PROGRESS, sessionName: 'Session 3' },
            approverId: 5,
            submitted: true,
          },
        ],
    ...
    it('admin sees all sessions regardless of role or status', async () => {
      const eventWithSessions = {
        ...
        sessionReports: [
          ...
          {
            id: 3,
            data: { status: TRAINING_REPORT_STATUSES.NOT_STARTED, sessionName: 'Session 3' },
          },
        ],
      }
  ...
  describe('deleteHandler', () => {
    it('works', async () => {
      ...
      findEventBySmartsheetIdSuffix.mockResolvedValue(mockEvent)
      findEventBySmartsheetIdSuffix.mockResolvedValue(mockEvent)
      await deleteHandler({ session: { userId: 1 }, params: { eventId: mockEvent.id } }, mockResponse)
      expect(mockResponse.status).toHaveBeenCalledWith(200)
    })
    ...
    it('handles errors', async () => {
      ...
      await deleteHandler({ session: { userId: 1 }, params: { eventId: mockEvent.id }, query: {} }, mockResponse)
