...
const newEvent = ({ ownerId, pocIds = [], collaboratorIds = [], regionId = 1, status = null, data }) => ({
  ownerId,
  pocIds,
  collaboratorIds,
  regionId,
...
describe('findEventByStatus', () => {
  ...
  afterAll(async () => {
    await Permission.destroy({
      where: {
        userId: [owner.id, collaborator.id, poc.id, otherPoc.id, otherRead.id, otherWrite.id, seesNone.id],
      },
    })

    await User.destroy({
      where: {
        id: [owner.id, collaborator.id, poc.id, otherWrite.id, otherRead.id, otherPoc.id, seesNone.id],
      },
    })
  ...
  describe('when status is IN_PROGRESS', () => {
    ...
    beforeAll(async () => {
      ...
      await createSession(newSession({ eventId: e.id, data: { status: TRAINING_REPORT_STATUSES.COMPLETE } }))
      await createSession(newSession({ eventId: e.id, data: { status: TRAINING_REPORT_STATUSES.IN_PROGRESS } }))
    })
