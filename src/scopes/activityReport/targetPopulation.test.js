import { Op, filtersToScopes, ActivityReport, submittedReport, setupSharedTestData, tearDownSharedTestData } from './testHelpers'
...
describe('target population filtersToScopes', () => {
  ...
  describe('target population', () => {
    ...
    it('filters out the appropriate population', async () => {
      ...
      expect(found.length).toBe(2)
      expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([reportThree.id, reportFour.id]))
    })

    it('only filters by possible population values', async () => {
      const filters = {
        'targetPopulations.in': ['(DROP SCHEMA public CASCADE)', 'Infants and Toddlers (ages birth to 3)'],
      }
      const { activityReport: scope } = await filtersToScopes(filters)
      const found = await ActivityReport.findAll({
        where: { [Op.and]: [scope, { id: possibleIds }] },
    ...
    it('filters out bad population values', async () => {
      ...
      expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([reportOne.id, reportTwo.id, reportThree.id, reportFour.id]))
    })
  })
})
