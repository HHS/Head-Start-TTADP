...
describe('lastSaved filtersToScopes', () => {
  ...
  describe('lastSaved', () => {
    ...
    it('before returns reports with updated ats before the given date', async () => {
      ...
      expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([firstReport.id, secondReport.id, sharedTestData.globallyExcludedReport.id]))
    })
    ...
    it('after returns reports with updated ats before the given date', async () => {
      ...
      expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([thirdReport.id, fourthReport.id]))
    })
    ...
    it('within returns reports with updated ats between the two dates', async () => {
      ...
      expect(found.length).toBe(2)
      expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([secondReport.id, thirdReport.id]))
    })
  })
})
