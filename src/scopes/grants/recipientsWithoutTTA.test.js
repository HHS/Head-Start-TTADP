import { Op, filtersToScopes, Grant, sequelize, recipients, setupSharedTestData, tearDownSharedTestData, sharedTestData } from './testHelpers'
...
describe('grants/recipientsWithoutTTA', () => {
  ...
  it('within plus inactivation date', async () => {
    ...
    expect(found.map((f) => f.id)).toEqual(expect.arrayContaining([recipients[0].id, recipients[3].id]))
  })
})
