...
export function withGrantNumber(grantNumber) {
  ...
  return {
    [Op.or]: sequelize.literal(
      `"Goal"."id" in (${grantNumberQuery} ILIKE ANY(ARRAY[${numbers.join(',')}]))`
    ),
  };
}
...
export function withoutGrantNumber(grantNumber) {
  ...
  return {
    [Op.and]: sequelize.literal(
      `"Goal"."id" not in (${grantNumberQuery} ILIKE ANY(ARRAY[${numbers.join(',')}]))`
    ),
  };
}
...
export function containsGrantNumber(grantNumber) {
  return {
    [Op.or]: [filterAssociation(grantNumberQuery, grantNumber, false)],
  };
}

export function doesNotContainGrantNumber(grantNumber) {
  return {
    [Op.and]: [filterAssociation(grantNumberQuery, grantNumber, true)],
  };
}
