...
describe('authMiddleware', () => {
  ...
  it('getUserInfo: throws if accessToken or subject missing', async () => {
    await expect(getUserInfo(undefined, 'user-123')).rejects.toThrow('Access token and subject are required')
    await expect(getUserInfo('fake-access', undefined)).rejects.toThrow('Access token and subject are required')
  })
  ...
  it('logoutOidc: redirects to IdP end-session URL and clears session (with id_token_hint)', async () => {
    ...
    expect(res.clearCookie).toHaveBeenCalledWith('session', expect.objectContaining({ path: '/', httpOnly: true, sameSite: 'lax' }))
