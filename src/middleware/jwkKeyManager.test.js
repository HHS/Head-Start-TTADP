...
describe('jwkKeyManager', () => {
  ...
  test('returns public JWK with computed kid and cloned results', async () => {
    ...
    const thumbCalls = jest.isMockFunction(jose.calculateJwkThumbprint) ? jose.calculateJwkThumbprint.mock.calls.length : 1
  ...
  test('returns private key and handles caching/cloning appropriately', async () => {
    ...
    const jwkShapeOk = isKey1
      ? true
      : k1.kty === 'RSA' && k1.n === 'test-n' && k1.e === 'AQAB' && k1.d === 'test-d' && k1.alg === 'RS256' && k1.use === 'sig'
