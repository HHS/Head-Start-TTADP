...
export default function SubmittedCollabReport({ report }) {
  ...
  const formattedMethod =
    COLLAB_REPORT_CONDUCT_METHODS.filter((m) => m.value === report.conductMethod)[0]?.label || '';
  const formattedStates =
    statesInvolved?.map((activityStateCode) => STATES[activityStateCode] || '').join(', ') || '';
  const formattedReasons = reportReasons
    ?.map((reasonId) => COLLAB_REPORT_REASONS[reasonId] || '')
    .join(', ');
  const formattedGoals =
    reportGoals?.map((goal) => goal?.goalTemplate?.standard || '').join(', ') || 'None';
