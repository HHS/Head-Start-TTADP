...
export default function SubmittedReport({ data }) {
  ...
  const participantCount = deliveryMethod === 'hybrid' ? data.numberOfParticipantsInPerson.toString() : data.numberOfParticipants.toString()

  const participantVirtualCount = data.numberOfParticipantsVirtually ? data.numberOfParticipantsVirtually.toString() : null
