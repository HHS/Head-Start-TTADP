...
import { createDateFilter, reasonsFilter as goalReasonsFilter, statusFilter, topicsFilter as goalTopicsFilter } from '../goalFilters'
import { creatorFilter, eventIdFilter, collaboratorsFilter, startDateFilter, regionFilter } from '../trainingReportFilters'
...
describe('Filter Menu', () => {
  ...
  const renderFilterMenu = (filters = [], onApplyFilters = jest.fn(), filterConfig = TTAHISTORY_FILTER_CONFIG) => {
    const user = {
      permissions: [
        {
          regionId: 1,
    ...
    render(
      <UserContext.Provider value={{ user }}>
        <h1>Filter menu</h1>
        <div>
          <FilterMenu filters={filters} onApplyFilters={onApplyFilters} applyButtonAria="apply test filters" filterConfig={filterConfig} />
        </div>
  ...
  it('renders training report filters', async () => {
    const config = [creatorFilter, eventIdFilter, collaboratorsFilter, startDateFilter, regionFilter]
  ...
  it('renders domain result filters', async () => {
    const config = [domainEmotionalSupportFilter, domainClassroomOrganizationFilter, domainInstructionalSupportFilter]
