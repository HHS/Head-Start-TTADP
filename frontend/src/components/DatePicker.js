...
export default function DatePicker({ onChange, defaultValue, minDate, maxDate, datePickerKey, error, setError, ...props }) {
  const [currentDate, setCurrentDate] = useState('')
  const today = useMemo(() => moment().format(DATE_PICKER_DATE_FORMAT), [])

  const formattedDefaultValue = moment(defaultValue, DATE_DISPLAY_FORMAT).format(DATE_PICKER_DATE_FORMAT)

  const formattedMaxDate = maxDate ? moment(maxDate, DATE_DISPLAY_FORMAT).format(DATE_PICKER_DATE_FORMAT) : today
