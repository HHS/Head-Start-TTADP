...
export default function WidgetContainer({
  ...
  return (
    <Container
      className={`smart-hub-widget-container width-full height-full shadow-2 padding-top-0 ${className}`}
      paddingX={0}
      paddingY={0}
      loading={loading}
    ...
    >
      ...
      <div className="margin-top-0">{children}</div>
      {showPagingBottom || footNote ? (
        <div
          className={`border-bottom smart-hub-border-base-lighter padding-3 ${displayPaginationBoxOutline ? 'smart-hub-border-base--pagination-box' : ''}`}
        >
          {footNote && <p className="usa-prose font-sans-3xs margin-top-0">{footNote}</p>}
          {showPagingBottom && (
            <PaginationCard
              currentPage={currentPage}
              totalCount={totalCount}
