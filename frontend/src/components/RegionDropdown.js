...
function RegionDropdown({ id, name, value, onChange, includeCentralOffice, includeAll }) {
  return (
    <>
      <Label htmlFor={id}>Region</Label>
      <Dropdown id={id} name={name} value={value} onChange={onChange}>
        <option name="default" disabled hidden value={0}>
          Select a region...
        </option>
        {REGIONS.map((number) => (
        ...
        {includeCentralOffice && (
          <option name="central-office" value={14}>
            Central Office
          </option>
        )}
        {includeAll && (
          <option name="central-office" value={ALL_REGIONS}>
            All
          </option>
        )}
      </Dropdown>
