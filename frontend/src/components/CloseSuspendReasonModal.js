...
const CloseSuspendReasonModal = ({ modalRef, goalIds, newStatus, onSubmit, resetValues, oldGoalStatus }) => {
      ...
      >
        <Form key={`close-suspend-reason-form-goal-${key}`}>
          <FormGroup error={showValidationError} className="margin-top-0">
            <Fieldset>
              <ErrorMessage>{showValidationError ? `Please select a reason for ${reasonDisplayStatus} goal.` : ''}</ErrorMessage>
