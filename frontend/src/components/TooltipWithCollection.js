...
export default function TooltipWithCollection({ collection, collectionTitle, position }) {
  ...
  const tags = collection.map((member) => (
    <span key={uuidv4()} className="smart-hub-tooltip--truncated smart-hub-tooltip--truncated--with-collection">
      {member}
      &nbsp;
    </span>
  ))
  ...
  return <Tooltip displayText={tags} tooltipText={tooltip} buttonLabel={`click to visually reveal the ${collectionTitle}`} position={position} />
}
