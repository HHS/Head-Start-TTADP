...
import { getGroupsForSession, getPossibleSessionParticipants } from '../fetchers/session';
...
const RecipientsWithGroups = ({ regionId, states, showTooltip }) => {
  ...
  const handleGroupChange = (event) => {
    ...
    const selectedGroupRecipients = options.reduce((acc, curr) => {
      const groupRecipients = curr.options.filter((option) =>
        groupToUse.recipients.includes(parseInt(option.value, DECIMAL_BASE))
      );
    ...
    <>
      ...
      {groups.length > 0 && (
        <div className="smart-hub-activity-summary-use-group margin-top-0 display-flex flex-align-center">
          ...
          />
          {showTooltip && (
            <QuestionTooltip text="You can use a group to speed up selection, then remove recipients who did not attend." />
          )}
        </div>
      )}
