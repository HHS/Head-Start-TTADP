...
import { Dropdown, Checkbox } from '@trussworks/react-uswds'
...
const RecipientsWithGroups = ({ regionId, states, showTooltip }) => {
  ...
  useDeepCompareEffect(() => {
    if (useGroups) {
      ...
      const selectedRecipientsNotInGroup = usedRecipientIds.filter((option) => !groupRecipientIds.includes(option))

      // If the user changes recipients manually while using groups.
      if (watchGroup && (groupRecipientIds.length !== watchFormRecipients.length || selectedRecipientsNotInGroup.length > 0)) {
  ...
  const handleGroupChange = (event) => {
    ...
    const selectedGroupRecipients = options.reduce((acc, curr) => {
      const groupRecipients = curr.options.filter((option) => groupToUse.recipients.includes(parseInt(option.value, DECIMAL_BASE)))
  ...
  return (
    <>
      {showGroupInfo && <GroupAlert resetGroup={resetGroup} />}
      {useGroups ? (
        <div className="margin-top-2">
      ...
      {groups.length > 0 && (
        <div className="smart-hub-activity-summary-use-group margin-top-0 display-flex flex-align-center">
          <Checkbox
            ...
            checked={useGroups}
          />
          {showTooltip && <QuestionTooltip text="You can use a group to speed up selection, then remove recipients who did not attend." />}
        </div>
      )}
      {useGroups ? renderRecipients() : null}
    </>
  )
}
