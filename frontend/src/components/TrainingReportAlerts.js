...
const ACTIONS_NEEDED = {
  noSessionsCreated: (alert) => (
    <>
      <Link to={`/training-report/${idForLink(alert.eventId)}/session/new/`} data-sort="no-sessions-created">
        Create a session
      </Link>
    </>
  ),
  missingEventInfo: (alert) => (
    <>
      <Link to={`/training-report/${idForLink(alert.eventId)}/event-summary`} data-sort="missing-event-info">
        Missing event info
      </Link>
    </>
  ),
  missingSessionInfo: (alert) => (
    <>
      <Link to={`/training-report/${idForLink(alert.eventId)}/session/${alert.id}/session-summary`} data-sort="missing-session-info">
  ...
  eventNotCompleted: (alert) => (
    <>
      <Link to={`/training-report/view/${idForLink(alert.eventId)}`} data-sort="event-not-completed">
        Event not completed
      </Link>
    </>
  ),
  waitingForApproval: (alert) => (
    <>
      <Link to={`/training-report/${idForLink(alert.eventId)}/session/${alert.id}/review`} data-sort="waiting-for-approval">
  ...
  changesNeeded: (alert) => (
    <>
      <Link to={`/training-report/${idForLink(alert.eventId)}/session/${alert.id}/review`} data-sort="changes-needed">
        Changes needed
      </Link>
    </>
  ),
    ...
    >
      {alertsForTable.length ? (
        <div className="ttahub-training-report-alerts-scrollable">
          <SimpleSortableTable className="ttahub-training-report-alerts" columns={columns} data={alertsForTable} elementSortProp="data-sort" />
        </div>
      ) : (
