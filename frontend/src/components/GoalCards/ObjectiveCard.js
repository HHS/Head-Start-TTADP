...
function ObjectiveCard({ objective, objectivesExpanded, goalStatus, regionId, dispatchStatusChange, forceReadOnly, isMonitoringGoal }) {
  const { title, endDate, topics = [], status, activityReports, supportType, ids, citations } = objective
  ...
  const onChangeStatus = async (newStatus) => {
    try {
      ...
      await updateObjectiveStatus(ids, regionId, newStatus, localCloseSuspendReason, localCloseSuspendContext)
        ...
        <div>
          {statusChangeError && (
            <Alert type="error" className="margin-top-1">
              There was an error updating the status of this objective. For more assistance, please contact support.
            </Alert>
          )}
