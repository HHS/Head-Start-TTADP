...
describe('Goals Table', () => {
  ...
  describe('Table displays objective data', () => {
    ...
    it('Expands and collapses objectives', async () => {
      ...
      expect(
        document.querySelector('.ttahub-goal-card__objective-list[hidden]')
      ).toBeInTheDocument();

      // Expand Objectives via click.
      const expandObjectives = await screen.findByRole('button', {
        name: 'View objectives for goal G-4598',
      });
      fireEvent.click(expandObjectives);
      expect(
        document.querySelector('.ttahub-goal-card__objective-list[hidden]')
      ...
      const collapseButton = await screen.findByRole('button', {
        name: 'Hide objectives for goal G-4598',
      });
  ...
  describe('Context Menu', () => {
    ...
    it('calls print passing all goal ids on the page', async () => {
      ...
      expect(history.push).toHaveBeenCalledWith(
        '/recipient-tta-records/1000/region/1/rttapa/print',
        {
  ...
  describe('Context Menu with Different User Permissions', () => {
    ...
    it("Hides the reopen button if the user doesn't have permissions", async () => {
      ...
      renderTable(
        { goals: [{ ...baseGoals[2], status: GOAL_STATUS.CLOSED }], goalsCount: 1 },
        user
      );
