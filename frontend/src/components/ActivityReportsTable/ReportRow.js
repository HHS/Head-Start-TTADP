...
function ReportRow({
  ...
  return (
    <tr onFocus={onFocus} onBlur={onBlur} className={trClassname} key={`landing_${id}`}>
      ...
      <td data-label="Created date">{format(parseISO(createdAt), DATE_DISPLAY_FORMAT)}</td>
      ...
      <td data-label="Approved at">{approvedAt && format(parseISO(approvedAt), DATE_DISPLAY_FORMAT)}</td>
