...
describe('ObjectiveFileUploader', () => {
  ...
  const RenderFileUploader = ({ onChange, files, upload = jest.fn(), reportId = 0, objIds = [] }) => (
  ...
  it('onDrop adds calls the onChange method', async () => {
    ...
    const FileUploader = <RenderFileUploader files={[]} onChange={mockOnChange} />
  ...
  it('files are properly displayed and can be removed', async () => {
    ...
    render(
      <RenderFileUploader
        ...
        files={[file('fileOne', 1, null), { ...file('fileTwo', 2), ObjectiveFile: { objectiveId: 1 } }, file('fileThree', 'abc')]}
      />
    )
  ...
  it('files are properly removed when on a report', async () => {
    ...
    render(
      <RenderFileUploader
        ...
        objIds={[1]}
        onChange={mockOnChange}
        files={[file('fileOne', 1, null), { ...file('fileTwo', 2), ObjectiveFile: { objectiveId: 1 } }]}
      />
    )
    expect(screen.getByText('fileOne')).toBeVisible()
    expect(screen.getByText('fileTwo')).toBeVisible()
