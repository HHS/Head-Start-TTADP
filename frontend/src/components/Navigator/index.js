...
import { Form, Grid, Alert } from '@trussworks/react-uswds'
...
const Navigator = ({
  ...
  const navigatorPages = pages.map((p) => {
    ...
    const hasGoalBeingEdited = goalForEditing != null && typeof goalForEditing === 'object' && Object.keys(goalForEditing).length > 0
  ...
  const DraftAlert = () => (
    <DismissingComponentWrapper shown={showSavedDraft} updateShown={updateShowSavedDraft} hideFromScreenReader={false}>
      {lastSaveTime && (
        ...
        <div id="navigator-form" className="navigator-form">
          ...
          {!page.review && (
            <Container skipTopPadding>
              <NavigatorHeader key={page.label} label={page.label} titleOverride={page.titleOverride} formData={formData} />
