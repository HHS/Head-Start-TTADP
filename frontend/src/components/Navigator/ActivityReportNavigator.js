...
const ActivityReportNavigator = ({
  ...
  const [isGoalFormClosed, toggleGoalForm] = useState(!goalForEditing && selectedGoals && selectedGoals.length > 0)
  ...
  const onSaveDraftGoalForNavigation = async () => {
    ...
    const allGoals = packageGoals(selectedGoals, goal, grantIds, prompts, goalForEditing?.originalIndex)
  ...
  const onSaveDraftGoal = async (isAutoSave = false) => {
    ...
    let allGoals = packageGoals(selectedGoals, goal, grantIds, prompts, goalForEditing?.originalIndex)
    ...
    try {
      ...
      const { goals, goalForEditing: newGoalForEditing } = convertGoalsToFormData(allGoals, grantIds, currentFormData.calculatedStatus, goalOrder)
