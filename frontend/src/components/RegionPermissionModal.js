...
function RegionPermissionModal({ filters, user, showFilterWithMyRegions }) {
  ...
  const missingRegionsList =
    missingRegions && missingRegions.length > 0
      ? missingRegions.sort((a, b) => a - b).join(', ')
      : '';
  ...
  const openSmartSheetRequest = () => (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      onClick={requestSmartSheetAccess}
    >
  ...
  return (
    <div className="smart-hub--region-permission-modal">
      ...
      >
        <p>
          Most TTA Hub users have access to one region. This link might be from someone working in a
          different region.
        </p>
      </Modal>
    </div>
