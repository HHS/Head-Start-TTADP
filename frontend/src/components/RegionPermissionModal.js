...
function RegionPermissionModal({ filters, user, showFilterWithMyRegions }) {
  ...
  const missingRegions = useMemo(
    () =>
      filters
        .filter((f) => f.topic === 'region' && f.condition !== 'is not' && !userRegions.includes(parseInt(f.query, DECIMAL_BASE)))
  ...
  useEffect(() => {
    if (missingRegions && missingRegions.length > 0 && !modalRef.current.modalIsOpen) {
  ...
  const openSmartSheetRequest = () => (
    <a href={SUPPORT_LINK} className="usa-button usa-button--primary" target="_blank" rel="noreferrer" onClick={requestSmartSheetAccess}>
      Request access via Smartsheet
    </a>
  )
  return (
    <div className="smart-hub--region-permission-modal">
      <Modal
        ...
        forceAction
      >
        <p>Most TTA Hub users have access to one region. This link might be from someone working in a different region.</p>
      </Modal>
    </div>
  )
}
