...
import { Form, FormGroup, ErrorMessage, Label, Fieldset, Radio, Textarea } from '@trussworks/react-uswds'
      ...
      >
        <Form key={`reopen-reason-form-goal-${goalId}`}>
          <FormGroup error={showValidationError} className="margin-top-0">
            <ErrorMessage>{showValidationError ? 'Please select a reason for reopening this goal.' : null}</ErrorMessage>
            <Fieldset>{generateReasonRadioButtons()}</Fieldset>
          </FormGroup>
          <FormGroup>
            <Fieldset>
              <Label htmlFor="reopen-reason-context">Additional context</Label>
              <Textarea id="reopen-reason-context" name="reopen-reason-context" type="text" value={reopenContext} onChange={contextChanged} />
            </Fieldset>
          </FormGroup>
        </Form>
      </Modal>
