...
function HeaderUserMenu({ areThereUnreadNotifications, setAreThereUnreadNotifications }) {
  ...
  const [isImpersonating, setIsImpersonating] = useState(haveStorage && window.sessionStorage.getItem(SESSION_STORAGE_IMPERSONATION_KEY) !== null)
  ...
  const menuItems = useMemo(
    () =>
      [
        ...
        .map(({ key, label, to, href, external = false, divider = false, space = false, showIfAdmin = false, badge = <></>, fn = onItemClick }) => {
          ...
          if (href) {
            return {
              ...
              presentation: false,
              element: (
                <a key={key} className="usa-nav__link" href={href} onClick={fn} rel="noopener noreferrer">
                  <span>{label}</span>
                  {badge}
                </a>
              ),
  ...
  Av.propTypes = {
    ...
    clickOutsideRef: PropTypes.oneOfType([PropTypes.shape({ current: PropTypes.instanceOf(Element) }), PropTypes.func]),
  }
  ...
  return (
    <DropdownMenu Trigger={Av} onApply={() => {}} buttonText={user.name} showApplyButton={false} direction="left" className="no-print">
