...
export const objectivesWithValidResourcesOnly = (objectives) => {
  ...
  return objectives.map((objective) => ({
    ...objective,
    resources: objective.resources.map((r) => ({ ...r, value: r.value.trim() })).filter((resource) => isValidResourceUrl(resource.value)),
  }))
}
...
export const OBJECTIVE_LINK_ERROR = (
  <span className="usa-error-message">Enter one resource per field. Valid resource links must start with http:// or https://</span>
)
...
export const grantsToGoals = ({ selectedGrants, name, status, source, isCurated, regionId, recipient, objectives, ids, prompts }) =>
