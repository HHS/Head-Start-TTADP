import { Alert, ErrorMessage, Fieldset, FormGroup, Label, Radio } from '@trussworks/react-uswds';
...
export default function ObjectiveFiles({
  ...
  const hideFileToggle = useMemo(
    () => hasFiles && files.some((file) => file.onAnyReport),
    [hasFiles, files]
  );
  ...
  const showSaveDraftInfo =
    forceObjectiveSave && (!selectedObjectiveId || !(typeof selectedObjectiveId === 'number'));
    ...
    <>
      <Fieldset className="ttahub-objective-files margin-top-3">
        ...
        {useFiles && !showSaveDraftInfo ? (
          <>
            <FormGroup
              className="ttahub-objective-files-dropzone margin-top-2 margin-bottom-0"
              error={fileError}
            >
