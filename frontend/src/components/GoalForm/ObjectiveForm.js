...
import { OBJECTIVE_ERROR_MESSAGES, OBJECTIVE_FORM_FIELD_INDEXES } from './constants';
...
export default function ObjectiveForm({
  ...
  const validateObjectiveTitle = () => {
    if (!title) {
      ...
      newErrors.splice(
        OBJECTIVE_FORM_FIELD_INDEXES.TITLE,
        1,
        <span className="usa-error-message">{objectiveTitleError}</span>
      );
