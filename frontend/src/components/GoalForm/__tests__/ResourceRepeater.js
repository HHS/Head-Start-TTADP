...
describe('ResourceRepeater', () => {
  it('shows the regular view', async () => {
    ...
    expect(
      await screen.findByText(/Did you use any other TTA resources that are available as a link/i)
    ).toBeVisible();
  ...
  it('render with alternate tool tip', async () => {
    ...
    expect(
      screen.queryAllByText(
        "Copy & paste web address of TTA resource you'll use for this objective. Usually a HeadStart.gov page."
      ).length
    ).toBe(2);
  });
  ...
  it('cannot add a resource if there is an error', async () => {
    ...
    render(
      <ResourceRepeater
        ...
        resources={[{ key: 1, value: 'garbelasdf' }]}
        setResources={setResources}
        validateResources={jest.fn()}
        status={OBJECTIVE_STATUS.IN_PROGRESS}
        isOnReport={false}
