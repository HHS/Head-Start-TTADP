...
import { Alert, Button, TextInput, Dropdown } from '@trussworks/react-uswds'
  ...
  return (
    <>
      ...
      {isEditing && multiGrant && (
        <Alert type="info">All of the recipients on this Communication log will receive the same updates once edits are saved.</Alert>
      )}
      ...
      <div className="margin-top-2">
        <FormItem label="Other TTA staff" name="otherStaff" id="otherStaff-label" htmlFor="otherStaff" required={false}>
          <MultiSelect
            control={control}
            simple={false}
            name="otherStaff"
      ...
      <div className="margin-top-2">
        <FormItem label="Date of communication" name="communicationDate" id="communicationDate-label" htmlFor="communicationDate" required>
      ...
      <div className="margin-top-2">
        <FormItem label="How was the communication conducted? " name="method">
          <Dropdown required id="method" name="method" inputRef={register({ required: 'Select a communication method' })}>
      ...
      <div className="margin-top-2">
        ...
        >
          <Dropdown required id="purpose" name="purpose" inputRef={register({ required: 'Select a purpose of communication' })}>
            {COMMUNICATION_PURPOSES.map((option) => (
              <option key={`purposeoptions${option}`}>{option}</option>
            ))}
          </Dropdown>
        </FormItemWithDrawerTriggerLabel>
        <Drawer triggerRef={purposeDrawerRef} stickyHeader stickyFooter title="Purpose of communication">
          <ContentFromFeedByTag tagName="ttahub-commlog-purpose" />
        </Drawer>
      </div>
      <div className="margin-top-2">
        <FormItem label="Select any recipient goals that this activity supports." name="goals" required={false}>
          <MultiSelect
            control={control}
            simple={false}
            name="goals"
