...
export const MULTISELECT_VALIDATION_DICTIONARY = {
  maxSelections: (validation) => (selectedOptions) =>
    selectedOptions.length <= validation.value || validation.message,
...
const MULTISELECT_RESPONSE_COMPLETION_DICTIONARY_KEYS = Object.keys(
  ...
  (templatePrompts || []).forEach((prompt) => {
    ...
    activityRecipients.forEach((activityRecipient) => {
      const existingPrompt = promptsPerGrant.find(
        (p) => p.grantId === activityRecipient.activityRecipientId && p.promptId === prompt.promptId
      );
      if (existingPrompt) {
