...
export const MULTISELECT_VALIDATION_DICTIONARY = {
  maxSelections: (validation) => (selectedOptions) => selectedOptions.length <= validation.value || validation.message,
  minSelections: (validation) => (selectedOptions) => selectedOptions.length >= validation.value || validation.message,
}

export const MULTISELECT_RESPONSE_COMPLETION_DICTIONARY = {
  maxSelections: (validation, selectedOptions) => selectedOptions.length <= validation.value,
  minSelections: (validation, selectedOptions) => selectedOptions.length >= validation.value,
}
...
export const confirmMultiselectResponseComplete = (validations) =>
  validations.rules.reduce((acc, validation) => {
    const isValidKey = MULTISELECT_RESPONSE_COMPLETION_DICTIONARY_KEYS.includes(validation.name)
    if (!isValidKey) {
      return acc
    }

    return [...acc, (selectedOptions) => MULTISELECT_RESPONSE_COMPLETION_DICTIONARY[validation.name](validation, selectedOptions)]
  }, [])

const transformMultiselectValidationsIntoRules = (validations) =>
  validations.rules.reduce(
    (acc, validation) => {
      ...
      return {
        ...
        validate: {
          ...acc.validate,
          [validation.name]: (value) => MULTISELECT_VALIDATION_DICTIONARY[validation.name](validation)(value),
        },
      }
    },
    {
...
export const combinePrompts = (
  ...
  ;(templatePrompts || []).forEach((prompt) => {
    ...
    activityRecipients.forEach((activityRecipient) => {
      const existingPrompt = promptsPerGrant.find((p) => p.grantId === activityRecipient.activityRecipientId && p.promptId === prompt.promptId)
      if (existingPrompt) {
...
export const combineRtrPrompts = (templatePrompts, goalPrompts) =>
  uniqBy(
    [
      ...(templatePrompts || []),
