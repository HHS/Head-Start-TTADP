...
describe('condtionalFieldConstants', () => {
        ...
        [
          {
            ...
            title: 'prompt 1',
            grantId: 1,
            grantDisplayName: 'Sample Grant 1',
          },
        ],
        // Goal responses.
        [
  ...
  describe('multiselectResponseCompletion', () => {
    it('min and max selections in MULTISELECT_RESPONSE_COMPLETION_DICTIONARY return the correct values', () => {
      const minCheck = MULTISELECT_RESPONSE_COMPLETION_DICTIONARY.minSelections({ value: 2 }, [
        'a',
        'b',
      ]);
      expect(minCheck).toEqual(true);

      const minCheck2 = MULTISELECT_RESPONSE_COMPLETION_DICTIONARY.minSelections({ value: 2 }, [
        'a',
      ]);
      expect(minCheck2).toEqual(false);

      const maxCheck = MULTISELECT_RESPONSE_COMPLETION_DICTIONARY.maxSelections({ value: 1 }, [
        'a',
        'b',
      ]);
      expect(maxCheck).toEqual(false);

      const maxCheck2 = MULTISELECT_RESPONSE_COMPLETION_DICTIONARY.maxSelections({ value: 2 }, [
        'a',
        'b',
      ]);
