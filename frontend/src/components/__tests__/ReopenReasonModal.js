...
describe('Reopen Goal Reason', () => {
  const ModalComponent = ({ goalId = 1, onSubmit = () => {}, resetValues = false }) => {
    ...
    return (
      <div>
        ...
        <ReopenReasonModal goalId={goalId} modalRef={modalRef} onSubmit={onSubmit} resetValues={resetValues} error={false} />
      </div>
    )
  }
  ...
  it('correctly shows reopen radio options', async () => {
    ...
    expect(await screen.findByText(REOPEN_REASONS[GOAL_STATUS.CLOSED].ACCIDENTALLY_CLOSED)).toBeVisible()
    expect(await screen.findByText(REOPEN_REASONS[GOAL_STATUS.CLOSED].RECIPIENT_REQUEST)).toBeVisible()
    expect(await screen.findByText(REOPEN_REASONS[GOAL_STATUS.CLOSED].PS_REQUEST)).toBeVisible()
    expect(await screen.findByText(REOPEN_REASONS[GOAL_STATUS.CLOSED].NEW_RECIPIENT_STAFF_REQUEST)).toBeVisible()
