...
describe('StepIndicator', () => {
  ...
  const renderTest = (children) => render(<StepIndicator helpLink={helpLink}>{children}</StepIndicator>)

  it('renders the current step number', () => {
    const children = [<Child key="1" label="Step 1" status="current" />, <Child key="2" label="Step 2" />]
