...
describe('TextTrim', () => {
  ...
  const renderTextTrim = (text) => {
    render(
      <div data-testid="text-trim-container">
        <TextTrim text={text} />
      </div>
    );
  };
  ...
  it('renders text without tooltip when text fits container', async () => {
    ...
    expect(
      screen.getByText('Short text', { selector: 'span[style*="display: block"]' })
    ).toBeVisible();
  });

  it('renders tooltip when text is truncated', async () => {
    // Mock element widths to simulate text overflow
    jest.spyOn(HTMLElement.prototype, 'offsetWidth', 'get').mockImplementation(function mock() {
    ...
    const longText =
      'This is a very long text that should definitely get truncated in the container';
  ...
  it('updates truncation on window resize', async () => {
    ...
    jest.spyOn(HTMLElement.prototype, 'offsetWidth', 'get').mockImplementation(() => 100);
