...
export const isInternalGovernmentLink = (url) => {
  ...
  return newUrl.host.endsWith(ECLKC_GOVERNMENT_HOSTNAME_EXTENSION) || newUrl.host.endsWith(HEAD_START_GOVERNMENT_HOSTNAME_EXTENSION)
}
...
export const reportIsEditable = (status) => status === REPORT_STATUSES.DRAFT || status === REPORT_STATUSES.NEEDS_ACTION
...
export function getCollabReportStatusDisplayAndClassnames(userId, report) {
  ...
  const isCreatorOrCollaborator = (author && userId === author.id) || collaboratingSpecialists.some((specialist) => specialist.id === userId)
  ...
  if (isApprover) {
    ...
    if (userApprover.status) {
      ...
      displayStatus = 'Reviewed'
      statusClassName = `smart-hub--table-tag-status smart-hub--status-${REPORT_STATUSES.SUBMITTED}`
    } else if (calculatedStatus === REPORT_STATUSES.SUBMITTED || calculatedStatus === REPORT_STATUSES.NEEDS_ACTION) {
...
export function getStatusDisplayAndClassnames(calculatedStatus, approvers = [], justSubmitted = false) {
  let statusClassName = `smart-hub--table-tag-status smart-hub--status-${calculatedStatus}`
  let displayStatus = calculatedStatus
