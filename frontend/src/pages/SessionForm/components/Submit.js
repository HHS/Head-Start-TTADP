...
export default function Submit({ onSaveDraft, onUpdatePage, onSubmit, reviewSubmitPagePosition, pages, isPoc, isAdmin }) {
  ...
  return (
    <div data-testid="session-form-submit">
      ...
      <div className="display-flex margin-top-4">
        <Button id={`${path}-save-continue`} className="margin-right-1" type="button" onClick={onFormSubmit}>
          Submit for approval
        </Button>
        <Button id={`${path}-save-draft`} className="usa-button--outline" type="button" onClick={onSaveDraft}>
          Save draft
        </Button>
        <Button
          id={`${path}-back`}
