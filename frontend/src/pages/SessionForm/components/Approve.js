...
export default function Approve({ onFormReview }) {
  ...
  const { additionalNotes, approver, dateSubmitted, status } = getValues()
  ...
  return (
    <div data-testid="session-form-approver">
      ...
      <FormItem label="Add manager notes" name="managerNotes" required={false}>
        <HookFormRichEditor ariaLabel="Add manager notes" name="managerNotes" />
      </FormItem>
      <ReadOnlyField label="Date submitted">{dateSubmitted}</ReadOnlyField>
      <FormItem name="approvalStatus" label="Choose approval status" className="margin-bottom-4">
        <Dropdown id="approvalStatus" name="approvalStatus" inputRef={register({ required: true })}>
          <option value="" selected>
            - Select -
          </option>
          {Object.values(APPROVER_STATUSES).map((reportStatus) => (
            <option key={reportStatus} value={reportStatus}>
              {startCase(reportStatus)}
            </option>
          ))}
        </Dropdown>
      </FormItem>
      ...
      <div className="display-flex margin-top-4">
        <Button id={`${path}-save-continue`} className="margin-right-1" type="button" onClick={onFormReview}>
          Submit{' '}
        </Button>
      </div>
    </div>
