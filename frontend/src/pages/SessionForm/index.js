...
import { defaultValues, baseDefaultValues, istKeys, pocKeys } from './constants'
...
export default function SessionForm({ match }) {
  ...
  const { isPoc, isAdminUser, isCollaborator, isOwner, isApprover, applicationPages, isSessionNavigationDead } = useSessionFormRoleAndPages(hookForm)
    ...
    async function fetchSession() {
      ...
      try {
        ...
        const isRegionalEventPoc =
          eventOrganizerFromSession === TRAINING_EVENT_ORGANIZER.REGIONAL_TTA_NO_NATIONAL_CENTERS && isPocFromSession && !isAdminUser
        // eslint-disable-next-line max-len
  ...
  if (redirectPagePath && (!currentPage || !pageExists)) {
    return <Redirect to={`/training-report/${trainingReportId}/session/${reportId.current}/${redirectPagePath}`} />
  }
  ...
  const removeCompleteDataBaseOnRole = (roleData) => {
    ...
    if (!isAdminUser) {
      ...
      } else if (isPoc && eventOrganizer === TRAINING_EVENT_ORGANIZER.REGIONAL_PD_WITH_NATIONAL_CENTERS) {
    ...
    if (formData.status !== TRAINING_REPORT_STATUSES.COMPLETE) {
      try {
        ...
        if (!isPoc && roleData.objectiveResources) {
          roleData = {
            ...roleData,
            objectiveResources: data.objectiveResources.filter((r) => r && isValidResourceUrl(r.value)),
          }
        }
      ...
      <Grid row className="flex-justify">
        <Grid col="auto">
          <div className="margin-top-2 margin-bottom-4">
            <h1 className="font-serif-2xl text-bold line-height-serif-2 margin-0 margin-bottom-1">Training report - Session</h1>
