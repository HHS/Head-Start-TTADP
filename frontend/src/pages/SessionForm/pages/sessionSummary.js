...
import { useFormContext, Controller, useFieldArray, useController } from 'react-hook-form'
import { TextInput, Fieldset, Label, Textarea, Dropdown, Radio, Button, ErrorMessage, Link } from '@trussworks/react-uswds'
...
import { sessionSummaryFields, pageComplete, NO_ERROR, sessionSummaryRequiredFields } from '../constants'
    ...
    <>
      ...
      <div>
        <FormItem label="Session name " name="sessionName" htmlFor="sessionName" required>
          <TextInput id="sessionName" name="sessionName" type="text" required inputRef={register({ required: 'Enter session name' })} />
        </FormItem>
      </div>

      <div className="maxw-mobile">
        <FormItem label="Session start date" name="startDate" id="startDate-label" htmlFor="startDate" required>
        ...
        <FormItem label="Session end date" name="endDate" id="endDate-label" htmlFor="endDate" required>
          <div className="usa-hint">mm/dd/yyyy</div>
          <ControlledDatePicker
            control={control}
            name="endDate"
        ...
        <FormItem
          ...
          label={
            <>
              ...
              <button type="button" className="usa-button usa-button--unstyled usa-button--no-margin" ref={goalDrawerTriggerRef}>
      ...
      <div>
        <Drawer triggerRef={topicsDrawerTriggerRef} stickyHeader stickyFooter title="Topic guidance">
        ...
        <FormItem
          ...
          label={
            <>
              Topics <Req />
              <button type="button" className="usa-button usa-button--unstyled margin-left-1 usa-button--no-margin" ref={topicsDrawerTriggerRef}>
      ...
      <Fieldset className="ttahub-objective-files">
        ...
        <Radio label="Yes" value="yes" id="addObjectiveFilesYes" checked={useFiles} name="addObjectiveFiles" onChange={() => setUseFiles(true)} />
        <Radio label="No" value="no" id="addObjectiveFilesNo" name="addObjectiveFiles" checked={!useFiles} onChange={() => setUseFiles(false)} />
        ...
        {useFiles ? (
          <>
            ...
            <Dropzone handleDrop={handleDrop} inputName="objectiveFiles" setErrorMessage={setFileUploadErrorMessage} />
            <FileTable onFileRemoved={deleteFile} files={files.map((f) => ({ ...f, showDelete: true }))} />
          </>
        ) : null}
      </Fieldset>
