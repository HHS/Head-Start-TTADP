...
import { Alert, Grid, Form, Button, ModalToggleButton } from '@trussworks/react-uswds'
import { useHistory } from 'react-router-dom'
import { FormProvider, useForm } from 'react-hook-form'
import useLocalStorage from '../../hooks/useLocalStorage'
import { LOCAL_STORAGE_ADDITIONAL_DATA_KEY, defaultValues } from './constants'
...
export default function TrainingReportForm({ match }) {
  ...
  const [additionalData, updateAdditionalData, localStorageAvailable] = useLocalStorage(LOCAL_STORAGE_ADDITIONAL_DATA_KEY(trainingReportId), {
    users: {
      pointOfContact: [],
      collaborators: [],
    },
  ...
  const onSave = async () => {
    try {
      ...
      const { ownerId, pocIds, collaboratorIds, regionId, sessionReports, ...data } = hookForm.getValues()
      ...
      <Grid row className="flex-justify">
        <Grid col="auto">
          <div className="margin-top-2 margin-bottom-4">
            <h1 className="font-serif-2xl text-bold line-height-serif-2 margin-0 margin-bottom-1">Training report - Event</h1>
