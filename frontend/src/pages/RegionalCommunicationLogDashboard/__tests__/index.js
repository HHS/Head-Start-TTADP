...
describe('RegionalCommunicationLogDashboard', () => {
  ...
  it('has the communication date filter applied by default', async () => {
    act(() => renderComm(userCentralOffice, '/communication-log'))
    await waitFor(() =>
      expect(screen.getByRole('button', { name: /this button removes the filter: communication date is within/i })).toBeInTheDocument()
    )
  })
  ...
  it('allows you to remove a filter', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: /this button removes the filter: communication date is/i })).not.toBeInTheDocument()
    )
  })
