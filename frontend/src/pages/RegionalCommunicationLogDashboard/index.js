...
import { canCreateCommunicationLog, getUserRegions } from '../../permissions';
...
export default function RegionalCommunicationLog() {
  ...
  const defaultRegion = useMemo(() => String(user?.homeRegionId || 0), [user?.homeRegionId]);
  ...
  const allRegionsFilters = useMemo(
    () => (userHasOnlyOneRegion ? [] : buildDefaultRegionFilters(regions)),
    [regions, userHasOnlyOneRegion]
  );
    ...
    <div className="ttahub-dashboard">
      ...
      <div className="comm-log-header flex-align-center margin-top-0 margin-bottom-3">
        <h1 className="landing">
          Communication logs - {userHasOnlyOneRegion ? 'your region' : 'your regions'}
        </h1>
        {canCreateCommLog && (
          <div>
