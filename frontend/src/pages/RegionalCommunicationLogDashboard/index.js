...
const createDefaultFilters = (hasCentralOffice, userHasOnlyOneRegion, defaultRegion, allRegionsFilters) => {
  const dateFilter = {
    id: uuidv4(),
...
export default function RegionalCommunicationLog() {
  ...
  const hasCentralOffice = useMemo(() => user && user.homeRegionId && user.homeRegionId === 14, [user])
  ...
  const canCreateCommLog = useMemo(() => canCreateCommunicationLog(user, parseInt(defaultRegion, DECIMAL_BASE)), [user, defaultRegion])
  ...
  const defaultFilters = useMemo(
    () => createDefaultFilters(hasCentralOffice, userHasOnlyOneRegion, defaultRegion, allRegionsFilters),
    [hasCentralOffice, userHasOnlyOneRegion, defaultRegion, allRegionsFilters]
  )

  const [filtersToApply, setFiltersInHook] = useSessionFiltersAndReflectInUrl(FILTER_KEY, defaultFilters)
  ...
  return (
    <div className="ttahub-dashboard">
      ...
      <RegionPermissionModal
        ...
        user={user}
        showFilterWithMyRegions={() => showFilterWithMyRegions(allRegionsFilters, filtersToApply, setFiltersInHook)}
      />
      <div className="comm-log-header flex-align-center margin-top-0 margin-bottom-3">
        <h1 className="landing">Communication logs - {userHasOnlyOneRegion ? 'your region' : 'your regions'}</h1>
        {canCreateCommLog && (
          <div>
            <NewReportButton to={`/communication-log/region/${defaultRegion}/log/new`}>Add communication</NewReportButton>
          </div>
        )}
      </div>
