...
describe('Approved Activity Report V2 component', () => {
    ...
    render(
      <ApprovedReportV2
        data={{
          ...report,
          goalsAndObjectives: [],
          objectivesWithoutGoals: mockObjectives,
  ...
  it('correctly displays recipient next steps', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: /specialist's next steps/i })
    ).toBeInTheDocument();
    expect(await screen.findByText(/First Step/i)).toBeInTheDocument();
    expect(await screen.findByText(/Second Step/i)).toBeInTheDocument();
    expect(
      await screen.findByRole('heading', { name: /recipient's next steps/i })
    ).toBeInTheDocument();
    expect(await screen.findByText(/Third Step/i)).toBeInTheDocument();
  ...
  it('correctly displays other-entity next steps', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: /other entities next steps/i })
    ).toBeInTheDocument();
    expect(
      await screen.findByRole('heading', { name: /specialist's next steps/i })
    ).toBeInTheDocument();
  ...
  it('correctly displays objectives with citations', async () => {
    ...
    expect(await screen.findByTestId('review-citation-label')).toHaveTextContent(
      'R1 - GRANT2 - EHS'
    );
