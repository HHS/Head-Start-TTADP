...
export const determineDefaultSort = (userHasCentralOffice) => (userHasCentralOffice ? DEFAULT_CENTRAL_OFFICE_SORT : DEFAULT_SORT)
...
function RecipientSearch({ user }) {
  ...
  useEffect(() => {
    ...
    async function fetchRecipients() {
      ...
      try {
        const response = await searchRecipients(query, expandFilters(filters), { ...sortConfig, offset })
        setResults(response)
      } catch (err) {
    ...
    fetchRecipients()
  }, [offset, sortConfig, user, queryAndSort, query, setQueryAndSort, filters, defaultSort, setIsAppLoading])
        ...
        <Grid className="ttahub-recipient-search--filter-row flex-fill display-flex flex-align-center flex-align-self-center flex-row flex-wrap margin-bottom-3">
          <form role="search" className="ttahub-recipient-search--search-form display-flex" onSubmit={onSubmit}>
            ...
            <button type="submit" className="ttahub-recipient-search--submit-button usa-button">
              <FontAwesomeIcon color="white" icon={faSearch} /> <span className="usa-sr-only">Search for matching recipients</span>
            </button>
          </form>
        </Grid>
