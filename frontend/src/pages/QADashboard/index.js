...
const DISALLOWED_FILTERS = ['domainClassroomOrganization', 'domainEmotionalSupport', 'domainInstructionalSupport']

const ALLOWED_SUBFILTERS = QA_DASHBOARD_FILTER_CONFIG.map(({ id }) => id).filter((id) => !DISALLOWED_FILTERS.includes(id))
...
export default function QADashboard() {
  ...
  const {
    ...
    onApplyFilters,
    onRemoveFilter,
  } = useFilters(user, QA_DASHBOARD_FILTER_KEY, true, additionalDefaultFilters, QA_DASHBOARD_FILTER_CONFIG)
    ...
    async function fetchQaData() {
      ...
      try {
        const [recipientsWithNoTtaData, feiData, classData, dashboardData] = await Promise.all([
          ...
          getSelfServiceData('recipients-with-ohs-standard-fei-goal', filters, ['with_fei_widget', 'with_fei_graph']),
          getSelfServiceData('recipients-with-class-scores-and-goals', filters, ['with_class_widget']),
          getSelfServiceData('qa-dashboard', filters, ['delivery_method_graph', 'role_graph', 'activity_widget']),
        ])
        ...
        const overviewData = {
          recipientsWithNoTTA: {
            ...
            pct: noTTAData.data.length > 0 && noTTAData.data[0]['% recipients without tta'] ? noTTAData.data[0]['% recipients without tta'] : '0',
          },
          ...
          recipientsWithOhsStandardClass: {
            ...
            pct:
              classOverviewData.data && classOverviewData.data.length > 0 && classOverviewData.data[0]['% recipients with class']
        ...
        const filteredReports = activityWidgetData.data.length ? activityWidgetData.data[0].filtered_reports : 0
        ...
        const roleGraph = {
          showDashboardFiltersNotApplicable,
          filteredReports,
          records: roleGraphData.data ? roleGraphData.data.sort((a, b) => a.role_name.localeCompare(b.role_name)) : [],
        }
    ...
    <>
      ...
      <div className="ttahub-qa-dashboard">
        ...
        <div className="margin-bottom-3">
          <DrawerTriggerButton drawerTriggerRef={drawerTriggerRef}>Learn how filters impact the data displayed</DrawerTriggerButton>
