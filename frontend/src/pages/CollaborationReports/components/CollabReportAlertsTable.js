...
const CollabReportAlertsTable = ({ emptyMsg, loading, offset, showCreateMsgOnEmpty, title, data, requestSort, sortConfig }) => {
  const {
    user: { id: userId },
    ...
    () =>
      data.rows.map((r) => ({
        ...
        data: [
          ...
          {
            value: (() => {
              const { displayStatus, statusClassName } = getCollabReportStatusDisplayAndClassnames(userId, r)
      ...
      >
        ...
        {data.rows.length > 0 && (
          <HorizontalTableWidget
            headers={['Activity name', 'Date started', 'Creator', 'Created date', 'Collaborators', 'Approvers', 'Status']}
            data={tabularData}
