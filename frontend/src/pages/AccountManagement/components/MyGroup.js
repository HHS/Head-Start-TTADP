  ...
  return (
    <tr key={group.id}>
      ...
      <td className={isViewOnly ? 'smart-hub--groups-view' : 'smart-hub--groups-edit-delete'} align="right">
        {isViewOnly ? (
          <Link to={`/account/group/${group.id}`} aria-label={`view ${group.name}`} className="usa-button usa-button--unstyled">
            View group
          </Link>
        ) : (
          <>
            ...
            <ModalToggleButton disabled={isAppLoading} opener aria-label={`delete ${group.name}`} modalRef={modalRef} unstyled>
              Delete group
            </ModalToggleButton>
          </>
        )}
