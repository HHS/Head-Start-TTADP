...
describe('ViewGoalDetails', () => {
  ...
  test('monitoring goal: Not Started due to Active monitoring citations shows "Added on ... by OHS"', async () => {
    const monitoringGoal = {
      id: 8,
    ...
    fetchMock.get('/api/goals/8/history', [monitoringGoal]);
    await act(async () => {
      renderViewGoalDetails(DEFAULT_USER, '?goalId=8');
    });

    const accordionButton = await screen.findByRole('button', { name: /G-8 \| Not Started/i });
    await waitFor(() => expect(accordionButton).toHaveAttribute('aria-expanded', 'true'));
    const accordionContent = document.getElementById(accordionButton.getAttribute('aria-controls'));
    const updates = within(accordionContent).getAllByRole('listitem');

    expect(updates).toHaveLength(1);
    expect(updates[0]).toHaveTextContent(`Added on ${formatDate('2025-03-01T00:00:00.000Z')} by OHS`);
  });

  test('monitoring goal: Closed due to No active monitoring citations shows "Closed on ... by OHS"', async () => {
    const monitoringGoal = {
      id: 9,
      name: 'Monitoring Goal Close',
      status: 'Closed',
      createdAt: '2025-03-01T00:00:00.000Z',
      standard: 'Monitoring',
      goalTemplate: null,
    ...
    fetchMock.get('/api/goals/9/history', [monitoringGoal]);
    await act(async () => {
      renderViewGoalDetails(DEFAULT_USER, '?goalId=9');
    });

    const accordionButton = await screen.findByRole('button', { name: /G-9 \| Closed/i });
    await waitFor(() => expect(accordionButton).toHaveAttribute('aria-expanded', 'true'));
    const accordionContent = document.getElementById(accordionButton.getAttribute('aria-controls'));
    const updates = within(accordionContent).getAllByRole('listitem');
    ...
    expect(updates[0]).toHaveTextContent(`Added on ${formatDate('2025-03-01T00:00:00.000Z')} by OHS`);
    expect(updates[1]).toHaveTextContent(`Closed on ${formatDate('2025-03-15T00:00:00.000Z')} by OHS`);
  });

  test('monitoring goal: Not Started after a prior Closed shows "Reopened on" and by OHS', async () => {
    const monitoringGoal = {
      id: 10,
      name: 'Monitoring Goal Reopen',
      status: 'Not Started',
      createdAt: '2025-03-01T00:00:00.000Z',
      standard: 'Monitoring',
      goalTemplate: null,
    ...
    fetchMock.get('/api/goals/10/history', [monitoringGoal]);
    await act(async () => {
      renderViewGoalDetails(DEFAULT_USER, '?goalId=10');
    });

    const accordionButton = await screen.findByRole('button', { name: /G-10 \| Not Started/i });
    await waitFor(() => expect(accordionButton).toHaveAttribute('aria-expanded', 'true'));
    const accordionContent = document.getElementById(accordionButton.getAttribute('aria-controls'));
    const updates = within(accordionContent).getAllByRole('listitem');

    // Expect three updates, with the last showing Reopened on ... by OHS
    expect(updates).toHaveLength(3);
    expect(updates[0]).toHaveTextContent(`Added on ${formatDate('2025-03-01T00:00:00.000Z')} by OHS`);
    expect(updates[1]).toHaveTextContent(`Closed on ${formatDate('2025-03-10T00:00:00.000Z')} by OHS`);
    expect(updates[2]).toHaveTextContent(`Reopened on ${formatDate('2025-03-20T00:00:00.000Z')} by OHS`);
  });

  test('deduplicates identical status updates (same time and statuses)', async () => {
    const goalWithDuplicates = {
      id: 11,
      name: 'Goal with Duplicates',
      status: 'In Progress',
      ...
      objectives: [],
      statusChanges: [
        // Two identical Not Started entries at the same time
        {
          id: 1100,
          goalId: 11,
          oldStatus: null,
        ...
        // Two identical In Progress entries at the same time
        {
          id: 1102,
          goalId: 11,
    ...
    fetchMock.get('/api/goals/11/history', [goalWithDuplicates]);
    await act(async () => {
    ...
    // Expect only two updates after deduplication
    expect(updates).toHaveLength(2);
    expect(updates[0]).toHaveTextContent(`Added on ${formatDate('2025-04-01T00:00:00.000Z')} by A, Role`);
    expect(updates[1]).toHaveTextContent(`Started on ${formatDate('2025-04-02T00:00:00.000Z')} by B, Role`);
  });
});
