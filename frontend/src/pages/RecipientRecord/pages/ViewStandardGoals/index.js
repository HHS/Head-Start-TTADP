...
export default function ViewGoalDetails({ recipient, regionId }) {
      ...
      content: (
        <div className="goal-history-content">
          <div className="print-only margin-bottom-2">
            <h2 className="margin-top-0 margin-bottom-0 smart-hub-serif goal-history-print-goal-number">
              Goal number
            </h2>
            ...
            <div className="objective-details-section margin-bottom-3">
              {objectives.map((objective) => (
                <div key={objective.id} className="margin-bottom-3">
                  ...
                  <div className="margin-top-2">
                    <ReadOnlyField label="Objective status">{objective.status}</ReadOnlyField>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <>
      ...
      <Container
        className="margin-y-3 margin-left-2 maxw-desktop view-standard-goals-print-container"
        paddingX={4}
        paddingY={5}
      >
        <div className="margin-bottom-3 goal-summary-block">
          <div className="display-flex flex-justify margin-bottom-3 goal-summary-heading-row">
            ...
            <div className="no-print">
              <ContextMenu label={`Actions for goal ${goalId}`} menuItems={menuItems} />
            </div>
          </div>
        ...
        <div className="goal-history-print-overview print-only margin-bottom-4">
          ...
          <p className="margin-y-0">
            <strong>{overview?.activityReports ?? 0}</strong> Activity Reports
          </p>
          <p className="margin-y-0">
            <strong>{overview?.objectives ?? 0}</strong> goal objectives
          </p>
          <p className="margin-y-0">
            <strong>{overview?.closures ?? 0}</strong> goal closures
          </p>
          <p className="margin-y-0">
            <strong>{overview?.suspensions ?? 0}</strong> goal suspensions
          </p>
        </div>
