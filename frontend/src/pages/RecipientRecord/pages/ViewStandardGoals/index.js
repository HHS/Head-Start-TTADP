...
export const GoalUserIdentifier = ({ goal }) => {
  ...
  return goal.goalCollaborators && goal.goalCollaborators.some((c) => c.collaboratorType && c.collaboratorType.name === 'Creator' && c.user)
...
export default function ViewGoalDetails({ recipient, regionId }) {
  ...
  const canView = user.permissions.filter((permission) => permission.regionId === parseInt(regionId, DECIMAL_BASE)).length > 0
  ...
  const sortedGoalHistory = [...goalHistory].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
  ...
  const accordionItems = sortedGoalHistory.map((goal, index) => {
    ...
    // a bit more readable
    const statusUpdates = (
      goal.statusChanges && goal.statusChanges.length > 0 ? goal.statusChanges.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)) : []
    ).map((gsc) => ({
      ...gsc,
    ...
    const dedupedStatusUpdates = statusUpdates.reduce((acc, curr) => {
      if (!acc.find((gsc) => gsc.performedAt === curr.performedAt && gsc.newStatus === curr.newStatus && gsc.oldStatus === curr.oldStatus)) {
        acc.push(curr)
      }
      return acc
    }, [])
    ...
    const hasAddedUpdate = dedupedStatusUpdates.some((u) => u.oldStatus === null || u.newStatus === GOAL_STATUS.NOT_STARTED)
              ...
              {statusUpdates.length > 0 ? (
                <ul className="usa-list" aria-label="Goal status updates">
                  {displayUpdates.map((update, updateIndex) => (
                    <li key={update.id}>
                      <strong>
                        <StatusActionTag update={update} goalHistory={sortedGoalHistory} currentGoalIndex={index} />
                      </strong>{' '}
                      <strong>{update.performedAt}</strong>
                      {getUserByFromStatus(update)}
                      {update.newStatus === GOAL_STATUS.SUSPENDED && updateIndex !== displayUpdates.length - 1 ? (
                        <>
                          <br />
                ...
                <ul className="usa-list" aria-label="Goal status updates">
                  <li>
                    <strong>{goal.status === GOAL_STATUS.NOT_STARTED ? 'Added' : `${goal.status}`}</strong> on{' '}
          ...
          {goal.responses && goal.responses.length > 0 && (
            <ReadOnlyField label="Root causes">
              {goal.responses.map((response) => (
                <div key={response.id}>{Array.isArray(response.response) ? response.response.join(', ') : <p>{response.response}</p>}</div>
              ))}
            </ReadOnlyField>
          )}
            ...
            <div className="objective-details-section margin-bottom-3">
              {objectives.map((objective) => (
                <div key={objective.id} className="margin-bottom-3">
                        ...
                        {objective.activityReportObjectives
                          ...
                          .map((aro, reportIndex, array) => (
                            <React.Fragment key={`report-${aro.activityReport.id}`}>
                              <Link to={`/activity-reports/view/${aro.activityReport.id}`}>{aro.activityReport.displayId}</Link>
                  ...
                  {/* Display Topics */}
                  {!objective.activityReportObjectives ||
                  !objective.activityReportObjectives.some((aro) => aro.topics && aro.topics.length > 0) ? null : (
                    <div className="margin-top-2">
                      <ReadOnlyField label="Topics">
                        {(objective.activityReportObjectives || [])
                          .flatMap((aro) => aro.topics || [])
                          .filter((topic, i, self) => i === self.findIndex((t) => t.id === topic.id))
                  ...
                  {/* Check if there are any resources before rendering the section */}
                  {
                    ...
                    ((objective.activityReportObjectives || [])
                      ...
                      .filter(
                        (courseObj, i, self) => courseObj.course && i === self.findIndex((c) => c.course && c.course.id === courseObj.course.id)
                      ).length > 0 ||
                      // Check for resources/links
                      (objective.activityReportObjectives || []).some((aro) => aro.resources && aro.resources.length > 0) ||
                      // Check for files
                      (objective.activityReportObjectives || []).some((aro) => aro.files && aro.files.length > 0)) && (
                      <>
                        ...
                        <div className="resource-sections-container">
                          ...
                          {(objective.activityReportObjectives || [])
                            ...
                            <ul className="usa-list margin-top-0 margin-bottom-0 resource-link-wrapper">
                              {objective.activityReportObjectives
                                ...
                                .filter(
                                  (courseObj, i, self) =>
                                    courseObj.course && i === self.findIndex((c) => c.course && c.course.id === courseObj.course.id)
                                )
                                .map((courseObj) => (
                                  <li key={`course-${courseObj.course.id}`}>{courseObj.course.name}</li>
                                ))}
                            </ul>
                          )}
                          ...
                          {/* Display Resources */}
                          {!objective.activityReportObjectives ||
                          !objective.activityReportObjectives.some((aro) => aro.resources && aro.resources.length > 0) ? null : (
                            <ul className="usa-list margin-top-0 resource-link-wrapper">
                              {(objective.activityReportObjectives || [])
                                .flatMap((aro) => aro.resources || [])
                                .filter((resource, i, self) => i === self.findIndex((r) => r.id === resource.id))
                                .map((resource) => (
                          ...
                          {!objective.activityReportObjectives ||
                          !objective.activityReportObjectives.some((aro) => aro.files && aro.files.length > 0) ? null : (
                            <ul className="usa-list margin-top-0 resource-link-wrapper">
                              {(objective.activityReportObjectives || [])
                                .flatMap((aro) => aro.files || [])
                                .filter((file, i, self) => i === self.findIndex((f) => f.id === file.id))
                                .map((file) => (
    ...
    <>
      ...
      <Container className="margin-y-3 margin-left-2 width-tablet" paddingX={4} paddingY={5}>
        <div className="margin-bottom-5">
          ...
          <ReadOnlyField label="Recipient grant numbers">{firstGoal.grant && firstGoal.grant.number ? firstGoal.grant.number : 'N/A'}</ReadOnlyField>
          <ReadOnlyField label="Recipient's goal">{firstGoal.name || goalTemplateName}</ReadOnlyField>
        </div>
