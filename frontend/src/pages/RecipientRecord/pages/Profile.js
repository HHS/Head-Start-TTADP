        ...
        <Grid row gap={4}>
          ...
          {(recipientSummary.grants || []).map((grant) => (
            <React.Fragment key={grant.number}>
              ...
              <Grid desktop={{ col: 6 }} tabletLg={{ col: 12 }} hidden={!hasClassData(grant.number)}>
                <div>
                  <ClassReview grantNumber={grant.number} regionId={regionId} recipientId={recipientId} />
                </div>
              </Grid>

              <Grid desktop={{ col: 6 }} tabletLg={{ col: 12 }} hidden={!hasMonitoringData(grant.number)}>
                <div>
                  <MonitoringReview grantNumber={grant.number} regionId={regionId} recipientId={recipientId} />
                </div>
              </Grid>
            </React.Fragment>
          ))}
