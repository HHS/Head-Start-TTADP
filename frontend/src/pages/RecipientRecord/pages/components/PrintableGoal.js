...
export default function PrintableGoal({ goal }) {
  ...
  const allTopics =
    goal.objectives && Array.isArray(goal.objectives) ? goal.objectives.flatMap((o) => (o.topics && Array.isArray(o.topics) ? o.topics : [])) : []
  const uniqueTopics = [...new Set(allTopics)]

  return (
    <div className="ttahub-printable-goal padding-x-3 padding-top-3 padding-bottom-2 margin-top-5 no-break-within">
      <h2 className="margin-top-0 padding-bottom-1 border-bottom-1px">Goal {goal.goalNumbers ? goal.goalNumbers.join(', ') : `G-${goal.id}`}</h2>
      ...
      {goal.objectives.length > 0 ? <h3 className="padding-x-1">Objectives for goal {`G-${goal.id}`}</h3> : null}
      {goal.objectives.map((objective) => (
        <PrintableObjective key={objective.id} objective={objective} />
      ))}
    </div>
  )
}
