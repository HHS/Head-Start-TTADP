  ...
  return (
    <div className="ttahub-review-card--finding-within-review margin-top-4">
      <DescriptionList className="ttahub-review-card--finding-within-review-description-list">
        ...
        <DescriptionItem title="Finding type">{finding.findingType}</DescriptionItem>
        <DescriptionItem
          title="Category"
          className="ttahub-review-card--finding-within-review-category"
        >
          {finding.category}
        </DescriptionItem>
      </DescriptionList>
      {finding.objectives.length > 0 ? (
        finding.objectives.map((objective) => (
          <ReviewObjective
            key={`review-objective-${objective.title}`}
            objective={objective}
            regionId={regionId}
          />
        ))
