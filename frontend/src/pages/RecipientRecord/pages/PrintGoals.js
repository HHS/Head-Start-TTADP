...
export default function PrintGoals({ location, recipientId, regionId }) {
  ...
  useEffect(() => {
    ...
    const goalIds = location.state && location.state.selectedGoalIds ? location.state.selectedGoalIds : []
    async function fetchGoals(query) {
      setLoading(true)
      try {
        const { goalRows } = await getRecipientGoals(recipientId, regionId, sortConfig.sortBy, sortConfig.direction, OFFSET, false, query, goalIds)
        setGoals(goalRows)
        setError('')
      } catch (e) {
