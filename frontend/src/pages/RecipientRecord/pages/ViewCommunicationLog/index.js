  ...
  return (
    <>
      ...
      <div className="padding-y-3">
        <BackLink to={`/recipient-tta-records/${recipientId}/region/${regionId}/communication`}>Back to communication log</BackLink>
        <h1 className="landing">{recipientName}</h1>
        <LogLine authorName={log.author.name} communicationDate={log.data.communicationDate} duration={log.data.duration} method={log.data.method} />
        <Container paddingX={4} paddingY={2} className="maxw-tablet-lg" positionRelative>
          {isAuthor && (
            <Link className="position-absolute top-0 right-0 margin-top-4 margin-right-4" to={editLink}>
              Edit
            </Link>
          )}
          <ReadOnlyField label="Other TTA staff">
            {log.data.otherStaff && log.data.otherStaff.map((u) => <div key={u.value}>{u.label}</div>)}
          </ReadOnlyField>
          <ReadOnlyField label="Purpose">{log.data.purpose}</ReadOnlyField>
          <ReadOnlyField label="Supporting goals">{log.data.goals && log.data.goals.map((goal) => goal.label).join(', ')}</ReadOnlyField>
          {hasRichTextContent(log.data.notes) && <ReadOnlyField label="Notes">{parse(log.data.notes)}</ReadOnlyField>}
          ...
          <p className="text-bold font-sans-3xs base-dark">Date of entry: {moment(log.createdAt).format('MMM Do, YYYY')}</p>
        </Container>
      </div>
    </>
  )
