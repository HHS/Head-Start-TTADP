...
export default function GrantsList({ summary }) {
  ...
  const grantsData = grants.map((grant) => ({
    ...
    programs: grant.programs
      ? getDistinctSortedArray(grant.programs.map((program) => program.programType)).join(', ')
      : '',
  ...
  return (
    <Container
      className="ttahub-recipient-record--grants-list ttahub-recipient-record--profile-table"
      paddingX={0}
      paddingY={0}
    >
