  ...
  try {
    ...
    if (event && event.data && event.data.owner) {
      if (event.eventReportPilotNationalCenterUsers) {
        const user = event.eventReportPilotNationalCenterUsers.find((erpnc) => erpnc.userId === event.data.owner.id)
...
export default function TrainingReportV1({ event, eventCollaborators, eventPoc, alertMessage }) {
  ...
  const generateIstOfficeOrRecipientProperties = (session) => {
    ...
    const recipientsList = session.data.recipients ? session.data.recipients.map((r) => r.label).join(', ') : ''
    const participantsList = session.data.participants ? session.data.participants.join(', ') : ''
  ...
  const generateNumberOfParticipants = (session) => {
    ...
    if (session.data.deliveryMethod === 'in-person' || session.data.deliveryMethod === 'virtual') {
      const numberOfParticipants = session.data.numberOfParticipants ? session.data.numberOfParticipants.toString() : NONE_PROVIDED
      return {
        'Number of participants': numberOfParticipants,
      }
    }
    // Hybrid.
    const numberOfParticipantsInPerson = session.data.numberOfParticipantsInPerson
      ? session.data.numberOfParticipantsInPerson.toString()
      : NONE_PROVIDED
    const numberOfParticipantsVirtually = session.data.numberOfParticipantsVirtually
            ...
            sections={[
              ...
              },
              {
                ...
                data: {
                  ...
                  'Resource links': valueOrDefault(session.data.objectiveResources ? session.data.objectiveResources.map((o) => o.value) : []),
                  'iPD Courses': valueOrDefault(session.data.courses ? session.data.courses.map((o) => o.name) : []),
                  'Resource attachments': valueOrDefault(session.data.files ? session.data.files.map((f) => f.originalFileName) : []),
                  'Support type': valueOrDefault(session.data.objectiveSupportType),
                },
              },
              {
                ...
                data: {
                  ...
                  'Delivery method': valueOrDefault(capitalize(session.data.deliveryMethod || '')),
                  ...generateNumberOfParticipants(session),
                  'Language used': valueOrDefault(session.data.language ? session.data.language.join(', ') : ''),
