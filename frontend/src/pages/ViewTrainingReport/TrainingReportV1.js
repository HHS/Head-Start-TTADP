  ...
  try {
    ...
    if (event && event.data && event.data.owner) {
      if (event.eventReportPilotNationalCenterUsers) {
        const user = event.eventReportPilotNationalCenterUsers.find(
          (erpnc) => erpnc.userId === event.data.owner.id
        );
...
export default function TrainingReportV1({ event, eventCollaborators, eventPoc, alertMessage }) {
  ...
  const isIstVisit = (session) => {
    if (
      session.data.isIstVisit === 'yes' ||
      (session.data.regionalOfficeTta && session.data.regionalOfficeTta.length > 0)
    ) {
      return true;
    }
    return false;
              ...
              },
              {
                ...
                data: {
                  ...
                  'Resource attachments': valueOrDefault(
                    session.data.files ? session.data.files.map((f) => f.originalFileName) : []
                  ),
              ...
              },
              {
                ...
                data: {
                  ...
                  'Language used': valueOrDefault(
                    session.data.language ? session.data.language.join(', ') : ''
                  ),
                  'TTA provided': valueOrDefault(session.data.ttaProvided),
                },
              },
              formatNextSteps(
                session.data.specialistNextSteps || [],
                "Specialist's next steps",
                false
              ),
              formatNextSteps(
                session.data.recipientNextSteps || [],
