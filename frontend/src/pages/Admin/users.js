...
import { Label, TextInput, Grid, SideNav, Alert, Radio, Fieldset } from '@trussworks/react-uswds'
...
function Admin(props) {
  ...
  const permissionsIncludesAccess = (permissions) => _.some(permissions, (perm) => perm.scopeId === SCOPE_IDS.SITE_ACCESS)
  ...
  return (
    <>
      ...
      <Container>
        <button type="button" className="usa-button usa-button--outline ttahub-export-reports" onClick={handleActiveUsersDownload}>
          Download users
        </button>
        ...
        <Grid row gap>
          <Grid col={4}>
            ...
            <select className="usa-select" id="feature-flag-filter-users" value={selectedFeature} onChange={onUpdateSelectedFeature}>
              {features ? (
                [{ label: 'No filter', value: 'none' }, { label: 'All Users with a Feature Flag', value: 'all' }, ...features].map((f) => (
          ...
          <Grid col={8}>
            ...
            {selectedUser && <UserSection onSave={onSave} user={selectedUser} features={features} />}
          </Grid>
        </Grid>
      </Container>
    </>
