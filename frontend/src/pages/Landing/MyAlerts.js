...
function ReportRow({ report, index, length, updateIdToDelete, modalRef }) {
  ...
  const { id, displayId, activityRecipients, startDate, calculatedStatus, approvers, createdAt, creatorName, activityReportCollaborators } = report

  const recipients = activityRecipients.map((ar) => (ar.grant ? ar.grant.recipient.name : ar.otherEntity.name))

  const collaboratorNames = uniq(activityReportCollaborators ? activityReportCollaborators.map((collaborator) => collaborator.fullName) : [])
  ...
  const { displayStatus, statusClassName } = getStatusDisplayAndClassnames(calculatedStatus, approvers, message && Number(message.reportId) === id)
  ...
  return (
    <tr key={idKey}>
      ...
      <td>
        <ContextMenu label={contextMenuLabel} menuItems={menuItems} menuWidthOffset={110} menuHeightOffset={80} up={index + 1 === length} fixed />
      </td>
    </tr>
  )
}
...
ReportRow.propTypes = {
  ...
  modalRef: PropTypes.oneOfType([PropTypes.func, PropTypes.shape({ current: PropTypes.any })]).isRequired,
}
...
export function ReportsRow({ reports, removeAlert }) {
  ...
  const tableRows = reports.map((report, index, { length }) => (
    <ReportRow key={report.id} report={report} index={index} length={length} updateIdToDelete={updateIdToDelete} modalRef={modalRef} />
  ))
      ...
      {reports && reports.length === 0 && (
        <Container className="landing" paddingX={0} paddingY={0} loading={loading}>
          <div className="text-center padding-10">
            ...
            {newBtn && <p className="padding-bottom-2">Would you like to begin a new activity report?</p>}
            {newBtn && (
        ...
        >
          ...
          <div className="usa-table-container--scrollable">
            <Table fullWidth striped>
              <caption className="smart-hub--table-caption usa-sr-only">My activity report alerts with sorting</caption>
              <thead>
                <tr>
...
MyAlerts.propTypes = {
  ...
  downloadAllAlertsButtonRef: PropTypes.oneOfType([PropTypes.func, PropTypes.shape({ current: PropTypes.instanceOf(Element) })]),
  downloadSelectedAlertsButtonRef: PropTypes.oneOfType([PropTypes.func, PropTypes.shape({ current: PropTypes.instanceOf(Element) })]),
  message: PropTypes.shape({
    time: PropTypes.string,
    reportId: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    displayId: PropTypes.string,
