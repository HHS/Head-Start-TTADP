...
function Landing() {
  ...
  const requestAlertsSort = (sortBy) => {
    let direction = 'asc'
    if (alertsSortConfig && alertsSortConfig.sortBy === sortBy && alertsSortConfig.direction === 'asc') {
      direction = 'desc'
    }
  ...
  return (
    <>
      ...
      <>
        ...
        <Grid row gap>
          <Grid col={12} className="display-flex flex-wrap">
            ...
            <div className="margin-bottom-2">
              {reportAlerts && reportAlerts.length > 0 && hasReadWrite(user) && appliedRegionNumber !== 14 && <NewActivityReportButton />}
            </div>
          </Grid>
        ...
        <DashboardOverview filters={filtersToApply} fields={['Grants served', 'Activity reports', 'Participants', 'Hours of TTA']} showTooltips />
        <Grid row>
          {error && (
