...
describe('EventCards', () => {
  ...
  const renderEventCards = (events = defaultEvents, eventType = EVENT_STATUS.NOT_STARTED, user = DEFAULT_USER, onDeleteEvent = jest.fn()) => {
    render(
      <MemoryRouter>
  ...
  it('admins see edit and delete event', () => {
    ...
    renderEventCards(collaboratorEvents, EVENT_STATUS.NOT_STARTED, COLLABORATOR_USER, deleteFunction)
  ...
  it('renders an Alert message if there is one', () => {
    const renderECWithAlert = (events = defaultEvents, eventType = EVENT_STATUS.NOT_STARTED, user = DEFAULT_USER, onDeleteEvent = jest.fn()) => {
      render(
        <MemoryRouter>
          <UserContext.Provider value={{ user }}>
            <EventCards
