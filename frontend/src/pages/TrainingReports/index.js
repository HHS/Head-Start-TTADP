...
const MESSAGE_TEMPLATES = {
  eventSubmitted: (_sessionName, eventId, dateStr) => (
    <>
      You submitted Training Event <Link to={`/training-report/view/${eventId.split('-').pop()}`}>{eventId}</Link> on {dateStr}
    </>
  ),
  sessionCreated: (_sessionName, eventId, dateStr) => (
    <>
      You created a session for Training Event <Link to={`/training-report/view/${eventId.split('-').pop()}`}>{eventId}</Link> on {dateStr}
    </>
  ),
  sessionSubmitted: (sessionName, eventId, dateStr) => (
    <>
      You submitted the session {sessionName} of Training Event <Link to={`/training-report/view/${eventId.split('-').pop()}`}>{eventId}</Link> on{' '}
      {dateStr}
    </>
  ),
  sessionReviewSubmitted: (sessionName, eventId, dateStr) => (
    <>
      Your review for session {sessionName} of Training Event <Link to={`/training-report/view/${eventId.split('-').pop()}`}>{eventId}</Link> was
...
export const evaluateMessageFromHistory = (history) => {
  ...
  if (state && state.message) {
    ...
    if (MESSAGE_TEMPLATES[messageTemplate]) {
      return MESSAGE_TEMPLATES[messageTemplate](message.sessionName, message.eventId, message.dateStr)
    }
        ...
        <>
          ...
          {msg && (
            <Alert
              ...
              cta={
                <Button role="button" unstyled aria-label="dismiss alert" onClick={() => setMsg(null)}>
          ...
          <Grid>
            ...
            <Grid row>
              <WidgetContainer title="Events" loading={false} loadingLabel="Training events loading" showPaging={false} showHeaderBorder={false}>
