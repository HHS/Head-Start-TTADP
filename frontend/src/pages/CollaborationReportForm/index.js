...
import { LOCAL_STORAGE_CR_DATA_KEY, LOCAL_STORAGE_CR_ADDITIONAL_DATA_KEY, LOCAL_STORAGE_CR_EDITABLE_KEY } from '../../Constants'
import { getRegionWithReadWrite } from '../../permissions'
import useTTAHUBLocalStorage from '../../hooks/useTTAHUBLocalStorage'
import { createReport, getReport, reviewReport, saveReport, submitReport } from '../../fetchers/collaborationReports'
...
export const formatReportWithSaveBeforeConversion = async (data, formData, user, userHasOneRole, reportId, forceUpdate) => {
  ...
  if (thereIsANeedToParseDates) {
    ...
    const isYMDFormat = updatedReport.startDate && updatedReport.startDate.match(/^\d{4}-\d{2}-\d{2}$/)
    if (isYMDFormat) {
      reportData = {
        ...reportData,
...
export const convertFormDataToReport = (data) => {
  const { participants, dataUsed, goals, reportGoals, statesInvolved, conductMethod, ...rest } = data

  const conductMethodValues = Array.isArray(conductMethod) ? conductMethod[0] : conductMethod || null
  ...
  if (goalsValues.length === 0) {
    reportGoalsToUse = reportGoalsValues
  }
...
function CollaborationReport({ match, location }) {
  const {
    params: { currentPage, collabReportId },
  } = match
  ...
  const [formData, updateFormData, localStorageAvailable] = useTTAHUBLocalStorage(LOCAL_STORAGE_CR_DATA_KEY(collabReportId), null)
  ...
  const [additionalData, updateAdditionalData] = useLocalStorage(LOCAL_STORAGE_CR_ADDITIONAL_DATA_KEY(collabReportId), {})
  ...
  const [editable, updateEditable] = useLocalStorage(
    LOCAL_STORAGE_CR_EDITABLE_KEY(collabReportId),
    collabReportId === 'new',
    currentPage !== 'review'
  )
      ...
      try {
        ...
        if (collabReportId !== 'new') {
          ...
          try {
            ...
            const { submissionStatus, approvers, calculatedStatus } = fetchedReport
        ...
        const apiCalls = [getCollaborators(report.regionId), getApprovers(report.regionId), getGoalTemplates([], false)]
        ...
        const isCollaborator = report.collabReportSpecialists && report.collabReportSpecialists.some((u) => u.specialistId === user.id)
  ...
  if (!currentPage) {
    return <Redirect to={`/collaboration-reports/${collabReportId}/activity-summary`} />
  }
  ...
  const onSave = async (data, forceUpdate = false) => {
    try {
      if (reportId.current === 'new') {
        ...
        const updatedReport = await formatReportWithSaveBeforeConversion(formDataValues, formData, user, userHasOneRole, reportId, forceUpdate)
  ...
  const onSaveDraft = async (isAutoSave = false, forceUpdate = false) => {
    try {
      ...
      updateErrorMessage(
        'A network error has prevented us from saving your collaboration report to our database. Your work is safely saved to your web browser in the meantime.'
      )
    } finally {
      setIsAppLoading(false)
    }
  ...
  return (
    <div className="smart-hub-collab-report" ref={formRef} tabIndex={-1}>
      ...
      {collabReportId !== 'new' && (
        <MeshPresenceManager room={`cr-${collabReportId}`} onPresenceUpdate={handlePresenceUpdate} onRevisionUpdate={handleRevisionUpdate} />
      )}
      ...
      <Grid row className="flex-justify">
        <Grid col="auto">
          <div className="margin-top-3 margin-bottom-5">
            <h1 className="font-serif-2xl text-bold line-height-serif-2 margin-0">Collaboration report for Region {formData?.regionId}</h1>
            {author}
          </div>
        </Grid>
        {!hideSideNav && (
          <Grid col="auto" className="flex-align-self-center">
            {formData?.calculatedStatus && (
              <div className={`${tagClass} smart-hub-status-label bg-gray-5 padding-x-2 padding-y-105 font-sans-md text-bold`}>
