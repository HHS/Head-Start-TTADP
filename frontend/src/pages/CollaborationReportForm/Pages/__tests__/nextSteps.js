...
jest.mock(
  ...
  () =>
    function MockNavigatorButtons({ isAppLoading, onContinue, onSaveDraft, onUpdatePage, path, position }) {
      return (
...
describe('nextSteps Page', () => {
  ...
  describe('NextSteps Component', () => {
    it('renders Helmet with correct title', () => {
      render(
        <TestWrapper>
          {nextStepsPage.render({}, {}, 1, false, mockOnContinue, mockOnSaveDraft, mockOnUpdatePage, false, '', jest.fn(), mockAlert)}
        </TestWrapper>
      )
  ...
  describe('getNextStepsSections Function', () => {
    ...
    it('handles null steps', () => {
      render(<TestWrapper defaultValues={{ steps: null }}>{nextStepsPage.reviewSection()}</TestWrapper>)
  ...
  describe('Edge Cases', () => {
    it('handles steps with missing properties', () => {
      const steps = [{ collabStepDetail: 'Only detail' }, { collabStepCompleteDate: '2024-01-15' }, {}]
