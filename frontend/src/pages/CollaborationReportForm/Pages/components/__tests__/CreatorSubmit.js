...
jest.mock(
  ...
  () =>
    function MockIncompletePages({ incompletePages }) {
      return <div data-testid="incomplete-pages">Incomplete Pages: {incompletePages.join(', ')}</div>
    }
)
...
jest.mock(
  ...
  () =>
    function MockApproverSelect({ name, valueProperty, labelProperty, options }) {
      return (
        <select name={name} data-testid="approver-select" data-value-property={valueProperty} data-label-property={labelProperty}>
          {options.map((option) => (
