...
const TopAlert = ({ author, user, isNeedsAction, pendingApprovalCount, approvers }) => {
  const formatNeedsActionApprovers = () => {
    ...
    const needActionApprovers = approversList.filter((a) => a.status === REPORT_STATUSES.NEEDS_ACTION)

    if (!needActionApprovers || needActionApprovers.length === 0) {
      return 'Changes have been requested for the Collaboration Report.'
    }

    const approverNames = needActionApprovers.filter((a) => a.user && a.user.fullName).map((a) => a.user.fullName)
...
const Review = ({
  ...
  const otherManagerNotes = approverStatusList ? approverStatusList.filter((a) => a.note && a.user.id !== user.id) : null
  ...
  return (
    <>
      ...
      {isSubmitted && (
        <TopAlert pendingApprovalCount={pendingApprovalCount} isNeedsAction={isNeedsAction} author={author} user={user} approvers={approvers} />
      )}
