...
import { Form, Fieldset, Button, Alert } from '@trussworks/react-uswds'
    ...
    <DismissingComponentWrapper shown={showSavedDraft} updateShown={updateShowSavedDraft} hideFromScreenReader={false}>
      {lastSaveTime && (
        <Alert className="margin-top-3 maxw-mobile-lg" noIcon slim type="success" aria-live="off">
          Draft saved on {/* eslint-disable-next-line react/prop-types */}
  ...
  return (
    <>
      ...
      <Form className="smart-hub--form-large" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button id={`draft-${path}-save-draft`} className="usa-button--outline" type="button" onClick={() => onSaveDraft()}>
          Save draft
        </Button>
        <Button
          id={`draft-${path}-back`}
          ...
          type="button"
          onClick={() => {
            onUpdatePage(PREVIOUS_POSITION)
          }}
        >
          Back
        </Button>
