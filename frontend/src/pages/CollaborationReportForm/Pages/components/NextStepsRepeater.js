...
import { FormGroup, Label, Button, Textarea, ErrorMessage } from '@trussworks/react-uswds'
...
export default function NextStepsRepeater({ name, ariaName, required }) {
  ...
  const onAddNewStep = () => {
    ...
    const canAdd = fieldArray.every((field, index) => {
      ...
      const isValid = !(errors[name]?.[index]?.collabStepDetail || errors[name]?.[index]?.collabStepCompleteDate)
    ...
    <>
      <div className="ttahub-next-steps-repeater">
        {fields.map((item, index) => (
          <div key={item.key}>
            <FormGroup className="margin-top-2 margin-bottom-2" error={errors[name] && errors[name][index] && errors[name][index].collabStepDetail}>
              <Label htmlFor={`next-step-${index + 1}`}>
                {`Step ${index + 1}`}
                {required && <Req />}
              </Label>
              {errors[name] && errors[name][index] && errors[name][index].collabStepDetail ? <ErrorMessage>Enter a next step</ErrorMessage> : null}
              <div className={`display-flex ${errors[name] && errors[name][index] && errors[name][index].collabStepDetail ? 'blank-next-step' : ''}`}>
                <Textarea
            ...
            <FormGroup
              ...
              error={errors[name] && errors[name][index] && errors[name][index].collabStepCompleteDate}
            >
              ...
              <div className={errors[name] && errors[name][index] && errors[name][index].collabStepCompleteDate ? 'blank-next-step-date' : ''}>
      ...
      <PlusButton onClick={onAddNewStep} text="Add next step" testId="NextSteps-button" className="margin-bottom-2" />
    </>
  )
}
