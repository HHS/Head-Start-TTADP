...
const ReportComponent = ({
  ...
  <Router history={history}>
    <AppLoadingContext.Provider value={{ isAppLoading: true, setIsAppLoading: jest.fn(), setAppLoadingText: jest.fn() }}>
...
describe('CollaborationReportForm', () => {
  ...
  it('renders with multiple user roles without setting creator role', async () => {
    ...
    render(<ReportComponent id="new" userRoles={[{ fullName: 'Health Specialist' }, { fullName: 'Education Specialist' }]} />)
  ...
  describe('formatReportWithSaveBeforeConversion', () => {
    ...
    it('sets creatorRole when user has one role and data has no creatorRole', async () => {
      const result = await formatReportWithSaveBeforeConversion(mockData, mockFormData, mockUser, true, { current: 'test-id' }, false)
    ...
    it('preserves existing creatorRole when data already has one', async () => {
      ...
      const result = await formatReportWithSaveBeforeConversion(dataWithCreatorRole, mockFormData, mockUser, true, { current: 'test-id' }, false)
    ...
    it('does not set creatorRole when user has multiple roles', async () => {
      ...
      const result = await formatReportWithSaveBeforeConversion(mockData, mockFormData, multiRoleUser, false, { current: 'test-id' }, false)
    ...
    it('calls saveReport when changes are detected', async () => {
      ...
      const originalModule = await import('../formDataHelpers')
      const findWhatsChangedSpy = jest.spyOn(originalModule, 'findWhatsChanged').mockReturnValue({ calculatedStatus: REPORT_STATUSES.SUBMITTED })
    ...
    it('calls saveReport when forceUpdate is true even with no changes', async () => {
      ...
      const result = await formatReportWithSaveBeforeConversion(mockData, mockFormData, mockUser, true, { current: 'test-id' }, true)
    ...
    it('uses real findWhatsChanged implementation to test integration', async () => {
      ...
      const result = await formatReportWithSaveBeforeConversion(dataWithChanges, mockFormData, mockUser, true, { current: 'test-id' }, false)
  ...
  describe('onFormSubmit functionality', () => {
    ...
    it('submits with multiple approvers', async () => {
      fetchMock.post('/api/collaboration-reports/123/submit', {
        calculatedStatus: REPORT_STATUSES.SUBMITTED,
        approvers: [{ user: { id: 1, name: 'Approver 1' } }, { user: { id: 2, name: 'Approver 2' } }],
      })
