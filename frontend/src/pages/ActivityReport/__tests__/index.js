...
import { history, formData, renderActivityReport, recipients, mockGoalsAndObjectives } from '../testHelpers'
...
describe('ActivityReport', () => {
  ...
  describe('reason for activity', () => {
    it('shows the reason for activity', async () => {
      ...
      const formGroup = formGroups.find(
        (group) => group.textContent.includes('Why was this activity requested?') && within(group).queryByRole('combobox') !== null
      )
  ...
  describe('localStorage error handling', () => {
    it('handles localStorage errors gracefully when loading stored data', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error loading from localStorage during fetch:', expect.any(Error))
