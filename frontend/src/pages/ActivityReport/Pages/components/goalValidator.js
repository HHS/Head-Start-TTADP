import { validateListOfResources, GOAL_NAME_ERROR } from '../../../../components/GoalForm/constants'
 ...
 * @returns boolean
 */
export const validateOnlyWithFlag = (user, flag, value, validator = Boolean) => {
...
export const unfinishedObjectives = (objectives, setError = () => {}, fieldArrayName = 'goalForEditing.objectives', isMonitoringGoal = false) => {
  const unfinished = objectives.some((objective, index) => {
    ...
    if (isMonitoringGoal && (!objective.citations || (objective.citations && !objective.citations.length))) {
