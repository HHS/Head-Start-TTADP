...
export const OBJECTIVE_RESOURCES =
  'Each resource should be a valid link. Invalid resources will not be saved.';
...
export const unfinishedObjectives = (
  ...
  const unfinished = objectives.some((objective, index) => {
    ...
    if (
      isMonitoringGoal &&
      (!objective.citations || (objective.citations && !objective.citations.length))
    ) {
...
export const unfinishedGoals = (goals, setError = () => {}) => {
  for (let i = 0; i < goals.length; i += 1) {
    ...
    if (goal.objectives && goal.objectives.length > 0) {
      ...
      const objectivesUnfinished = unfinishedObjectives(
        goal.objectives,
        setError,
