...
export default function NextStepsRepeater({ name, ariaName, recipientType, required }) {
  ...
  const showCompleteDateError = (errorsObj, fieldName, idx) => {
    const hasCompleteDateError = errorsObj[fieldName] && errorsObj[fieldName][idx] && errorsObj[fieldName][idx].completeDate
    ...
    <>
      <div className="ttahub-next-steps-repeater">
        {fields.map((item, index) => (
          <div key={item.key}>
            <FormGroup className="margin-top-2 margin-bottom-2" error={errors[name] && errors[name][index] && errors[name][index].note}>
              <Label htmlFor={`${stepType}-next-step-${index + 1}`}>
                {`Step ${index + 1}`}
                {required && <Req />}
              </Label>
              {errors[name] && errors[name][index] && errors[name][index].note ? <ErrorMessage>Enter a next step</ErrorMessage> : null}
              <div className={`display-flex ${errors[name] && errors[name][index] && errors[name][index].note ? 'blank-next-step' : ''}`}>
                <Textarea
            ...
            <FormGroup className="margin-top-1 margin-bottom-2" error={showCompleteDateError(errors, name, index)}>
              <Label htmlFor={`${stepType}-next-step-date-${index + 1}`}>
                {dateLabel(index)}
                {required && <Req announce />}
              </Label>
              {showCompleteDateError(errors, name, index) ? <ErrorMessage>Enter a valid date</ErrorMessage> : null}
              <div className={showCompleteDateError(errors, name, index) ? 'blank-next-step-date maxw-mobile' : 'maxw-mobile'}>
