...
import { OBJECTIVE_TITLE, OBJECTIVE_TTA, OBJECTIVE_TOPICS, OBJECTIVE_CITATIONS } from './goalValidator'
...
export default function Objective({
  ...
  const citationNames = useMemo(() => rawCitations.map((rawCitation) => rawCitation.citation), [rawCitations])
   ...
   */
  const {
    field: { onChange: onChangeCitations, onBlur: onBlurCitations, value: objectiveCitations, name: objectiveCitationsInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeTitle, onBlur: onBlurTitle, value: objectiveTitle, name: objectiveTitleInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeTopics, onBlur: onBlurTopics, value: objectiveTopics, name: objectiveTopicsInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeResources, onBlur: onBlurResources, value: objectiveResources, name: objectiveResourcesInputName },
  } = useController({
    ...
    rules: {
      validate: {
        ...
        allResourcesAreValid: (value) =>
          validateListOfResources(value) || 'Enter one resource per field. Valid resource links must start with http:// or https://',
      },
    },
  ...
  const {
    field: { onChange: onChangeFiles, onBlur: onBlurFiles, value: objectiveFiles, name: objectiveFilesInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeUseIpdCourses, onBlur: onBlurUseIpdCourses, value: objectiveUseIpdCourses, name: objectiveUseIpdCoursesInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeIpdCourses, onBlur: onBlurIpdCourses, value: objectiveIpdCourses, name: objectiveIpdCoursesInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeTta, onBlur: onBlurTta, value: objectiveTta, name: objectiveTtaInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeSupportType, onBlur: onBlurSupportType, value: supportType, name: supportTypeInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeStatus, onBlur: onBlurStatus, value: objectiveStatus, name: objectiveStatusInputName },
  } = useController({
  ...
  const {
    field: { onChange: onChangeSuspendReason, value: objectiveSuspendReason, name: objectiveSuspendInputName },
  } = useController({
    rules: {
  ...
  const {
    field: { onChange: onChangeSuspendContext, value: objectiveSuspendContext, name: objectiveSuspendContextInputName },
  } = useController({
  ...
  useDeepCompareEffect(() => {
    ...
    if (!objectiveCitations || !objectiveCitations.length || (selectedGoals && selectedGoals.length > 0)) {
  ...
  return (
    <>
      <ObjectiveSelect onChange={onChangeObjective} selectedObjectives={selectedObjective} options={options} onRemove={onRemove} />
      <FormFieldThatIsSometimesReadOnly
        label="TTA Objective"
        value={objectiveTitle}
        permissions={[
          createdHere,
          statusForCalculations !== OBJECTIVE_STATUS.COMPLETE && statusForCalculations !== OBJECTIVE_STATUS.SUSPENDED,
          !onApprovedAR,
        ]}
      >
        <ObjectiveTitle
          ...
          <span>
            ...
            <span className="margin-top-2 margin-bottom-0">
              ...
              <ul className="margin-top-2 margin-bottom-0">
                ...
                <li>
                  Remove the grant from the <Link to={`/activity-reports/${reportId}/activity-summary`}>Activity summary</Link>
                </li>
