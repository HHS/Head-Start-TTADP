...
const RecipientReviewSection = () => {
  ...
  const goals = hasLiveGoals ? [...(selectedGoals || []), ...(goalForEditing ? [goalForEditing] : [])] : goalsAndObjectives || []
              ...
              <div className="grid-col-12 desktop:grid-col-6 print:grid-col-6 padding-x-2">
                <div key={`${item.label}${v}`} className="desktop:flex-align-end display-flex flex-column flex-justify-center">
                  {v.responses.length ? (
                    ...
                    <div>
                      <FontAwesomeIcon className="margin-right-1" icon={faTriangleExclamation} /> Missing Information
                    </div>
                  )}
                </div>
                <div>
      ...
      >
        {objectiveSections.map((section) =>
          section.items.map((item) => {
            if (item.component) {
              return <item.component key={uuidv4()} label={item.label} name={item.name} customValue={objective} />
            }
        ...
        goals.map((goal, index) => {
          ...
          return (
            <Section
              ...
              isLastSection={false}
            >
              <div className="smart-hub-review-section margin-top-2 desktop:margin-top-0 margin-bottom-3">{goalReview}</div>
              {objectiveReview}
            </Section>
          )
        })
