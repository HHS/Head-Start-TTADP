...
const GoalsObjectives = ({ reportId, isAppLoading, onContinue, onSaveDraft, onUpdatePage, weAreAutoSaving }) => {
    ...
    if (messages.length > 0) {
      return (
        <Alert className="maxw-desktop" type="info">
          To add goals and objectives, indicate in the <Link to={`/activity-reports/${reportId}/activity-summary`}>Activity Summary</Link>:{' '}
          <ul>
  ...
  return (
    <>
       ...
       */}
      {hasGrant && isGoalFormClosed && <PlusButton onClick={addNewGoal} className="ttahub-plus-button-no-margin-top" text="Add new goal" />}
