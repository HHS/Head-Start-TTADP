...
const HtmlReviewItem = ({ label, name }) => {
  ...
  values = values.map((v, index) => {
    ...
    return <Editor readOnly toolbarHidden defaultEditorState={defaultEditorState} ariaLabel={values.length > 1 ? `${label} ${index + 1}` : label} />
  })
  ...
  return (
    <div className={`grid-row ${classes} margin-bottom-3 desktop:margin-bottom-0`}>
      <div className="grid-col-12 desktop:grid-col-6 print:grid-col-6  font-sans-2xs desktop:font-sans-sm text-bold desktop:text-normal">{label}</div>
