...
import { Form, Fieldset, Button, Alert, Dropdown } from '@trussworks/react-uswds'
...
const Draft = ({
  ...
  const allGoalsHavePromptResponses = (() => {
    const curatedGoals = (goalsAndObjectives || []).filter((goal) => goal.prompts && goal.prompts.length > 0)
    if (!curatedGoals.length) return true

    return curatedGoals.every((goal) =>
      goal.prompts.every((prompt) => {
  ...
  const canSubmitReport = allGoalsHavePromptResponses && !hasIncompletePages && !grantsMissingMonitoring.length && !grantsMissingCitations.length
  ...
  return (
    <>
      ...
      {reviewItems && reviewItems.length > 0 && <Accordion bordered items={reviewItems} multiselectable />}
      <Form className="smart-hub--form-large smart-hub--form__draft smart-hub--form" onSubmit={handleSubmit(onSubmit)}>
        {showRolesDropdown ? (
          <Fieldset className="smart-hub--report-legend margin-top-4 smart-hub--report-legend__no-legend-margin-top" legend="Creator Role">
            <FormItem label="Creator role" name="creatorRole" required>
              <Dropdown id="creatorRole" name="creatorRole" inputRef={register({ required: 'Select one' })}>
        ...
        <Fieldset className={`smart-hub--report-legend margin-top-0 ${!showRolesDropdown ? 'smart-hub--report-legend__no-legend-margin-top' : ''}`}>
          <FormItem label="Creator notes" name="additionalNotes" required={false} formGroupClassName="margin-top-4">
        ...
        <Fieldset className="smart-hub--report-legend">
          {!connectionActive && <ConnectionError />}
          <FormItem label="Approving manager" name="approvers" formGroupClassName="margin-top-4">
            <ApproverSelect
              name="approvers"
        ...
        <Button className="draft-button-margin" disabled={!connectionActive} id="draft-review-submit" type="submit">
          Submit for approval
        </Button>
        {!connectionActive && (
          <Alert type="warning" noIcon>
            ...
            <br />
            {localStorageAvailable ? 'Your work is saved on this computer.' : ''} <br />
            If you continue to have problems, <a href="https://app.smartsheetgov.com/b/form/f0b4725683f04f349a939bd2e3f5425a">contact us</a>.
          </Alert>
        )}
        <Button
      ...
      <DismissingComponentWrapper shown={showSavedDraft} updateShown={updateShowSavedDraft}>
        {lastSaveTime && (
