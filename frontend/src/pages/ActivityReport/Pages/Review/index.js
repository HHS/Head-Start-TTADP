...
const ReviewSubmit = ({
  ...
  const isCollaborator = activityReportCollaborators && activityReportCollaborators.find((u) => u.userId === user.id)
  ...
  const editing = calculatedStatus === REPORT_STATUSES.DRAFT || calculatedStatus === REPORT_STATUSES.NEEDS_ACTION
  const items = editing
    ? reviewItems.map((ri) => ({
        ...ri,
        expanded: false,
