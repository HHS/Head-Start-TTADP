...
import { Dropdown, Form, Label, Fieldset, Button } from '@trussworks/react-uswds'
...
const Review = ({
  ...
  const otherManagerNotes = approverStatusList ? approverStatusList.filter((a) => a.user.id !== user.id) : null
  const thisApprovingManager = approverStatusList ? approverStatusList.filter((a) => a.user.id === user.id) : null
  const hasBeenReviewed = thisApprovingManager && thisApprovingManager.length > 0 && thisApprovingManager[0].status !== null
  const hasReviewNote = thisApprovingManager && thisApprovingManager.length > 0 && thisApprovingManager[0].note
  ...
  return (
    <>
      ...
      <div className="smart-hub--creator-notes" aria-label="additionalNotes">
        ...
        <Editor readOnly toolbarHidden defaultEditorState={defaultEditorState} ariaLabel="Creator notes" />
      </div>
      ...
      <Form className="smart-hub--form-large" onSubmit={handleSubmit(onFormReview)}>
        <Fieldset className="smart-hub--report-legend margin-top-4 smart-hub--report-legend__no-legend-margin-top">
          ...
          <div className={`margin-top-1 ${textAreaClass}`}>
            <HookFormRichEditor ariaLabel="Manager notes" id="note" name="note" defaultValue={hasReviewNote ? thisApprovingManager[0].note : null} />
          </div>
        </Fieldset>
        ...
        {!showDraftViewForApproverAndCreator ? (
          <>
            ...
            <FormItem name="status" label="Choose approval status" className="margin-bottom-3">
              <Dropdown
                id="status"
                name="status"
        ...
        <Button disabled={hasIncompletePages} type="submit">
          {hasBeenReviewed ? 'Update report' : 'Submit'}
        </Button>
      </Form>
    </>
  )
