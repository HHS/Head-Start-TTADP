...
const RenderApprover = ({
  ...
  return (
    <FormProvider {...hookForm}>
      <Approver onFormReview={onFormReview} reviewed={reviewed} isPendingApprover pages={pages} availableApprovers={[{ id: 1, name: 'Approver 1' }]}>
        <div>test</div>
      </Approver>
    </FormProvider>
  )
...
const renderReview = (calculatedStatus, onFormReview, reviewed, approvers = defaultApprover, pages = defaultPages, extraFormData = {}) => {
  const formData = {
  ...
  render(
    <Router history={history}>
      <UserContext.Provider value={{ user }}>
        <RenderApprover onFormReview={onFormReview} reviewed={reviewed} formData={formData} pages={pages} />
...
describe('Approver review page', () => {
  describe('when the report is submitted', () => {
    ...
    it('hides date submitted if missing', async () => {
      renderReview(REPORT_STATUSES.SUBMITTED, () => {}, true, defaultApprover, defaultPages, { submittedDate: null })
