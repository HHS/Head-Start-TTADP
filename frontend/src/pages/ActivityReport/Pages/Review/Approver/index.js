...
const Approver = ({ onFormReview, reviewed, children, error, isPendingApprover, pages, onFormSubmit, availableApprovers, reviewItems }) => {
  ...
  const review = calculatedStatus === REPORT_STATUSES.SUBMITTED || calculatedStatus === REPORT_STATUSES.NEEDS_ACTION
  ...
  const showDraftViewForApproverAndCreator = approverIsAlsoCreator && calculatedStatus === REPORT_STATUSES.DRAFT
  ...
  return (
    <>
      ...
      <Container skipTopPadding className="margin-bottom-0 padding-top-2 padding-bottom-5" skipBottomPadding paddingY={0}>
        ...
        {reviewed && review && <Redirect to={{ pathname: '/activity-reports', state: { message: { ...message, status: 'reviewed' } } }} />}

        {reviewed && approved && <Redirect to={{ pathname: '/activity-reports', state: { message: { ...message, status: 'approved' } } }} />}
