...
describe('PrintSummary', () => {
  it('when a reportCreator with name and role is provided, it renders that', () => {
    const reportCreator = { name: 'Lois Lane', roles: [{ fullName: 'Reporter' }] }

    render(<PrintSummary reportCreator={reportCreator} />)
  ...
  it('when a reportCreator is empty or not provided, it does not render the report creator row', () => {
    ...
    render(<PrintSummary reportCreator={{}} />)
    expect(screen.queryByText('Report Creator')).toBeNull()
  })

  it('when a reportCreator has empty name and role strings, it does not render the report creator row', () => {
    const emptyCreator = { name: '', roles: [] }
    render(<PrintSummary reportCreator={emptyCreator} />)
    expect(screen.queryByText('Report Creator')).toBeNull()

    const roleWithoutAName = { name: '', roles: [{ fullName: 'Ghost' }] }
    render(<PrintSummary reportCreator={roleWithoutAName} />)
    expect(screen.queryByText('Report Creator')).toBeNull()
  })
})
