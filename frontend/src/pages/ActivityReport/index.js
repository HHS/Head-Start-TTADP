...
import { LOCAL_STORAGE_AR_ADDITIONAL_DATA_KEY, LOCAL_STORAGE_AR_DATA_KEY, LOCAL_STORAGE_AR_EDITABLE_KEY } from '../../Constants'
import { getRegionWithReadWrite } from '../../permissions'
import { convertGoalsToFormData, convertReportToFormData, findWhatsChanged, packageGoals, extractGoalIdsInOrder } from './formDataHelpers'
import {
  submitReport,
  saveReport,
  getReport,
...
const pagesByPos = keyBy(
  pages.filter((p) => !p.review),
  (page) => page.position
)
const defaultPageState = mapValues(pagesByPos, () => NOT_STARTED)

export const formatReportWithSaveBeforeConversion = async (data, lastSavedData, user, userHasOneRole, reportId, approverIds, forceUpdate) => {
  ...
  const creatorRole = !data.creatorRole && userHasOneRole ? user.roles[0].fullName : data.creatorRole
...
function ActivityReport({ match, location, region }) {
  const {
    params: { currentPage, activityReportId },
  } = match
  ...
  const { presenceData, handlePresenceUpdate } = usePresenceData(setShouldAutoSave)
  ...
  const [initialAdditionalData, updateAdditionalData] = useLocalStorage(LOCAL_STORAGE_AR_ADDITIONAL_DATA_KEY(activityReportId), {
    recipients: {
      grants: [],
      otherEntities: [],
    },
  ...
  const [editable, updateEditable] = useLocalStorage(
    LOCAL_STORAGE_AR_EDITABLE_KEY(activityReportId),
    activityReportId === 'new',
    currentPage !== 'review'
  )
  ...
  const showLastUpdatedTime = (location.state && location.state.showLastUpdatedTime && connectionActive) || false
  ...
  useDeepCompareEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const apiCalls = [getRecips(), getCollaborators(report.regionId), getApprovers(report.regionId), getGroupsForActivityReport(report.regionId)]
        ...
        const isCollaborator = report.activityReportCollaborators && report.activityReportCollaborators.find((u) => u.userId === user.id)
        ...
        const canWriteAsCollaboratorOrAuthor =
          (isCollaborator || isAuthor) &&
          (report.calculatedStatus === REPORT_STATUSES.DRAFT || report.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION)

        const canWriteAsApprover =
          isMatchingApprover &&
          isMatchingApprover.length > 0 &&
          (report.calculatedStatus === REPORT_STATUSES.SUBMITTED || report.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION)
        ...
        try {
          ...
          if (stored) {
            ...
            const localTimestamp = localData && localData.savedToStorageTime ? new Date(localData.savedToStorageTime) : null

            const serverTimestamp = report.updatedAt ? new Date(report.updatedAt) : null

            // If localStorage data is newer, prefer it
            const shouldUseLocalStorage = localTimestamp && (!serverTimestamp || localTimestamp > serverTimestamp)
  ...
  if (formData && formData.calculatedStatus === REPORT_STATUSES.APPROVED) {
    return <Redirect to={`/activity-reports/view/${activityReportId}`} />
  }

  if (connectionActive && isCollaboratorOrCreator && formData && formData.calculatedStatus === REPORT_STATUSES.SUBMITTED && !isPendingApprover) {
    return <Redirect to={`/activity-reports/submitted/${activityReportId}`} />
  }

  const approverCanEdit =
    isApprover && formData && (formData.calculatedStatus === REPORT_STATUSES.SUBMITTED || formData.calculatedStatus === REPORT_STATUSES.NEEDS_ACTION)

  if (connectionActive && !editable && currentPage !== 'review' && !approverCanEdit) {
    return <Redirect to={`/activity-reports/${activityReportId}/review`} />
  }
  ...
  if (!currentPage) {
    return <Redirect to={`/activity-reports/${activityReportId}/activity-summary`} />
  }
  ...
  const onSave = async (data, forceUpdate = false) => {
    ...
    try {
      if (reportId.current === 'new') {
        ...
        const allGoalsInOrder = packageGoals(goals, goalForEditing, grantIds, goalForEditing?.prompts || [], goalForEditing?.originalIndex)
        ...
        if (JSON.stringify(goalOrder) !== JSON.stringify(savedReport.goalOrder)) {
          await saveReport(reportId.current, { goalOrder }, {})
        }
  ...
  const author = creatorNameWithRole ? (
    <>
      <hr />
      <p>
        <strong>Creator:</strong> {creatorNameWithRole}
      </p>
    </>
  ) : null
  ...
  const renderMultiUserAlert = () => {
    if (presenceData.hasMultipleUsers) {
      ...
      return <Alert type="warning">{usersText}</Alert>
    }
    return null
  }
  ...
  const renderMultipleTabAlert = () => {
    if (presenceData.tabCount > 1) {
      return (
        <Alert type="warning">
          You have this report open in multiple browser tabs. To prevent losing your work, please close the other tabs before continuing.
        </Alert>
      )
    }
    return null
  ...
  return (
    <div className="smart-hub-activity-report">
      ...
      {activityReportId !== 'new' && (
        <MeshPresenceManager room={`ar-${activityReportId}`} onPresenceUpdate={handlePresenceUpdate} onRevisionUpdate={handleRevisionUpdate} />
      )}
      ...
      <Grid row>
        <Grid col="auto">
          <div className="margin-top-3 margin-bottom-5">
            <h1 className="font-serif-2xl text-bold line-height-serif-2 margin-0">Activity report for Region {formData && formData.regionId}</h1>
            {author}
          </div>
        </Grid>
        <Grid col="auto" className="flex-align-self-center">
          {formData && formData.calculatedStatus && (
            <div
              className={`${tagClass} smart-hub-status-label smart-hub--status-draft bg-gray-5 padding-x-2 padding-y-105 font-sans-md text-bold margin-bottom-2 margin-left-2`}
            >
              {startCase(formData.calculatedStatus)}
            </div>
          )}
        </Grid>
      </Grid>
