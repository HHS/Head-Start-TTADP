...
export default function SessionReportFacilitation({ match }) {
  ...
  const { data: trainingReport, error, statusCode } = useFetch(null, async () => eventById(trainingReportId), [trainingReportId])
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <FormGroup error={fieldError}>
            <Fieldset className="margin-bottom-3">
              ...
              <ReactHookFormError errors={errors} name={INPUT_NAME} render={({ message }) => <ErrorMessage>{message}</ErrorMessage>} />
              <Radio
