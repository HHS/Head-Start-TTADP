...
export default function useGoalTemplates(selectedGrants, filterOutUsedTemplates = false, includeClosedSuspended = false) {
    ...
    async function fetchGoalTemplates() {
      try {
        ...
        if (filterOutUsedTemplates) {
          ...
          const filtered = templates.filter((template) => !template.goals || template.goals.every((goal) => goal.prestandard === true))
