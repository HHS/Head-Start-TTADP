...
export default function useExistingApprovers(options, fieldName = 'approvers', valueProperty = 'user.id', labelProperty = 'user.fullName') {
  ...
  const filteredOptions = useMemo(() => {
    ...
    const initialValues = (Array.isArray(initialValueRef.current) ? initialValueRef.current : [initialValueRef.current]).map((r) =>
      _.get(r, valueProperty)
    )
