...
export function cleanupLocalStorage(
  ...
  try {
    if (replacementKey) {
      ...
      window.localStorage.setItem(newAdditionalDataKey, window.localStorage.getItem(oldAdditionalDataKey))
    }
...
export default function useLocalStorageCleanup(
  ...
  useEffect(() => {
    if (formData && (formData.calculatedStatus === REPORT_STATUSES.APPROVED || formData.calculatedStatus === REPORT_STATUSES.SUBMITTED)) {
  ...
  const cleanup = useCallback(
    (reportIdToClean, dataKeyToClean) => cleanupLocalStorage(reportIdToClean, dataKeyToClean, data, editable, additional),
