...
describe('useFilters', () => {
  ...
  const wrapper = ({ children }) => (
    <MemoryRouter>
      <AriaLiveContext.Provider value={{ announce: jest.fn() }}>{children}</AriaLiveContext.Provider>
    </MemoryRouter>
  )
  ...
  it('should filter out region filter from filter config when a user has only one region', () => {
    ...
    const { result } = renderHook(() => useFilters(u, filterKey, manageRegions, [], [{ id: 'region' }]), { wrapper })
