...
describe('useEventAndSessionStaff', () => {
  ...
  describe('REGIONAL_PD_WITH_NATIONAL_CENTERS - isEvent=true or facilitation=national_center', () => {
    ...
    it('returns national center trainers with flat array structure', () => {
      ...
      expect(result.current.trainerOptions).not.toContainEqual(expect.objectContaining({ label: expect.any(String) }))
    })
  })
  ...
  describe('REGIONAL_PD_WITH_NATIONAL_CENTERS - facilitation=regional_tta_staff', () => {
    ...
    it('returns empty arrays in all scenarios for regional_tta_staff with empty trainer data', () => {
      ...
      useFetch.mockReturnValueOnce({ data: [], loading: false, error: null }).mockReturnValueOnce({ data: [], loading: false, error: null })
  ...
  describe('Return Value Structure Validation', () => {
    ...
    it('trainerOptions structure varies correctly by scenario', () => {
      ...
      const isFlatArray =
        Array.isArray(result1.current.trainerOptions) && result1.current.trainerOptions.length > 0 && !('label' in result1.current.trainerOptions[0])
      ...
      const { result: result2 } = renderHook(() => useEventAndSessionStaff(event, false), { wrapper: wrapper2 })

      const isGroupedArray =
        Array.isArray(result2.current.trainerOptions) && result2.current.trainerOptions.length > 0 && 'label' in result2.current.trainerOptions[0]
