...
import { getRecipient, getRecipientGoals, goalsByIdAndRecipient, getRecipientLeadership } from '../recipient'
...
describe('recipient fetcher', () => {
  ...
  it('goalsByIdAndRecipient throws when given NaN', async () => {
    await expect(goalsByIdAndRecipient([1, 2, 3], 'asdf')).rejects.toThrow('Recipient ID must be a number')
  })
