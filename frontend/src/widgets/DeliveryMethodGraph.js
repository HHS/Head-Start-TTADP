...
export default function DeliveryMethodGraph({ data }) {
  ...
  const { exportRows } = useWidgetExport(tabularData, TABLE_HEADINGS, checkboxes, 'Months', EXPORT_NAME)
  ...
  useEffect(() => {
    ...
    const { records: unfilteredRecords, filteredReports, showDashboardFiltersNotApplicable: showDashboardFiltersNotApplicableProp } = data
  ...
  const subtitle = (
    <div className="margin-bottom-3">
      <WidgetContainerSubtitle marginY={0}>
        How much TTA is being delivered in-person, virtually, or hybrid as reported on Activity Reports
  ...
  return (
    <WidgetContainer loading={false} title="Delivery method" subtitle={subtitle} menuItems={menuItems} titleMargin={{ bottom: 1 }}>
      <LineGraph
