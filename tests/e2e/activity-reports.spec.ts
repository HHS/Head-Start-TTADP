...
test.beforeAll(async ({ request }) => {
  ...
  const readScopes = [SCOPE_IDS.READ_ACTIVITY_REPORTS, SCOPE_IDS.READ_WRITE_ACTIVITY_REPORTS, SCOPE_IDS.APPROVE_ACTIVITY_REPORTS]
...
test.afterAll(async ({ request }) => {
  ...
  if (originalActivityReportPermissions.length > 0) {
    const values = originalActivityReportPermissions.map((p) => `(${p.userId}, ${p.regionId}, ${p.scopeId})`).join(', ')
    await query(
      request,
      `INSERT INTO "Permissions" ("userId", "regionId", "scopeId")
       VALUES ${values};`
